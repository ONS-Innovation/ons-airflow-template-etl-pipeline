...
from rich_argparse._common import (_HIGHLIGHTS, _fix_legacy_win_text,
                                   rich_fill, rich_strip, rich_wrap)
...
class RichHelpFormatter(argparse.HelpFormatter):
    ...
    def _rich_expand_help(self, action: Action) -> r.Text:
        ...
        parts = []
        defaults: list[str] = []
        default_sub_template = (
            "rich-argparse-f3ae8b55df34d5d83a8189d2e4766e68-{}-argparse-rich"
        )
        default_n = 0
        last = 0
                ...
                repl = next(
                    (
                        ...
                        for m in re.finditer(
                            rf"\[([^\]]*{printf_pat}[^\]]*)\]", help_string, re.X
                        )
                        if m.group("mapping") == "default"
                    ),
    ...
    def _rich_fill_text(self, text: r.Text, width: int, indent: r.Text) -> r.Text:
        return (
            rich_fill(self.console, self._rich_whitespace_sub(text), width, indent)
            + "\n\n"
        )
