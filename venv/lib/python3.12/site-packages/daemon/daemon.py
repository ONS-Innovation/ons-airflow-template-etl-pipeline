...
def get_stream_file_descriptors(
    ...
    file_descriptors = {
        fd
        for fd in {_get_file_descriptor(stream) for stream in {stdin, stdout, stderr}}
        if fd is not None
    }
...
def is_socket(fd):
    ...
    socket.
    """
    warnings.warn(DeprecationWarning("migrate to `is_socket_file` instead"))
...
def _validate_fd_values(fds):
    ...
    if invalid_fds:
        ...
        message = "not an integer file descriptor: {!r}".format(value_to_complain_about)
