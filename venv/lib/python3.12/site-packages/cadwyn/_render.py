...
def render_module_by_path(module_path: str, app_path: str, version: str):
    ...
    attributes_to_alter = [
        ...
        if lenient_issubclass(value, (Enum, BaseModel))
    ...
    try:
        ...
        raise CadwynRenderError(
            f"Failed to find the source for module {module.__name__}"
        ) from e

    return ast.unparse(
        ast.Module(
            body=[
                _render_model_from_ast(
                    node, getattr(module, node.name), app.versions, version
                )
...
def render_model(
    model: type[Union[BaseModel, Enum]], versions: VersionBundle, version: str
) -> str:
    try:
    ...
    return ast.unparse(
        _render_model_from_ast(original_cls_node, model, versions, version)
    )


def _render_model_from_ast(
    model_ast: ast.ClassDef,
    model: type[Union[BaseModel, Enum]],
...
def _render_enum_model(wrapper: _EnumWrapper, original_cls_node: ast.ClassDef):
    ...
    old_body = [
        ...
        if not isinstance(n, (ast.AnnAssign, ast.Assign, ast.Pass, ast.Constant))
    ]
...
def _render_pydantic_model(
    wrapper: _PydanticModelWrapper, original_cls_node: ast.ClassDef
):
    # This is for possible schema renaming
    ...
    docstring = pop_docstring_from_cls_body(old_body)
    original_cls_node.body = (
        docstring + field_definitions + validator_definitions + old_body
    )
    if not original_cls_node.body:
        original_cls_node.body = [ast.Pass()]
    return original_cls_node
