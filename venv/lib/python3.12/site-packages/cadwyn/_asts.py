...
def get_fancy_repr(value: Any) -> Any:
    if isinstance(value, annotated_types.GroupedMetadata) and hasattr(
...
def transform_grouped_metadata(value: "annotated_types.GroupedMetadata"):
    ...
    return PlainRepr(
        ...
        + "("
        + ", ".join(
            f"{PlainRepr(key)}={get_fancy_repr(v)}" for key, v in modified_fields
        )
        + ")",
    )
...
def transform_dict(value: dict) -> Any:
    return PlainRepr(
        value.__class__(
            (get_fancy_repr(k), get_fancy_repr(v)) for k, v in value.items()
        ),
    )
...
def transform_auto(
    _: auto,
) -> Any:  # pragma: no cover # it works but we no longer use auto
    return PlainRepr("auto()")


def transform_union(value: UnionType) -> Any:  # pragma: no cover
    return (
        "typing.Union[" + (", ".join(get_fancy_repr(a) for a in get_args(value))) + "]"
    )
...
def transform_lambda(value: LambdaType) -> Any:
    ...
    return _get_lambda_source_from_default_factory(
        inspect.getsource(value).strip(" \n\t.")
    )
