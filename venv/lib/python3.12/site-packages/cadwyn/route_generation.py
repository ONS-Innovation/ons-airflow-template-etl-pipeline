...
class VersionedAPIRouter(fastapi.routing.APIRouter):
    def only_exists_in_older_versions(self, endpoint: _Call) -> _Call:
        ...
        if _DELETED_ROUTE_TAG in route.tags:
            raise CadwynError(
                f'The route "{endpoint.__name__}" was already deleted. You can\'t delete it again.'
            )
...
class _EndpointTransformer(Generic[_R, _WR]):
    def __init__(
        self, parent_router: _R, versions: VersionBundle, webhooks: _WR
    ) -> None:
        super().__init__()
    ...
    def transform(self) -> GeneratedRouters[_R, _WR]:
        ...
        for version in self.versions:
            self.schema_generators[
                str(version.value)
            ].annotation_transformer.migrate_router_to_version(router)
            self.schema_generators[
                str(version.value)
            ].annotation_transformer.migrate_router_to_version(webhook_router)
            ...
            self._apply_endpoint_changes_to_router(
                router.routes + webhook_router.routes, version
            )
        ...
        for route_index, head_route in enumerate(self.parent_router.routes):
            ...
            for older_router in list(routers.values()):
                ...
                if (
                    older_route.body_field is not None
                    and _route_has_a_simple_body_schema(older_route)
                ):
                    if hasattr(
                        older_route.body_field.type_, "__cadwyn_original_model__"
                    ):
                ...
                _add_data_migrations_to_route(
                    ...
                    head_route,
                    template_older_body_model,
                    older_route.body_field.alias
                    if older_route.body_field is not None
                    else None,
                    copy_of_dependant,
                    self.versions,
                )
        for router in routers.values():
            router.routes = [
                ...
                if not (
                    ...
                    and _DELETED_ROUTE_TAG in route.tags
                )
            ]
    ...
    def _attach_routes_to_data_converters(
        self, router: APIRouter, head_router: APIRouter, version: Version
    ):
        ...
        for version_change in version.changes:
            ...
            for (
                by_schema_converters
            ) in version_change.alter_request_by_schema_instructions.values():
                for by_schema_converter in by_schema_converters:
                    if not by_schema_converter.check_usage:  # pragma: no cover
                        continue
                    missing_models = (
                        set(by_schema_converter.schemas) - head_request_bodies
                    )
                    if missing_models:
            ...
            for (
                by_schema_converters
            ) in version_change.alter_response_by_schema_instructions.values():
                for by_schema_converter in by_schema_converters:
                    if not by_schema_converter.check_usage:  # pragma: no cover
                        continue
                    missing_models = (
                        set(by_schema_converter.schemas) - head_response_models
                    )
                    if missing_models:
    ...
    def _attach_routes_by_path_converter(
        ...
        path_to_route_methods_mapping: dict[
            _RoutePath, dict[_RouteMethod, set[_RouteId]]
        ],
        version_change: type[VersionChange],
        by_path_converter: Union[
            _AlterResponseByPathInstruction, _AlterRequestByPathInstruction
        ],
    ):
        missing_methods = set()
        for method in by_path_converter.methods:
            if method in path_to_route_methods_mapping[by_path_converter.path]:
                for route_index in path_to_route_methods_mapping[
                    by_path_converter.path
                ][method]:
                    route = head_router.routes[route_index]
                    if isinstance(by_path_converter, _AlterResponseByPathInstruction):
                        version_change._route_to_response_migration_mapping[
                            id(route)
                        ].append(by_path_converter)
                    else:
                        version_change._route_to_request_migration_mapping[
                            id(route)
                        ].append(by_path_converter)
            else:
    ...
    def _extract_all_routes_identifiers_for_route_to_converter_matching(
        ...
        response_models = set()
        request_bodies = set()
        path_to_route_methods_mapping: dict[str, dict[str, set[int]]] = defaultdict(
            lambda: defaultdict(set)
        )

        for index, route in enumerate(router.routes):
            if isinstance(route, APIRoute):
                if route.response_model is not None and lenient_issubclass(
                ...
                if (
                    ...
                    if annotation is not None and lenient_issubclass(
        ...
        head_response_models = {
            model.__cadwyn_original_model__ for model in response_models
        }
        head_request_bodies = {
            getattr(body, "__cadwyn_original_model__", body) for body in request_bodies
        }
    ...
    def _apply_endpoint_changes_to_router(  # noqa: C901
        ...
        for version_change in version.changes:
            for instruction in version_change.alter_endpoint_instructions:
                ...
                methods_we_should_have_applied_changes_to = (
                    instruction.endpoint_methods.copy()
                )
                ...
                if isinstance(instruction, EndpointDidntExistInstruction):
                    ...
                    for deleted_route in deleted_routes:
                        ...
                        if len(routes_that_never_existed) == 1:
                            self.routes_that_never_existed.remove(
                                routes_that_never_existed[0]
                            )
                    ...
                    for original_route in original_routes:
                        ...
                        _apply_endpoint_had_instruction(
                            version_change.__name__, instruction, original_route
                        )
                    err = (
                ...
                method_diff = (
                    methods_we_should_have_applied_changes_to
                    - methods_to_which_we_applied_changes
                )
                if method_diff:
                    raise RouterGenerationError(
                        err.format(
...
def _add_data_migrations_to_route(
    ...
    if not (
        route.dependant.request_param_name and route.dependant.response_param_name
    ):  # pragma: no cover
        raise CadwynError(
        ...
        if attr is not Sentinel:
            ...
            if attr_name == "path":
                original_path_params = {
                    p.alias for p in original_route.dependant.path_params
                }
    ...
    *,
    ...
    return [
        ...
        if (
            ...
            and (
                endpoint_func_name is None
                or route.endpoint.__name__ == endpoint_func_name
            )
