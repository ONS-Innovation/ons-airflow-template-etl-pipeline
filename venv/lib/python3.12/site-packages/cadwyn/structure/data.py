...
class _AlterDataInstruction:
    ...
    def __call__(
        self, __request_or_response: Union[RequestInfo, ResponseInfo], /
    ) -> None:
...
@dataclass
class _AlterRequestBySchemaInstruction(
    _BaseAlterBySchemaInstruction, _BaseAlterRequestInstruction
):
    ...
...
def convert_request_to_next_version_for(
    ...
    *additional_schemas: type,
    ...
    _validate_decorator_args(
        schema_or_path, methods_or_second_schema, additional_schemas
    )
...
@dataclass
class _AlterResponseBySchemaInstruction(
    _BaseAlterBySchemaInstruction, _BaseAlterResponseInstruction
):
    ...
...
def _validate_decorator_args(
    ...
    if isinstance(schema_or_path, str):
        if not isinstance(methods_or_second_schema, list):
            raise TypeError(
                "If path was provided as a first argument, methods must be provided as a second argument"
            )
        _validate_that_strings_are_valid_http_methods(methods_or_second_schema)
        if additional_schemas:
            raise TypeError(
                "If path was provided as a first argument, then additional schemas cannot be added"
            )

    elif methods_or_second_schema is not None and not isinstance(
        methods_or_second_schema, type
    ):
        raise TypeError(
            "If schema was provided as a first argument, all other arguments must also be schemas"
        )
