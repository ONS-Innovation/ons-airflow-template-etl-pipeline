...
class AlterFieldInstructionFactory:
    ...
    def didnt_have(
        self, *attributes: PossibleFieldAttributes
    ...
    def didnt_exist(self) -> FieldDidntExistInstruction:
        return FieldDidntExistInstruction(
            is_hidden_from_changelog=False, schema=self.schema, name=self.name
        )
...
class AlterSchemaInstructionFactory:
    ...
    def validator(
        self,
        func: "Union[Callable[..., Any], classmethod[Any, Any, Any], PydanticDescriptorProxy]",
        /,
    ) -> AlterValidatorInstructionFactory:
        func = cast(
            "Union[Callable[..., Any], PydanticDescriptorProxy]",
        ...
        if not isinstance(func, PydanticDescriptorProxy):
            ...
            raise CadwynStructureError(
                "The passed function must be a pydantic validator"
            )
        return AlterValidatorInstructionFactory(self.schema, func)

    def had(self, *, name: str) -> SchemaHadInstruction:
        return SchemaHadInstruction(
            is_hidden_from_changelog=False, schema=self.schema, name=name
        )
