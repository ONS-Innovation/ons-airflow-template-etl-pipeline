...
APIVersionVarType: TypeAlias = Union[
    ContextVar[Union[VersionType, None]], ContextVar[VersionType]
]
...
class VersionChange:
    ...
    alter_schema_instructions: ClassVar[
        list[Union[AlterSchemaSubInstruction, SchemaHadInstruction]]
    ] = Sentinel
    ...
    _route_to_request_migration_mapping: ClassVar[
        dict[_RouteId, list[_AlterRequestByPathInstruction]]
    ] = Sentinel
    _route_to_response_migration_mapping: ClassVar[
        dict[_RouteId, list[_AlterResponseByPathInstruction]]
    ] = Sentinel
    ...
    def _extract_body_instructions_into_correct_containers(cls):
        for instruction in cls.__dict__.values():
            if isinstance(instruction, _AlterRequestBySchemaInstruction):
                ...
                cls.alter_request_by_path_instructions[instruction.path].append(
                    instruction
                )
            elif isinstance(instruction, _AlterResponseBySchemaInstruction):
                for schema in instruction.schemas:
                    cls.alter_response_by_schema_instructions[schema].append(
                        instruction
                    )
            elif isinstance(instruction, _AlterResponseByPathInstruction):
                cls.alter_response_by_path_instructions[instruction.path].append(
                    instruction
                )
    ...
    def _extract_list_instructions_into_correct_containers(cls):
        ...
        for alter_instruction in cls.instructions_to_migrate_to_previous_version:
            if isinstance(
                alter_instruction, SchemaHadInstruction
            ) or isinstance(  # noqa: SIM101
...
class VersionChangeWithSideEffects(VersionChange, _abstract=True):
    ...
    def is_applied(
        ...
        if api_version is None:
            return True
        return (
            cls._bound_version_bundle._version_changes_to_version_mapping[cls]
            <= api_version
        )
...
class VersionBundle:
    ...
    def versioned_schemas(self) -> dict[IdentifierPythonPath, type[VersionedModel]]:
        ...
        migrated_schemas = {
            ...
            for version_change in version.changes
            for schema in list(
                version_change.alter_request_by_schema_instructions.keys()
            )
        }
        ...
        *,
        ...
        for v in self.reversed_versions[start + 1 :]:
            for version_change in v.changes:
                if (
                    ...
                    for (
                        instruction
                    ) in version_change.alter_request_by_schema_instructions[body_type]:
                        instruction(request_info)
                if head_route_id in version_change._route_to_request_migration_mapping:
                    for (
                        instruction
                    ) in version_change._route_to_request_migration_mapping[
                        head_route_id
                    ]:
                        instruction(request_info)
        ...
        if result.errors:
            raise CadwynHeadRequestValidationError(
                _normalize_errors(result.errors),
                body=request_info.body,
        ...
        def wrapper(endpoint: "Endpoint[_P, _R]") -> "Endpoint[_P, _R]":
            ...
            if request_param_name == _CADWYN_REQUEST_PARAM_NAME:
                _add_keyword_only_parameter(
                    decorator, _CADWYN_REQUEST_PARAM_NAME, FastapiRequest
                )
            if response_param_name == _CADWYN_RESPONSE_PARAM_NAME:
                _add_keyword_only_parameter(
                    decorator, _CADWYN_RESPONSE_PARAM_NAME, FastapiResponse
                )
    ...
    async def _convert_endpoint_response_to_version(  # noqa: C901
        ...
        try:
            if is_async_callable(func_to_get_response_from):
                response_or_response_body: Union[
                    FastapiResponse, object
            ...
            if (raised_exception.headers or {}).get(
                "content-length"
            ) is None:  # pragma: no branch
        ...
        if isinstance(response_or_response_body, FastapiResponse):
            ...
            if isinstance(response_or_response_body, (StreamingResponse, FileResponse)):
                ...
                ) and isinstance(response_or_response_body.body, (str, bytes)):
                    body = json.loads(response_or_response_body.body)
                elif isinstance(response_or_response_body.body, bytes):
                    body = response_or_response_body.body.decode(
                        response_or_response_body.charset
                    )
        ...
        if isinstance(response_or_response_body, FastapiResponse):
            ...
            if response_info.body is not None and hasattr(
                response_info._response, "body"
            ):
                # TODO (https://github.com/zmievsa/cadwyn/issues/51): Only do this if there are migrations
                if (
                    isinstance(response_info.body, str)
                    and response_info._response.headers.get("content-type")
                    != "application/json"
                ):
                ...
                if response_info.headers.get("content-length") is not None:
                    ...
                    response_info.headers["content-length"] = str(
                        len(response_info._response.body)
                    )

            if raised_exception is not None and response_info.status_code >= 400:
                if (
                    isinstance(response_info.body, dict)
                    and "detail" in response_info.body
                ):
...
async def _get_body(
    request: FastapiRequest,
    body_field: Union[ModelField, None],
    exit_stack: AsyncExitStack,
