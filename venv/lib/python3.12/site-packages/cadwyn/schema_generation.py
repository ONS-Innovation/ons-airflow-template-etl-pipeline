...
from fastapi.dependencies.utils import (is_async_gen_callable,
                                        is_coroutine_callable, is_gen_callable)
...
dict_of_empty_field_info = {
    k: getattr(_empty_field_info, k) for k in FieldInfo.__slots__
}
...
class PydanticFieldWrapper:
    ...
    def __post_init__(self, init_model_field: FieldInfo):
        self.passed_field_attributes = _extract_passed_field_attributes(
            init_model_field
        )
...
def _extract_passed_field_attributes(field_info: FieldInfo) -> dict[str, object]:
    return {
        k: v
        for k, v in (
            field_info._attributes_set | collect_known_metadata(field_info.metadata)[0]
        ).items()
...
def migrate_response_body(
    ...
    *,
    ...
    versioned_response_model: type[pydantic.BaseModel] = generate_versioned_models(
        versions
    )[str(version)][latest_response_model]
...
def _unwrap_model(model: type[_T_ANY_MODEL]) -> type[_T_ANY_MODEL]:
    while hasattr(model, "__cadwyn_original_model__"):
        model = (
            model.__cadwyn_original_model__
...
def _wrap_validator(
    ...
    if is_pydantic_v1_style_validator:
        ...
        if (
            isinstance(decorator_info, RootValidatorDecoratorInfo)
            and decorator_info.mode == "after"
...
def _wrap_pydantic_model(
    model: type[_T_PYDANTIC_MODEL],
) -> "_PydanticModelWrapper[_T_PYDANTIC_MODEL]":
    ...
    for decorator_wrapper in decorators:
        ...
        wrapped_validator = _wrap_validator(
            decorator_wrapper.func, decorator_wrapper.shim, decorator_wrapper.info
        )
    ...
    other_attributes = {
        ...
        and not (
            _is_dunder(attr_name)
            or attr_name in {"_abc_impl", "model_fields", "model_computed_fields"}
        )
    }
...
@cache
def _get_field_and_validator_names_from_model(
    cls: type,
) -> tuple[set[_FieldName], set[str]]:
...
def _get_validator_info_or_none(method: ast.FunctionDef) -> Union[str, None]:
    for decorator in method.decorator_list:
        ...
        if (
            ...
            and ast.unparse(decorator.func).endswith("validator")
        ) or (
            isinstance(decorator, (ast.Name, ast.Attribute))
            and ast.unparse(decorator).endswith("validator")
        ):
            return method.name
    return None
...
class _PydanticModelWrapper(Generic[_T_PYDANTIC_MODEL]):
    ...
    def __post_init__(self):
        ...
        while hasattr(self.cls, "__cadwyn_original_model__"):  # pragma: no cover
            self.cls = (
                self.cls.__cadwyn_original_model__
            )  # pyright: ignore[reportAttributeAccessIssue]
        ...
        for k, annotation in self.annotations.items():
            if get_origin(annotation) == Annotated:
                ...
                self.annotations[k] = _AnnotatedAlias(
                    copy.deepcopy(sub_annotations[0]),
                    tuple(copy.deepcopy(sub_ann) for sub_ann in sub_annotations[1:]),
                )
    ...
    def generate_model_copy(
        ...
        root_validators = {
            ...
            for name, validator in self.validators.items()
            if not validator.is_deleted
            and type(validator) == _ValidatorWrapper  # noqa: E721
        }
        fields = {
        ...
        model_copy = type(self.cls)(
            self.name,
            tuple(
                generator[cast("type[BaseModel]", base)] for base in self.cls.__bases__
            ),
            ...
            | {
                ...
                "__doc__": self.doc,
                "__qualname__": self.cls.__qualname__.removesuffix(self.cls.__name__)
                + self.name,
            },
        )
...
class _AnnotationTransformer:
    ...
    def change_version_of_annotation(self, annotation: Any) -> Any:
        ...
        if isinstance(annotation, dict):
            return {
                self.change_version_of_annotation(
                    key
                ): self.change_version_of_annotation(value)
                for key, value in annotation.items()
            }

        elif isinstance(annotation, (list, tuple)):
            return type(annotation)(
                self.change_version_of_annotation(v) for v in annotation
            )
        else:
    ...
    def migrate_route_to_version(
        self, route: fastapi.routing.APIRoute, *, ignore_response_model: bool = False
    ):
        if route.response_model is not None and not ignore_response_model:
            route.response_model = self.change_version_of_annotation(
            ...
            route.secure_cloned_response_field = fastapi.utils.create_cloned_field(
        ...
        for callback in route.callbacks or []:
            if not isinstance(callback, fastapi.routing.APIRoute):
                continue
            self.migrate_route_to_version(
                callback, ignore_response_model=ignore_response_model
            )
    ...
    def _change_version_of_a_non_container_annotation(self, annotation: Any) -> Any:
        ...
        if isinstance(annotation, (types.GenericAlias, _BaseGenericAlias)):
            return get_origin(annotation)[
                tuple(
                    self.change_version_of_annotation(arg)
                    for arg in get_args(annotation)
                )
            ]
        elif is_typealiastype(annotation):
            if (
                annotation.__module__ is not None
                and (annotation.__module__.startswith("pydantic."))
                ...
                return TypeAliasType(  # pyright: ignore[reportGeneralTypeIssues]
                    ...
                    type_params=self.change_version_of_annotation(
            ...
            return getitem(
                tuple(
                    self.change_version_of_annotation(a) for a in get_args(annotation)
                ),
            )
    ...
    def _remake_endpoint_dependencies(cls, route: fastapi.routing.APIRoute):
        ...
        route_copy = fastapi.routing.APIRoute(
            route.path, route.endpoint, dependencies=route.dependencies
        )
    ...
    def _modify_callable_annotations(  # pragma: no branch # because of lambdas
        ...
        *,
        ...
        callable_annotations = {
            k: v if type(v) is not str else _try_eval_type(v, call.__globals__)
            for k, v in callable_annotations.items()
        }
        annotation_modifying_wrapper.__annotations__ = modify_annotations(
            callable_annotations
        )
...
class SchemaGenerator:
    ...
    def __init__(self, model_bundle: _ModelBundle) -> None:
        ...
        self.concrete_models = {
            ...
            for k, wrapper in (
                self.model_bundle.schemas | self.model_bundle.enums
            ).items()
        }
...
def generate_versioned_models(
    ...
    context = _RuntimeSchemaGenContext(
        current_version=versions.head_version, models=models, version_bundle=versions
    )
    _migrate_classes(context)

    for version in versions.versions:
        context = _RuntimeSchemaGenContext(
            current_version=version, models=models, version_bundle=versions
        )
...
def _create_model_bundle(versions: "VersionBundle"):
    return _ModelBundle(
        ...
        schemas={
            schema: _wrap_pydantic_model(schema)
            for schema in versions.versioned_schemas.values()
        },
    )
...
def _apply_alter_schema_instructions(
    ...
    alter_schema_instructions: Sequence[
        Union[AlterSchemaSubInstruction, SchemaHadInstruction]
    ],
    ...
    for alter_schema_instruction in alter_schema_instructions:
        ...
        if isinstance(alter_schema_instruction, FieldExistedAsInstruction):
            ...
            _delete_field_from_model(
                schema_info, alter_schema_instruction.name, version_change_name
            )
        elif isinstance(alter_schema_instruction, ValidatorExistedInstruction):
            validator_name = get_name_of_function_wrapped_in_pydantic_validator(
...
def _add_field_to_model(
    ...
    field = PydanticFieldWrapper(
        alter_schema_instruction.field,
        alter_schema_instruction.field.annotation,
...
def _change_field_in_model(
    ...
    model.fields[alter_schema_instruction.name] = field
    model.annotations[alter_schema_instruction.name] = defined_annotations[
        alter_schema_instruction.name
    ]
...
def _delete_field_from_model(
    model: _PydanticModelWrapper, field_name: str, version_change_name: str
):
    if field_name in model.fields:
        ...
        for validator_name, validator in model.validators.copy().items():
            if (
                isinstance(validator, _PerFieldValidatorWrapper)
                and field_name in validator.fields
            ):
...
class _EnumWrapper(Generic[_T_ENUM]):
    ...
    def generate_model_copy(self, generator: "SchemaGenerator") -> type[_T_ENUM]:
        ...
        raw_member_map = {
            k: v.value if isinstance(v, Enum) else v for k, v in self.members.items()
        }
        initialization_namespace = (
            self._get_initialization_namespace_for_enum(self.cls) | raw_member_map
        )
        for attr_name, attr in initialization_namespace.items():
            enum_dict[attr_name] = attr
        enum_dict["__doc__"] = self.cls.__doc__
        model_copy = cast(
            "type[_T_ENUM]", type(self.name, self.cls.__bases__, enum_dict)
        )
        model_copy.__cadwyn_original_model__ = (
            self.cls
        )  # pyright: ignore[reportAttributeAccessIssue]
        return model_copy
