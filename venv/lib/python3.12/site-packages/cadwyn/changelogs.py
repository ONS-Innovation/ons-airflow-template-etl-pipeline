...
from fastapi._compat import (GenerateJsonSchema, ModelField,
                             get_compat_model_name_map, get_definitions)
...
def hidden(instruction_or_version_change: T) -> T:
    if isinstance(
        instruction_or_version_change,
        (staticmethod, ValidatorDidntExistInstruction, ValidatorExistedInstruction),
    ):
...
def _generate_changelog(
    versions: VersionBundle, router: _RootCadwynAPIRouter
) -> "CadwynChangelogResource":
    changelog = CadwynChangelogResource()
    schema_generators = generate_versioned_models(versions)
    for version, older_version in zip(
        versions, versions.versions[1:], **ZIP_STRICT_FALSE
    ):
        routes_from_newer_version = router.versioned_routers[version.value].routes
        schemas_from_older_version = get_fields_from_routes(
            router.versioned_routers[older_version.value].routes
        )
            ...
            for instruction in [
                ...
                *version_change.alter_schema_instructions,
                ...
                if (
                    isinstance(
                        instruction,
                        (ValidatorDidntExistInstruction, ValidatorExistedInstruction),
                    )
                ...
                if (
                    changelog_entry is not None
                ):  # pragma: no branch # This should never happen
                    version_change_changelog.instructions.append(
                        CadwynVersionChangeInstruction(changelog_entry)
                    )
...
def _get_older_field_name(
    ...
    newer_names_mapping = {
        field.name_from_newer_version: old_name
        for old_name, field in older_model_wrapper.fields.items()
    }
...
def _get_affected_model_names(
    ...
    changed_model = generator_from_newer_version._get_wrapper_for_model(
        instruction.schema
    )
    ...
    models = {
        generator_from_newer_version._get_wrapper_for_model(annotation)
        for annotation in basemodel_annotations
    }
    return [
        model.name
        for model in models
        ...
        or (
            ...
            and changed_model
            in (
                parents := model._get_parents(
                    generator_from_newer_version.model_bundle.schemas
                )
            )
            and all(
                instruction.name not in parent.fields
                for parent in parents[: parents.index(changed_model)]
            )
        )
    ]
...
def _get_openapi_representation_of_a_field(
    ...
    _, definitions = get_definitions(
        fields=[
            ModelField(
                CadwynDummyModelForRepresentation.model_fields["my_field"], "my_field"
            )
        ],
...
class CadwynSchemaFieldWasAddedChangelogEntry(BaseModel):
    type: Literal[
        ChangelogEntryType.schema_field_added
    ] = ChangelogEntryType.schema_field_added
    models: list[str]
    field: str
...
class CadwynSchemaFieldWasRemovedChangelogEntry(BaseModel):
    type: Literal[
        ChangelogEntryType.schema_field_removed
    ] = ChangelogEntryType.schema_field_removed
    models: list[str]
    field: str
...
class CadwynEnumMembersWereAddedChangelogEntry(BaseModel):
    type: Literal[
        ChangelogEntryType.enum_members_added
    ] = ChangelogEntryType.enum_members_added
    enum: str
...
class CadwynEnumMembersWereChangedChangelogEntry(BaseModel):
    type: Literal[
        ChangelogEntryType.enum_members_removed
    ] = ChangelogEntryType.enum_members_removed
    enum: str
...
class CadwynEndpointHadChangelogEntry(BaseModel):
    type: Literal[
        ChangelogEntryType.endpoint_changed
    ] = ChangelogEntryType.endpoint_changed
    path: str
...
class CadwynEndpointWasRemovedChangelogEntry(BaseModel):
    type: Literal[
        ChangelogEntryType.endpoint_removed
    ] = ChangelogEntryType.endpoint_removed
    path: str
    methods: list[HTTPMethod]
    ...
    if isinstance(instruction, EndpointDidntExistInstruction):
            ...
            if attr_value is not Sentinel:
                attribute_changes.append(
                    CadwynEndpointAttributeChange(
                        name=renaming_map.get(attr, attr), new_value=attr_value
                    )
                )
        if (
            instruction.attributes.name is not Sentinel
            and instruction.attributes.summary is Sentinel
        ):
            attribute_changes.append(
                CadwynEndpointAttributeChange(
                    name="summary", new_value=instruction.attributes.name
                )
            )
        ...
        if any(
            ...
            changed_responses = {
                ...
                for method, route_openapi in newer_openapi["paths"][
                    instruction.endpoint_path
                ].items()
            }
            attribute_changes.append(
                CadwynEndpointAttributeChange(
                    name="responses", new_value=changed_responses
                )
            )
        ...
        old_field_name = _get_older_field_name(
            instruction.schema, instruction.name, generator_from_older_version
        )

        if (
            isinstance(instruction, FieldHadInstruction)
            and instruction.new_name is not Sentinel
        ):
            ...
            attribute_changes = []
        newer_model_wrapper = generator_from_newer_version._get_wrapper_for_model(
            instruction.schema
        )
        ...
        newer_field_openapi = _get_openapi_representation_of_a_field(
            newer_model, instruction.name
        )
        ...
        return CadwynFieldAttributesWereChangedChangelogEntry(
            models=_get_affected_model_names(
        ...
        return CadwynEnumMembersWereAddedChangelogEntry(
            enum=enum.name,
            members=[
                CadwynEnumMember(name=name, value=value)
        ...
        return CadwynSchemaWasChangedChangelogEntry(
            model=instruction.name,
            modified_attributes=CadwynModelModifiedAttributes(name=model.name),
        )
        ...
        return CadwynSchemaFieldWasRemovedChangelogEntry(
            models=affected_model_names, field=instruction.name
        )
