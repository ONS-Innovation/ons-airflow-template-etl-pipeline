...
class Cadwyn(FastAPI):
    ...
    def __init__(
        ...
        *,
        ...
        api_version_default_value: Union[
            str, None, Callable[[Request], Awaitable[str]]
        ] = None,
        api_version_title: Optional[str] = None,
        api_version_description: Optional[str] = None,
        versioning_middleware_class: type[
            VersionPickingMiddleware
        ] = VersionPickingMiddleware,
        ...
        **extra: Any,
        ...
        self._latest_version_router = APIRouter(
        ...
        self.api_version_pythonic_parameter_name = api_version_parameter_name.replace(
            "-", "_"
        )
        ...
        if api_version_location == "custom_header":
            self._api_version_manager = HeaderVersionManager(
            ...
            self._api_version_manager = URLVersionManager(
                possible_version_values=self.versions._version_values_set
            )
        ...
        self.router: _RootCadwynAPIRouter = (
            _RootCadwynAPIRouter(  # pyright: ignore[reportIncompatibleVariableOverride]
        ...
        if self.api_version_format == "date" and (
            sorted(self.versions.versions, key=lambda v: v.value, reverse=True)
            != list(self.versions.versions)
        ):
            raise CadwynStructureError(
    ...
    def _add_default_versioned_routers(self) -> None:
        for version in self.versions:
            self.router.versioned_routers[version.value] = APIRouter(
                **self._kwargs_to_router
            )
    ...
    async def openapi_jsons(self, req: Request) -> JSONResponse:
        version = req.query_params.get("version") or req.headers.get(
            self.router.api_version_parameter_name
        )
    ...
    def _there_are_public_unversioned_routes(self):
        return any(
            isinstance(route, Route) and route.include_in_schema
            for route in self.router.unversioned_routes
        )
    ...
    @deprecated(
        "Use generate_and_include_versioned_routers and VersionBundle versions instead"
    )
    def add_header_versioned_routers(
        ...
        *other_routers: APIRouter,
        ...
        return self._add_versioned_routers(
            first_router, *other_routers, version=header_value
        )
