    ...
    def close(self) -> None:
        ...
        for name, oc in reversed(self._on_close):
            if iscoroutinefunction(oc) or isawaitable(oc):
                warnings.warn(
                    f"Skipped async cleanup for {name!r}. " "Use aclose() instead.",
...
def _takes_container(factory: Callable) -> bool:
    ...
    try:
        ...
        sig = inspect.signature(factory, locals={"Container": Container}, eval_str=True)
    except Exception:  # noqa: BLE001
...
class Container:
    ...
    def _lookup(self, svc_type: type) -> tuple[bool, object, str, bool]:
        ...
        if self._lazy_local_registry is not None:
            with suppress(ServiceNotFoundError):
                rs = self._lazy_local_registry.get_registered_service_for(svc_type)
    ...
    @overload
    def get(self, svc_type1: type[T1], svc_type2: type[T2], /) -> tuple[T1, T2]:
        ...
    ...
    def get(self, *svc_types: type) -> object:
        ...
        for svc_type in svc_types:
            ...
            if iscoroutine(svc) or isinstance(svc, AbstractAsyncContextManager):
    ...
    @overload
    async def aget(self, svc_type1: type[T1], svc_type2: type[T2], /) -> tuple[T1, T2]:
        ...
