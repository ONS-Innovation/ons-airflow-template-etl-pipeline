    ...
    *,
    ...
    while True:
        stack = Stack(
            ...
            exc_notes=[safe_str(note) for note in getattr(exc_value, "__notes__", ())],
            is_cause=is_cause,
        )

        if sys.version_info >= (3, 11):
            if isinstance(
                exc_value, (BaseExceptionGroup, ExceptionGroup)
            ):  # noqa: F821
                stack.is_group = True
        ...
        for stack in trace.stacks:
            ...
            for frame_dict in stack_dict["frames"]:
                if frame_dict["locals"] is None or any(
                    frame_dict["filename"].startswith(path) for path in self.suppress
                ):
                    del frame_dict["locals"]
            if stack.is_group:
                stack_dict["exceptions"] = [self._as_dict(t) for t in stack.exceptions]
