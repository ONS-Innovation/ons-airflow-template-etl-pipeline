...
from mypy.nodes import (AssignmentStmt, CallExpr, ClassDef, Decorator,
                        DictionaryComprehension, Expression, ForStmt, FuncDef,
                        GeneratorExpr, IndexExpr, LambdaExpr, MemberExpr,
                        MypyFile, NamedTupleExpr, NameExpr, NewTypeExpr, Node,
                        OpExpr, RefExpr, TupleExpr, TypedDictExpr, TypeInfo,
                        TypeVarExpr, Var, WithStmt)
from mypy.traverser import TraverserVisitor
from mypy.types import (AnyType, Instance, ProperType, Type, TypeOfAny,
                        get_proper_type)
...
def generate_annotated_html(
    ...
    formatted = formatter.style(
        os.path.abspath(html_fnam), "none", underline=True, bold=True
    )
    ...
    def check_iteration(self, expressions: list[Expression], kind: str) -> None:
        for expr in expressions:
            ...
            if isinstance(typ, AnyType):
                self.annotate(
                    expr, f'{kind} uses generic operations (iterable has type "Any").'
                )
    ...
    def visit_call_expr(self, o: CallExpr, /) -> None:
        ...
        if (
            ...
            if (class_ir and not class_ir.is_ext_class) or (
                ...
                self.annotate(
                    o,
                    f'Creating an instance of non-native class "{info.name}" '
                    + "is slow.",
                    2,
                )
    ...
    def check_isinstance_arg(self, arg: Expression) -> None:
        if isinstance(arg, RefExpr):
            if isinstance(arg.node, TypeInfo) and arg.node.is_protocol:
                self.annotate(
                    arg,
                    f'Expensive isinstance() check against protocol "{arg.node.name}".',
                )
...
def colorize_line(linenum: str, s: str, hint_html: str) -> str:
    ...
    line_span = (
        f'<div class="collapsible" style="background-color: #fcc">{linenum}  {s}</div>'
    )
    hint_div = (
        f'<div class="content">{hint_prefix}<div class="hint">{hint_html}</div></div>'
    )
