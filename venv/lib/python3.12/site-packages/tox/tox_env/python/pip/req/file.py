...
class ParsedRequirement:
    def __init__(
        self, req: str, options: dict[str, Any], from_file: str, lineno: int
    ) -> None:
        req = req.encode("utf-8").decode("utf-8")
        try:
            self._requirement: Requirement | Path | str = Requirement(req)
        except InvalidRequirement:
            if is_url(req) or any(
                req.startswith(f"{v}+") and is_url(req[len(v) + 1 :]) for v in VCS
            ):
...
class RequirementsFile:
    ...
    def _parse_requirements(
        self, opt: Namespace, recurse: bool
    ) -> list[ParsedRequirement]:  # noqa: FBT001
        result, found = [], set()
        for parsed_line in self._parse_and_recurse(
            str(self._path), self.is_constraint, recurse
        ):
    ...
    def _key_func(
        self, line: ParsedRequirement
    ) -> tuple[int, tuple[int, str, str]]:  # noqa: PLR6301
        ...
        for line in self._parse_file(filename, constraint):
            if not line.is_requirement and (
                ...
                if _SCHEME_RE.search(filename):  # original file is over http
                    req_path = urllib.parse.urljoin(
                        filename, req_path
                    )  # do a url join so relative paths work
                elif not _SCHEME_RE.search(
                    req_path
                ):  # original file and nested file are paths
                    req_path = str(
                        Path(filename).parent / req_path
                    )  # do a join so relative paths work
                if recurse:
                    yield from self._req_parser._parse_and_recurse(
                        req_path, nested_constraint, recurse
                    )  # noqa: SLF001
                else:
    ...
    def _parse_file(
        self, url: str, constraint: bool
    ) -> Iterator[ParsedLine]:  # noqa: FBT001
    ...
    def _handle_requirement_line(line: ParsedLine) -> ParsedRequirement:
        ...
        return ParsedRequirement(
            line.requirement, req_options, line.filename, line.lineno
        )
    ...
    def _merge_option_line(  # noqa: C901, PLR0912, PLR0915, PLR6301
        ...
        if opt.find_links:
            ...
            if os.path.exists(
                str(relative_to_reqs_file)
            ):  # noqa: PTH110 # Path.exists fails on win32 <=3.7 with URI
    ...
    def _option_to_args(
        self, opt: Namespace
    ) -> list[str]:  # noqa: C901, PLR0912, PLR6301
        result: list[str] = []
