...
class PythonInstallerListDependencies(Installer[Python], ABC):
    def __init__(
        self, tox_env: Python, with_list_deps: bool = True
    ...
    def installed(self) -> list[str]:
        ...
        result = self._env.execute(
            cmd=cmd.args, stdin=StdinSource.OFF, run_id="freeze", show=False
        )
...
class Pip(PythonInstallerListDependencies):
    ...
    def default_install_command(
        self, conf: Config, env_name: str | None
    ) -> Command:  # noqa: ARG002
        isolated_flag = (
            "-E" if self._env.base_python.version_info.major == 2 else "-I"
        )  # noqa: PLR2004
        cmd = Command(
            ["python", isolated_flag, "-m", "pip", "install", "{opts}", "{packages}"]
        )
    ...
    def _install_requirement_file(
        self, arguments: PythonDeps, section: str, of_type: str
    ) -> None:  # noqa: C901
        try:
            ...
            if not eq:  # pragma: no branch
                if old is not None:
                    self._recreate_if_diff(
                        "install flag(s)", new_options, old["options"], lambda i: i
                    )
                    self._recreate_if_diff(
                        "constraint(s)",
                        new_constraints,
                        old["constraints"],
                        operator.itemgetter(slice(3, None)),
                    )
                    missing_requirement = set(old["requirements"]) - set(
                        new_requirements
                    )
    ...
    def _recreate_if_diff(
        ...
        removed = (
            f" removed {', '.join(sorted(fmt(i) for i in removed_opts))}"
            if removed_opts
            else ""
        )
        added_opts = set(new_opts) - set(old_opts)
        added = (
            f" added {', '.join(sorted(fmt(i) for i in added_opts))}"
            if added_opts
            else ""
        )
    ...
    def _install_list_of_deps(  # noqa: C901
        ...
        for arg in arguments:
            if isinstance(arg, Requirement):
                ...
                raise SystemExit(1)
        req_of_type = (
            f"{of_type}_deps" if groups["pkg"] or groups["dev_pkg"] else of_type
        )
    ...
    def _execute_installer(self, deps: Sequence[Any], of_type: str) -> None:
        ...
        outcome = self._env.execute(
            cmd, stdin=StdinSource.OFF, run_id=f"install_{of_type}"
        )
