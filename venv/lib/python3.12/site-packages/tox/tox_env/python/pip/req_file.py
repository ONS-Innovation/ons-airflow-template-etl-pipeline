...
class PythonDeps(RequirementsFile):
    ...
    def _merge_option_line(
        self, base_opt: Namespace, opt: Namespace, filename: str
    ) -> None:
        super()._merge_option_line(base_opt, opt, filename)
        if getattr(
            opt, "no_deps", False
        ):  # if the option comes from a requirements file this flag is missing there
    ...
    def _normalize_raw(raw: str) -> str:
        ...
        lines: list[str] = [
            PythonDeps._normalize_line(line) for line in raw.splitlines()
        ]
        adjusted = "\n".join(lines)
        return (
            f"{adjusted}\n" if raw.endswith("\\\n") else adjusted
    ...
    def unroll(self) -> tuple[list[str], list[str]]:
        if self._unroll is None:
            ...
            result_opts: list[str] = [
                f"{key}={value}" for key, value in opts_dict.items()
            ]
    ...
    def factory(cls, root: Path, raw: object) -> PythonDeps:
        if not (
            ...
            or (
                isinstance(raw, list)
                and (
                    all(isinstance(i, str) for i in raw)
                    or all(isinstance(i, Requirement) for i in raw)
                )
            )
        ):
            raise TypeError(raw)
