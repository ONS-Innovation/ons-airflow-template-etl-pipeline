...
PY_FACTORS_RE_EXPLICIT_VERSION = re.compile(
    r"^((?P<impl>cpython|pypy)-)?(?P<version>[2-9]\.[0-9]+)(?P<threaded>t?)$"
)
...
class Python(ToxEnv, ABC):
    ...
    def register_config(self) -> None:
        ...
        def validate_base_python(value: list[str]) -> list[str]:
            return self._validate_base_python(
                self.name, value, self.core["ignore_base_python_conflict"]
            )
        ...
        self.conf.add_constant(
            "py_dot_ver", "<python major>.<python minor>", value=self.py_dot_ver
        )
        self.conf.add_constant("py_impl", "python implementation", value=self.py_impl)
        self.conf.add_constant(
            "py_free_threaded", "is no-gil interpreted", value=self.py_free_threaded
        )
    ...
    def _default_pass_env(self) -> list[str]:
        ...
        env.extend(
            binary_extension_build
        )  # used by binary extensions during installation
        env.extend(["REQUESTS_CA_BUNDLE"])
        return env

    def default_base_python(
    ...
    def _setup_env(self) -> None:
        ...
        self._paths = (
            self.prepend_env_var_path()
        )  # now that the environment exist we can add them to the path
