...
class PythonRun(Python, RunToxEnv, ABC):
    ...
    def _register_package_conf(self) -> bool:
        ...
        desc = (
            f"package installation mode - {' | '.join(i for i in self._package_types)} "
        )
        ...
        if getattr(self.options, "install_pkg", None) is not None:
            ...
            develop_mode = self.conf["use_develop"] or getattr(
                self.options, "develop", False
            )
            if develop_mode:
