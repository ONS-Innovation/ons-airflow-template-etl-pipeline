...
from tox.tox_env.python.package import (PythonPackageToxEnv, SdistPackage,
                                        WheelPackage)
...
class VenvCmdBuilder(PythonPackageToxEnv, ABC):
    ...
    def extract_install_info(self, for_env: EnvConfigSet, path: Path) -> list[Package]:
        ...
        if path.suffix == ".whl":
            ...
            deps = dependencies_with_extras(
                [Requirement(i) for i in requires], extras, wheel_dist.metadata["Name"]
            )
            ...
            with self._sdist_meta_tox_env.display_context(self._has_display_suspended):
                self._sdist_meta_tox_env.root = next(
                    work_dir.iterdir()
                )  # contains a single egg info folder
    ...
    def child_pkg_envs(
        self, run_conf: EnvConfigSet
    ) -> Iterator[PackageToxEnv]:  # noqa: ARG002
...
class WheelDistribution(Distribution):  # cannot subclass has type Any
    ...
    def read_text(self, filename: str) -> str | None:
        with ZipFile(self._wheel) as zip_file:
            try:
                with TextIOWrapper(
                    zip_file.open(f"{self.dist_name}/{filename}"), encoding="utf-8"
                ) as file_handler:
    ...
    def locate_file(self, path: str | PathLike[str]) -> Path:
        return (
            self._wheel / path
        )  # pragma: no cover # not used by us, but part of the ABC
