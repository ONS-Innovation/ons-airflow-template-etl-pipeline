...
from typing import (TYPE_CHECKING, Any, Dict, Generator, Iterator, Literal,
                    NoReturn, Optional, Sequence, cast)
...
from tox.tox_env.python.package import (EditableLegacyPackage, EditablePackage,
                                        PythonPackageToxEnv, SdistPackage,
                                        WheelPackage)
...
class ToxCmdStatus(CmdStatus):
    ...
    def out_err(self) -> tuple[str, str]:
        status = self._execute_status
        if (
            status is None or status.outcome is None
        ):  # interrupt before status create # pragma: no branch
...
class Pep517VenvPackager(PythonPackageToxEnv, ABC):
    ...
    def _setup_build_requires(self, of_type: str) -> None:
        settings: ConfigSettings = self.conf[
            f"config_settings_get_requires_for_build_{of_type}"
        ]
        requires = getattr(self._frontend, f"get_requires_for_build_{of_type}")(
            config_settings=settings
        ).requires
        self._install(
            requires, PythonPackageToxEnv.__name__, f"requires_for_build_{of_type}"
        )
        ...
        if executor is not None:  # pragma: no branch
            try:
                if executor.is_alive:
                    self._frontend._send(
                        "_exit"
                    )  # try first on amicable shutdown  # noqa: SLF001
            except (
                SystemExit
            ):  # pragma: no cover  # if already has been interrupted ignore
                pass
            finally:
                executor.close()
    ...
    def perform_packaging(self, for_env: EnvConfigSet) -> list[Package]:
        ...
        try:
            ...
            targets = [
                e for e in self.builds.pop("editable") if e["package"] == "editable"
            ]
            ...
            self._run_state[
                "setup"
            ] = False  # force setup again as we need to provision wheel to get dependencies
            deps = self._load_deps(for_env)
        of_type: str = for_env["package"]
        if of_type == "editable-legacy":
            self.setup()
            config_settings: ConfigSettings = self.conf[
                "config_settings_get_requires_for_build_sdist"
            ]
            sdist_requires = self._frontend.get_requires_for_build_sdist(
                config_settings=config_settings
            ).requires
            deps = [*self.requires(), *sdist_requires, *deps]
            package: Package = EditableLegacyPackage(
                self.core["tox_root"], deps
            )  # the folder itself is the package
        elif of_type == "sdist":
            self.setup()
            with self._pkg_lock:
                config_settings = self.conf["config_settings_build_sdist"]
                sdist = self._frontend.build_sdist(
                    sdist_directory=self.pkg_dir, config_settings=config_settings
                ).sdist
            ...
            if w_env is not None and w_env is not self:
                ...
                package = (EditablePackage if of_type == "editable" else WheelPackage)(
                    wheel, deps
                )
    ...
    def _load_deps_from_static(self, for_env: EnvConfigSet) -> list[Requirement] | None:
        ...
        for dynamic in project.get("dynamic", []):
            if dynamic == "dependencies" or (
                extras and dynamic == "optional-dependencies"
            ):
        ...
        for extra, reqs in optional_deps.items():
            deps_with_markers.extend(
                (Requirement(req), {extra}) for req in (reqs or [])
            )
    ...
    def _load_deps_from_built_metadata(
        self, for_env: EnvConfigSet
    ) -> list[Requirement]:
            ...
            if w_env is not None and w_env is not self:
                with w_env.display_context(self._has_display_suspended):
                    if isinstance(w_env, Pep517VirtualEnvPackager):
                        reqs, name = w_env.get_package_dependencies(
                            for_env
                        ), w_env.get_package_name(for_env)
                    else:
                        reqs = []
        if reqs is None:
    ...
    def get_package_dependencies(self, for_env: EnvConfigSet) -> list[Requirement]:
        with self._pkg_lock:
            if self._package_dependencies is None:  # pragma: no branch
                ...
                requires: list[str] = (
                    cast("PathDistribution", self._distribution_meta).requires or []
                )
                self._package_dependencies = [
                    Requirement(i) for i in requires
                ]  # pragma: no branch
        ...
        with self._pkg_lock:
            if self._package_name is None:  # pragma: no branch
                ...
                self._package_name = cast(
                    "PathDistribution", self._distribution_meta
                ).metadata["Name"]
    ...
    def _ensure_meta_present(self, for_env: EnvConfigSet) -> None:
        ...
        target: Literal["editable", "wheel"] = (
            "editable" if for_env["package"] == "editable" else "wheel"
        )
        self.call_require_hooks.add(target)

        self.setup()
        hook = getattr(self._frontend, f"prepare_metadata_for_build_{target}")
        config: ConfigSettings = self.conf[
            f"config_settings_prepare_metadata_for_build_{target}"
        ]
        result: MetadataForBuildWheelResult | MetadataForBuildEditableResult | None = (
            hook(self.meta_folder, config)
        )
        if result is None:
            config = self.conf[f"config_settings_build_{target}"]
            dist_info_path, _, __ = self._frontend.metadata_from_built(
                self.meta_folder, target, config
            )
    ...
    def __init__(self, root: Path, env: Pep517VenvPackager) -> None:
        ...
        for hook in chain(
            (
                ...
                for build_type in ["editable", "wheel", "sdist"]
            ),
            (
                f"prepare_metadata_for_build_{build_type}"
                for build_type in ["editable", "wheel"]
            ),
            (f"build_{build_type}" for build_type in ["editable", "wheel", "sdist"]),
        ):  # wrap build methods in a cache wrapper

            def key(
                *args: Any, bound_return: str = hook, **kwargs: Any
            ) -> str:  # noqa: ARG001
                return bound_return
        ...
        try:
            if self._can_skip_prepare(cmd):
                return (
                    ...
                    "",
                    "",
                )  # will need to build wheel either way, avoid prepare
            return super()._send(cmd, **kwargs)
        except BackendFailed as exception:
            raise exception if isinstance(
                exception, ToxBackendFailed
            ) else ToxBackendFailed(exception) from exception
