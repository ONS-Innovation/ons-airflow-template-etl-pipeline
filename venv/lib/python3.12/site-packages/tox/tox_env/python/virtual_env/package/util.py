...
def dependencies_with_extras(
    deps: list[Requirement], extras: set[str], package_name: str
) -> list[Requirement]:
    return dependencies_with_extras_from_markers(
        extract_extra_markers(deps), extras, package_name
    )
...
def extract_extra_markers(
    deps: list[Requirement],
) -> list[tuple[Requirement, set[str | None]]]:
    """
...
def _extract_extra_markers(req: Requirement) -> tuple[Requirement, set[str | None]]:
    req = deepcopy(req)
    markers: list[str | tuple[Variable, Op, Variable]] = (
        getattr(req.marker, "_markers", []) or []
    )
