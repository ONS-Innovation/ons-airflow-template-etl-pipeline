...
def resolve(root: Path, groups: set[str]) -> set[Requirement]:
    ...
    if not isinstance(dependency_groups_raw, dict):
        ...
        raise Fail(msg)
    original_names_lookup, dependency_groups = _normalize_group_names(
        dependency_groups_raw
    )
    result: set[Requirement] = set()
    for group in groups:
        result = result.union(
            _resolve_dependency_group(dependency_groups, group, original_names_lookup)
        )
    return result
...
def _resolve_dependency_group(
    ...
    if group in past_groups:
        ...
        original_past_groups = tuple(
            original_names_lookup.get(g, g) for g in past_groups
        )
