...
from typing import (TYPE_CHECKING, Any, Iterator, List, NamedTuple, Sequence,
                    Set, cast)
...
SECRET_ENV_VAR_REGEX = re.compile(
    ".*(" + "|".join(SECRET_KEYWORDS) + ").*", re.IGNORECASE
)
...
class ToxEnv(ABC):
    ...
    def __init__(self, create_args: ToxEnvCreateArgs) -> None:
        ...
        """
        self.journal: EnvJournal = (
            create_args.journal
        )  #: handler to the tox reporting system
        self.conf: EnvConfigSet = (
            create_args.conf
        ...
        self.log_handler: ToxHandler = (
            create_args.log_handler
        )  #: handler to the tox reporting system

        #: encode the run state of various methods (setup/clean/etc)
        self._run_state = {"setup": False, "clean": False, "teardown": False}
        self._paths_private: list[
            Path
        ] = []  #: a property holding the PATH environment variables
    ...
    def register_config(self) -> None:
        ...
        self.conf.add_config(
            keys=["env_dir", "envdir"],
            of_type=Path,
            default=lambda conf, name: cast("Path", conf.core["work_dir"])
            / self.name,  # noqa: ARG005
        ...
        )  # noqa: S101 # trigger installer creation to allow config registration

    def _recreate_default(
        self, conf: Config, value: str | None
    ) -> bool:  # noqa: ARG002
    ...
    def _setup_env(self) -> None:
        ...
        with self.cache.compare(conf, ToxEnv.__name__) as (eq, old):
            if (
                eq is False and old is not None
            ):  # pragma: no branch  # recreate if already created and not equals
    ...
    def environment_variables(self) -> dict[str, str]:
        ...
        if (
            self._env_vars_pass_env == pass_env
            and not set_env.changed
            and self._env_vars is not None
        ):
        ...
        self._env_vars, self._env_vars_pass_env, set_env.changed = (
            result,
            pass_env.copy(),
            False,
        )
    ...
    def _load_pass_env(pass_env: list[str]) -> dict[str, str]:
        ...
        result: dict[str, str] = {
            e: v for e, v in os.environ.items() if any(p.match(e) for p in patterns)
        }
        return result
    ...
    def execute_async(  # noqa: PLR0913
        ...
        if (
            show
            and self._hidden_outcomes is not None
            and execute_status.outcome is not None
        ):
    ...
    def _log_execute(self, request: ExecuteRequest, status: ExecuteStatus) -> None:
        ...
        self._log_id += 1
        self._write_execute_log(
            self.name,
            self.env_log_dir / f"{self._log_id}-{request.run_id}.log",
            request,
            status,
        )

    @staticmethod
    def _write_execute_log(
        env_name: str, log_file: Path, request: ExecuteRequest, status: ExecuteStatus
    ) -> None:
    ...
    def display_context(self, suspend: bool) -> Iterator[None]:  # noqa: FBT001
        with self._log_context(), self.log_handler.suspend_out_err(
            suspend, self._suspended_out_err
        ) as out_err:
    ...
    def close_and_read_out_err(self) -> tuple[bytes, bytes] | None:
        ...
        out_b, err_b = (
            cast("BytesIO", out.buffer).getvalue(),
            cast("BytesIO", err.buffer).getvalue(),
        )
        out.close()
        err.close()
        return out_b, err_b
