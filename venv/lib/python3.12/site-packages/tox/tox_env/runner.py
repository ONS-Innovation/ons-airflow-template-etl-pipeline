...
class RunToxEnv(ToxEnv, ABC):
    ...
    def get_package_env_types(self) -> tuple[str, str] | None:
        if self._register_package_conf():
            ...
            self.conf.add_constant(
                ...
                value=self._external_pkg_tox_env_type
                if is_external
                else self._package_tox_env_type,
            )
    ...
    def _handle_journal_package(journal: EnvJournal, packages: list[Package]) -> None:
        ...
        for package in packages:
            if isinstance(package, PathPackage):
                pkg = package.path
                of_type = (
                    "file" if pkg.is_file() else ("dir" if pkg.is_dir() else "N/A")
                )
        ...
        if installed_meta:
            journal["installpkg"] = (
                installed_meta[0] if len(installed_meta) == 1 else installed_meta
            )
    ...
    def environment_variables(self) -> dict[str, str]:
        ...
        if self.package_env is not None and self._packages:
            ...
            environment_variables["TOX_PACKAGE"] = os.pathsep.join(
                str(i) for i in self._packages
            )
