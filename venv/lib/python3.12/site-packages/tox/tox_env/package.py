...
def _lock_method(
    thread_lock: RLock, file_lock: FileLock | None, meth: Callable[..., Any]
) -> Callable[..., Any]:
    def _func(*args: Any, **kwargs: Any) -> Any:
        with thread_lock:
            file_locks = False
            if (
                file_lock is not None and file_lock.is_locked is False
            ):  # file_lock is to lock from other tox processes
                file_lock.acquire()
                file_locks = True
            try:
...
class PackageToxEnv(ToxEnv, ABC):
    ...
    def _recreate_default(self, conf: Config, value: str | None) -> bool:
        return self.options.no_recreate_pkg is False and super()._recreate_default(
            conf, value
        )
    ...
    ) -> Generator[tuple[str, str], PackageToxEnv, None]:  # noqa: ARG002, PLR6301
