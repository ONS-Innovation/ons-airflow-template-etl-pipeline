...
class Plugin:
    ...
    def _register_plugins(self, inline: ModuleType | None) -> None:
        ...
        from tox.session.cmd import exec_  # noqa: PLC0415
        from tox.session.cmd import (depends, devenv, legacy, list_env,
                                     quickstart, schema, show_config,
                                     version_flag)
    ...
    def tox_after_run_commands(
        self, tox_env: ToxEnv, exit_code: int, outcomes: list[Outcome]
    ) -> None:
        self.manager.hook.tox_after_run_commands(
            tox_env=tox_env, exit_code=exit_code, outcomes=outcomes
        )

    def tox_on_install(
        self, tox_env: ToxEnv, arguments: Any, section: str, of_type: str
    ) -> None:
        self.manager.hook.tox_on_install(
            tox_env=tox_env, arguments=arguments, section=section, of_type=of_type
        )
    ...
    def load_plugins(self, path: Path) -> None:
        for (
            ...
            self.manager.get_plugins()
        ):  # make sure we start with a clean state, repeated in memory run
...
def _load_inline(
    path: Path,
) -> ModuleType | None:  # used to be able to unregister plugin tests
