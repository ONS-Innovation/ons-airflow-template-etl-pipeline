...
def _file_support_encoding(chars: Sequence[str], file: IO[str]) -> bool:
    ...
    ):  # pragma: no branch  # this should be always set, unless someone passes in something bad
        try:
            for char in chars:
...
class Spinner:
    ...
    def __init__(
        ...
        self.frames = (
            self.UNICODE_FRAMES
            if _file_support_encoding(self.UNICODE_FRAMES, stream)
            else self.ASCII_FRAMES
        )
        self.outcome = (
            self.UNICODE_OUTCOME
            if _file_support_encoding(self.UNICODE_OUTCOME, stream)
            else self.ASCII_OUTCOME
        )
        self.stream = stream
        self.total = total
    ...
    def frame(self) -> str:
        ...
        text_frame = textwrap.shorten(
            text_frame, width=self.max_width - 1, placeholder="..."
        )
    ...
    def finalize(self, key: str, status: str, color: str) -> None:
        ...
        if self.print_report:
            duration = (
                MISS_DURATION if start_at is None else time.monotonic() - start_at
            )
...
def td_human_readable(seconds: float) -> str:
    ...
    for period_name, period_seconds in _PERIODS:
        ...
        if period_str is not None:
            texts.append(
                f"{period_str} {period_name}{'' if period_str == '1' else 's'}"
            )
    return " ".join(texts)
