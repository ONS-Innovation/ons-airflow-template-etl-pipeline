...
class LocalSubProcessPep517Executor(Execute):
    ...
    def __init__(
        self, colored: bool, cmd: Sequence[str], env: dict[str, str], cwd: Path
    ) -> None:  # noqa: FBT001
        super().__init__(colored)
        self.cmd = cmd
        self.env = env
        self.cwd = cwd
        self._local_execute: tuple[
            LocalSubProcessExecuteInstance, ExecuteStatus
        ] | None = None
    ...
    def build_instance(
        ...
        return LocalSubProcessPep517ExecuteInstance(
            request, options, out, err, self.local_execute(options)
        )

    def local_execute(
        self, options: ExecuteOptions
        ...
        if self._local_execute is None:
            ...
            instance = LocalSubProcessExecuteInstance(
                ...
                options=options,
                out=SyncWrite(
                    name="pep517-out", target=None, color=None
                ),  # not enabled no need to enter/exit
                err=SyncWrite(
                    name="pep517-err", target=None, color=None
                ),  # not enabled no need to enter/exit
                on_exit_drain=False,
            )
    ...
    def close(self) -> None:
        if self._local_execute is not None:  # pragma: no branch
            ...
            if (
                execute.process is not None and execute.process.returncode is None
            ):  # pragma: no cover
                try:  # pragma: no cover
