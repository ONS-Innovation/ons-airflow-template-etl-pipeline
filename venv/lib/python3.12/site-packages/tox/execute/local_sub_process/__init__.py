...
from tox.execute.api import (Execute, ExecuteInstance, ExecuteOptions,
                             ExecuteStatus)
...
if sys.platform == "win32":  # explicit check for mypy # pragma: win32 cover
    ...
    if (
        TYPE_CHECKING
    ):  # the typeshed libraries don't contain this, so replace it with normal one
...
class LocalSubprocessExecuteStatus(ExecuteStatus):
    ...
    def interrupt(self) -> None:
        ...
        if self._process is not None:  # pragma: no branch
            ...
            if self.wait(self.options.suicide_timeout) is None:  # still alive -> INT
                ...
                if (
                    self.wait(self.options.interrupt_timeout) is None
                ):  # still alive -> TERM # pragma: no branch
                    terminate_output = self.options.terminate_timeout
                    logging.warning(
                        msg, f"SIGTERM({SIGTERM})", to_pid, host_pid, terminate_output
                    )
                    self._process.terminate()
                    # Windows terminate is UNIX kill
                    if (
                        sys.platform != "win32" and self.wait(terminate_output) is None
                    ):  # pragma: no branch
                        logging.warning(
                            msg[:-18], f"SIGKILL({SIGKILL})", to_pid, host_pid
                        )
                ...
                logging.warning(
                    "process already dead with %s within %s",
    ...
    def write_stdin(self, content: str) -> None:
        ...
        try:
            if (
                sys.platform == "win32"
            ):  # explicit check for mypy  # pragma: win32 cover
...
class LocalSubProcessExecuteInstance(ExecuteInstance):
            ...
            if executable is None:
                ...
                if self.request.allow is not None:
                    for allow in self.request.allow:
                        ...
                        if fnmatch.fnmatch(
                            self.request.cmd[0], allow
                        ) or fnmatch.fnmatch(executable, allow):
                            break
                    else:
                        msg = (
                            f"{base} (resolves to {executable})"
                            if base == executable
                            else base
                        )
                        msg = (
                            f"{msg} is not allowed, use allowlist_externals to allow it"
                        )
                        raise Fail(msg)
                cmd = [executable]
                if (
                    sys.platform != "win32"
                    and self.request.env.get("TOX_LIMITED_SHEBANG", "").strip()
                ):
    ...
    def __enter__(self) -> ExecuteStatus:
        ...
        stdout, stderr = self.get_stream_file_no("stdout"), self.get_stream_file_no(
            "stderr"
        )
        try:
            self.process = process = Popen(
                ...
                stderr=next(stderr),
                stdin={
                    StdinSource.USER: None,
                    StdinSource.OFF: DEVNULL,
                    StdinSource.API: PIPE,
                }[self.request.stdin],
            ...
            logging.error(
                "Exception running subprocess %s", str(exception)
            )  # noqa: TRY400
            return LocalSubprocessExecuteFailedStatus(
                self.options, self._out, self._err, exception.errno
            )

        status = LocalSubprocessExecuteStatus(
            self.options, self._out, self._err, process
        )
        drain, pid = self._on_exit_drain, self.process.pid
        self._read_stderr = ReadViaThread(
            stderr.send(process), self.err_handler, name=f"err-{pid}", drain=drain
        )
        self._read_stderr.__enter__()
        self._read_stdout = ReadViaThread(
            stdout.send(process), self.out_handler, name=f"out-{pid}", drain=drain
        )
            ...
            for stream in (
                ...
                if stream is not None and not getattr(stream, "closed", False):
                    try:
                        ...
                        logging.warning(
                            "error while trying to close %r with %r", stream, exc
                        )  # pragma: no cover
    ...
    def set_out_err(
        self, out: SyncWrite, err: SyncWrite
