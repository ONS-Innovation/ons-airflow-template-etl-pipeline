...
class CliEnv:  # noqa: PLW1641
    ...
    def __str__(self) -> str:
        return (
            "ALL"
            if self.is_all
            else ("<env_list>" if self.is_default_list else ",".join(self))
        )
...
def register_env_select_flags(
    ...
    if not group_only:
        ...
        add_to.add_argument(
            "-e", dest="env", help=help_msg, default=default, type=CliEnv
        )
    if multiple:
    ...
    add_to.add_argument(
        "--skip-env", dest="skip_env", metavar="re", help=help_msg, default="", type=str
    )
    return add_to
...
class _ToxEnvInfo:
    ...
    package_skip: tuple[
        str, Skip
    ] | None = None  #: if set the creation of the packaging environment failed
...
class EnvSelector:
    def __init__(self, state: State) -> None:
        ...
        self._warned_about: set[
            str
        ] = set()  #: shared set of skipped environments that were already warned about
        self._state = state
        ...
        self._state.conf.core.add_config(
            "labels", Dict[str, EnvList], {}, "core labels"
        )
        tox_env_filter_regex = getattr(state.conf.options, "skip_env", "").strip()
        self._filter_re = (
            re.compile(tox_env_filter_regex) if tox_env_filter_regex else None
        )
    ...
    def _collect_names(self) -> Iterator[tuple[Iterable[str], bool]]:
        ...
        label_envs = dict.fromkeys(
            chain.from_iterable(self._state.conf.core["labels"].values())
        )
        if label_envs:
    ...
    def _ensure_envs_valid(self) -> None:
        valid_factors = set(
            chain.from_iterable(env.split("-") for env in self._state.conf)
        )
    ...
    def _env_name_to_active(self) -> dict[str, bool]:
        ...
        if (
            not (
                getattr(self._state.conf.options, "labels", [])
                or getattr(self._state.conf.options, "factors", [])
            )
    ...
    def _defined_envs(self) -> dict[str, _ToxEnvInfo]:  # noqa: C901, PLR0912
        ...
        if self._defined_envs_ is None:  # noqa: PLR1702
            ...
            for name, is_active in env_name_to_active.items():
                if (
                    name in self._pkg_env_counter
                ):  # already marked as packaging, nothing to do here
                    continue
                ...
                if pkg_name_type is not None:
                    ...
                    try:
                        run_env.package_env = self._build_pkg_env(
                            pkg_name_type, name, env_name_to_active
                        )
                        ...
                        try:
                            for env in run_env.package_envs:
                                ...
                                if other_env_info is not None and isinstance(
                                    other_env_info.env, RunToxEnv
                                ):
                                    del self._defined_envs_[
                                        env.name
                                    ]  # pragma: no cover
                                    for (
                                        _pkg_env
                                    ) in (
                                        other_env_info.env.package_envs
                                    ):  # pragma: no cover
                                        self._pkg_env_counter[
                                            _pkg_env.name
                                        ] -= 1  # pragma: no cover
                        except Exception:  # noqa: BLE001
                            assert (
                                self._defined_envs_[name].package_skip is not None
                            )  # noqa: S101
            ...
            order = chain(
                env_name_to_active,
                (i for i in self._defined_envs_ if i not in env_name_to_active),
            )
            self._defined_envs_ = {
                name: self._defined_envs_[name]
                for name in order
    ...
    def _build_run_env(self, name: str) -> RunToxEnv | None:
        if (
            self._provision is not None
            and self._provision[0] is False
            and name == self._provision[1]
        ):
            # ignore provision env unless this is a provision run
            return None
        if (
            self._provision is not None
            and self._provision[0]
            and name != self._provision[1]
        ):
    ...
    def _build_pkg_env(
        self, name_type: tuple[str, str], run_env_name: str, active: dict[str, bool]
    ) -> PackageToxEnv:
        ...
        with self._log_handler.with_context(name):
            ...
            try:
                package_tox_env = self._get_package_env(
                    core_type, name, active.get(name, missing_active)
                )
                ...
                try:
                    ...
                    while True:
                        child_pkg_env = self._build_pkg_env(
                            name_type, run_env_name, active
                        )
                ...
                assert self._defined_envs_ is not None  # noqa: S101
                self._defined_envs_[run_env_name].package_skip = (
                    name_type[0],
                    exception,
                )
            return package_tox_env

    def _get_package_env(
        self, packager: str, name: str, is_active: bool
    ) -> PackageToxEnv:  # noqa: FBT001
        assert self._defined_envs_ is not None  # noqa: S101
        if name in self._defined_envs_:
            ...
            if isinstance(env, PackageToxEnv):
                if (
                    env.id() != packager
                ):  # pragma: no branch # same env name is used by different packaging
    ...
    def _parse_factors(self) -> tuple[set[str], ...]:
        ...
        return tuple(
            {f for factor in factor_list for f in factor.split("-")}
            for factor_list in raw_factors
        )
        ...
        for name, env_info in self._defined_envs.items():
            ...
            if self._filter_re is not None and self._filter_re.match(name):
                if name not in self._warned_about:
                    ...
                    LOGGER.warning(
                        "skip environment %s, matches filter %r",
                        name,
    ...
    def ensure_only_run_env_is_active(self) -> None:
        ...
        invalid = [
            n for n, a in active.items() if a and isinstance(envs[n].env, PackageToxEnv)
        ]
        if invalid:
