...
class SkipMissingInterpreterAction(Action):
    def __call__(
        ...
        if value not in {"config", "true", "false"}:
            raise ArgumentError(
                self, f"value must be 'config', 'true', or 'false' (got {value!r})"
            )
...
def report(
    start: float, runs: list[ToxEnvRunResult], is_colored: bool, verbosity: int
) -> int:  # noqa: FBT001
    def _print(color_: int, message: str) -> None:
        if verbosity:
            print(
                f"{color_ if is_colored else ''}{message}{Fore.RESET if is_colored else ''}"
            )  # noqa: T201
    ...
    for run in runs:
        ...
        duration_individual = (
            [o.elapsed for o in run.outcomes] if verbosity >= 2 else []
        )  # noqa: PLR2004
        extra = (
            f"+cmd[{','.join(f'{i:.2f}' for i in duration_individual)}]"
            if duration_individual
            else ""
        )
...
def execute(
    state: State, max_workers: int | None, has_spinner: bool, live: bool
) -> int:  # noqa: FBT001
    ...
    try:
        ...
        try:
            ...
            spinner.print_report = (
                False  # no need to print reports at this point, final report coming up
            )
            logger.error(
                "[%s] KeyboardInterrupt - teardown started", os.getpid()
            )  # noqa: TRY400
            interrupt.set()
        ...
        ordered_results: list[ToxEnvRunResult] = [
            name_to_run[env] for env in to_run_list
        ]
        # write the journal
        write_journal(
            getattr(state.conf.options, "result_json", None), state._journal
        )  # noqa: SLF001
        # report the outcome
        exit_code = report(
...
class ToxSpinner(Spinner):
    ...
    def update_spinner(
        self, result: ToxEnvRunResult, success: bool
    ) -> None:  # noqa: FBT001
        ...
        with spinner:
            ...
            try:
                executor = ThreadPoolExecutor(
                    max_workers=max_workers, thread_name_prefix="tox-driver"
                )
                env_list: list[str] = []
                while True:
...
def _handle_one_run_done(
    ...
    if (
        live is False and state.conf.options.parallel_live is False
    ):  # teardown background run
        out_err = (
            tox_env.close_and_read_out_err()
        )  # sync writes from buffer to stdout/stderr
        pkg_out_err_list = []
        ...
        if not success or tox_env.conf["parallel_show_output"]:
            for pkg_out_err in pkg_out_err_list:
                state._options.log_handler.write_out_err(
                    pkg_out_err
                )  # pragma: no cover  # noqa: SLF001
...
def ready_to_run_envs(
    state: State, to_run: list[str], completed: set[str]
) -> Iterator[list[str]]:
