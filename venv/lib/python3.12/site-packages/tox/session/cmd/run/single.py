...
def run_one(
    tox_env: RunToxEnv, no_test: bool, suspend_display: bool
) -> ToxEnvRunResult:  # noqa: FBT001
    ...
    return ToxEnvRunResult(
        name, skipped, code, outcomes, duration, tox_env.conf["ignore_outcome"]
    )


def _evaluate(
    tox_env: RunToxEnv, no_test: bool
) -> tuple[bool, int, list[Outcome]]:  # noqa: FBT001
    skipped = False
    code: int = 0
    outcomes: list[Outcome] = []
    try:
        ...
        SystemExit
    ) as exception:  # setup command fails (interrupted or via invocation)
...
def run_commands(
    tox_env: RunToxEnv, no_test: bool
) -> tuple[int, list[Outcome]]:  # noqa: FBT001
    outcomes: list[Outcome] = []
    if no_test:
        exit_code = Outcome.OK
    else:
        from tox.plugin.manager import \
            MANAGER  # importing this here to avoid circular import  # noqa: PLC0415
        ...
        try:
            try:
                status_pre = run_command_set(
                    tox_env, "commands_pre", chdir, ignore_errors, outcomes
                )
                if status_pre == Outcome.OK or ignore_errors:
                    status_main = run_command_set(
                        tox_env, "commands", chdir, ignore_errors, outcomes
                    )
                else:
                    status_main = Outcome.OK
            finally:
                status_post = run_command_set(
                    tox_env, "commands_post", chdir, ignore_errors, outcomes
                )
        finally:
    ...
    for at, cmd in enumerate(command_set):
        ...
        try:
            ...
            if cmd.ignore_exit_code:
                logging.warning(
                    "command failed but is marked ignore outcome so handling it as success"
                )
                continue
            if ignore_errors:
                if exit_code == Outcome.OK:
                    exit_code = cast(
                        "int", exception.code
                    )  # ignore errors continues ahead but saves the exit code
                continue
