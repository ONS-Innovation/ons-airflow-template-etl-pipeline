...
def tox_add_option(parser: ToxParser) -> None:
    our = parser.add_command(
        "schema", [], "Generate schema for tox configuration", gen_schema
    )
    our.add_argument(
        "--strict",
        action="store_true",
        help="Disallow extra properties in configuration",
    )
...
def _process_type(of_type: typing.Any) -> dict[str, typing.Any]:  # noqa: C901, PLR0911
    ...
    if typing.get_origin(of_type) in {list, set}:
        ...
        if typing.get_args(of_type)[0] is tox.config.types.Command:
            return {
                "type": "array",
                "items": _process_type(typing.get_args(of_type)[0]),
            }
...
def _get_schema(conf: ConfigSet, path: str) -> dict[str, dict[str, typing.Any]]:
    ...
    for x in conf.get_configs():
        ...
        try:
            ...
            print(
                name, "has unrecoginsed type:", of_type, file=sys.stderr
            )  # noqa: T201
...
def gen_schema(state: State) -> int:
    ...
    env_properties = _get_schema(
        state.envs["py"].conf, path="#/properties/env_run_base/properties"
    )
