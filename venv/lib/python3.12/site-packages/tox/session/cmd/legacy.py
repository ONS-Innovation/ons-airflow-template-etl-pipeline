...
from tox.session.cmd.run.parallel import (OFF_VALUE, parallel_flags,
                                          run_parallel)
...
def legacy(state: State) -> int:
    ...
    if option.devenv_path:
        ...
        return devenv(state)
    if (
        option.parallel_no_spinner is True or option.parallel != 0
    ):  # only 0 means sequential
