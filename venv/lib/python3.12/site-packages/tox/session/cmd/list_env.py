...
def tox_add_option(parser: ToxParser) -> None:
    ...
    our.add_argument(
        "--no-desc",
        action="store_true",
        help="do not show description",
        dest="list_no_description",
    )
    d = register_env_select_flags(our, default=None, group_only=True)
    d.add_argument(
        "-d",
        action="store_true",
        help="list just default envs",
        dest="list_default_only",
    )
...
def list_env(state: State) -> int:
    ...
    inactive = (
        ...
        if active_only
            ...
            env: None for env in state.envs.iter(only_active=False) if env not in active
        }
    )
