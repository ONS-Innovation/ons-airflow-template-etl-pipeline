...
class _LogThreadLocal(local):
    ...
    def __init__(self, out_err: OutErr) -> None:
        self.name = self._ident_to_data.get(
            getattr(current_thread(), "parent_ident", None), "ROOT"
        )
    ...
    def name(self, value: str) -> None:
        ...
        for ident in self._ident_to_data.keys() - {
            t.ident for t in enumerate_threads()
        }:
    ...
    @contextmanager
    def suspend_out_err(
        self, yes: bool, out_err: OutErr | None = None
    ) -> Iterator[OutErr]:  # noqa: FBT001
    ...
    def _make(prefix: str, based_of: TextIOWrapper) -> TextIOWrapper:
        return TextIOWrapper(
            NamedBytesIO(f"{prefix}-{based_of.name}"),
...
class ToxHandler(logging.StreamHandler):  # type: ignore[type-arg] # is generic but at runtime doesn't take a type arg
    ...
    def __init__(
        self, level: int, is_colored: bool, out_err: OutErr
    ) -> None:  # noqa: FBT001
    ...
    def _setup_level(self, is_colored: bool, level: int) -> None:  # noqa: FBT001
        ...
        self._warning_formatter = self._get_formatter(
            logging.WARNING, level, is_colored
        )
    ...
    @staticmethod
    def _get_formatter(
        level: int, enabled_level: int, is_colored: bool
    ) -> logging.Formatter:  # noqa: FBT001
        color: int | str = ""
        if is_colored:
        ...
        fmt = (
            f"{_c(Style.BRIGHT)}{_c(Fore.MAGENTA)}%(env_name)s:{_c(Style.RESET_ALL)}"
            + fmt
        )
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        len_sys_path_match = max(
            (len(p) for p in sys.path if basename.startswith(p)), default=-1
        )
...
def setup_report(verbosity: int, is_colored: bool) -> ToxHandler:  # noqa: FBT001
    ...
    logging.debug(
        "setup logging to %s on pid %s", logging.getLevelName(level), os.getpid()
    )
    return handler
