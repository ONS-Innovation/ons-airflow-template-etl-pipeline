...
def provision(state: State) -> int | bool:
    ...
    loader = MemoryLoader(  # these configuration values are loaded from in-memory always (no file conf)
        ...
        pass_env=[
            "*"
        ],  # do not filter environment variables, will be handled by provisioned tox
        recreate=state.conf.options.recreate
        and not state.conf.options.no_recreate_provision,
    )
    ...
    if no_provision:
        msg = (
            f"provisioning explicitly disabled within {sys.executable}, but {miss_msg}"
        )
        if isinstance(no_provision, str):
            msg += f" and wrote to {no_provision}"
            min_version = str(
                next(i.specifier for i in requires if i.name == "tox")
            ).split("=")
            requires_dict = {
            ...
            Path(no_provision).write_text(
                json.dumps(requires_dict, indent=4),
                encoding=locale.getpreferredencoding(do_setlocale=False),
            )
        raise HandledError(msg)

    logging.warning(
        "will run in automatically provisioned tox, host %s %s",
        sys.executable,
        miss_msg,
    )
...
def run_provision(name: str, state: State) -> int:
    ...
    logging.info(
        "will run in a automatically provisioned python environment under %s",
        env_python,
    )
    try:
        tox_env.setup()
