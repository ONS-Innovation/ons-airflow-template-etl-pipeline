...
class Config:
    ...
    def __init__(
        ...
        self.memory_seed_loaders: defaultdict[str, list[MemoryLoader]] = defaultdict(
            list
        )
    ...
    @classmethod
    def make(
        cls, parsed: Parsed, pos_args: Sequence[str] | None, source: Source
    ) -> Config:
        ...
        work_dir: Path = (
            source.path.parent if parsed.work_dir is None else parsed.work_dir
        )
    ...
    def core(self) -> CoreConfigSet:
        ...
        core.loaders.extend(
            self._src.get_loaders(
                core_section, base=[], override_map=self._overrides, conf=core
            )
        )
        self._core_set = core
        return core
    ...
    def get_section_config(
        ...
        try:
            ...
            for loader in self._src.get_loaders(
                section, base, self._overrides, conf_set
            ):
    ...
    def clear_env(self, name: str) -> None:
        ...
        self._key_to_conf_set = {
            k: v
            for k, v in self._key_to_conf_set.items()
            if k[0] == section.key and k[1] == name
        }
