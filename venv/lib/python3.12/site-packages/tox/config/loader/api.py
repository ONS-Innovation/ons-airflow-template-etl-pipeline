...
class ConfigLoadArgs:
    ...
    def __init__(
        self, chain: list[str] | None, name: str | None, env_name: str | None
    ) -> None:
        """
    ...
    def load(
        ...
        for override in overrides:
            ...
            if override.append and converted is not None:
                ...
                elif isinstance(converted, dict) and isinstance(
                    converted_override, dict
                ):
                    converted.update(converted_override)
                elif isinstance(converted, SetEnv) and isinstance(
                    converted_override, SetEnv
                ):
                    converted.update(converted_override, override=True)
                elif isinstance(converted, PythonDeps) and isinstance(
                    converted_override, PythonDeps
                ):
                    converted += converted_override
                else:
