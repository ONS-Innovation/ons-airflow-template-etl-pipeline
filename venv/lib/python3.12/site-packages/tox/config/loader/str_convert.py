...
class StrConvert(Convert[str]):
    ...
    @staticmethod
    def to_dict(
        value: str, of_type: tuple[type[Any], type[Any]]
    ) -> Iterator[tuple[str, str]]:  # noqa: ARG004
    ...
    @staticmethod
    def _win32_process_path_backslash(
        value: str, escape: str, special_chars: str
    ) -> str:
        """
        ...
        for ix, char in enumerate(value):
            ...
            if char == escape:
                ...
                if next_char not in {escape, *special_chars}:
                    result.append(
                        escape
                    )  # escape escapes that are not themselves escaping a special character
        return "".join(result)
    ...
    def to_env_list(value: str) -> EnvList:
        from tox.config.loader.ini.factor import \
            extend_factors  # noqa: PLC0415

        value = expand_ranges(value)
        elements = list(
            chain.from_iterable(extend_factors(expr) for expr in value.split("\n"))
        )
