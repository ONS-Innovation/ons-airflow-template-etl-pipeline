...
class ReplaceReferenceIni(ReplaceReference):
    ...
    def __call__(
        self, value: str, conf_args: ConfigLoadArgs
    ) -> str | None:  # noqa: C901
        ...
        if match:
            ...
            try:
                for src in self._config_value_sources(
                    settings["env"], settings["section"], conf_args.env_name
                ):
                    try:
                        if isinstance(src, SectionProxy):
                            return self.loader.process_raw(
                                self.conf, conf_args.env_name, src[key]
                            )
                        ...
                        return as_str.replace(
                            "#", r"\#"
                        )  # escape comment characters as these will be stripped
