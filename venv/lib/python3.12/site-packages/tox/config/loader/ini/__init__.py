...
class IniLoader(StrConvert, Loader[str]):
    ...
    def process_raw(conf: Config | None, env_name: str | None, value: str) -> str:
        ...
        if (
            conf is None
        ):  # noqa: SIM108 # conf is None when we're loading the global tox configuration file for the CLI
            factor_filtered = strip_comments  # we don't support factor and replace functionality there
        else:
            factor_filtered = filter_for_env(
                strip_comments, env_name
            )  # select matching factors
            ...
            if conf is None:
                ...
                try:
                    ...
                    if isinstance(exception, HandledError):
                        raise
                    name = (
                        self.core_section.key
                        if args_.env_name is None
                        else args_.env_name
                    )
