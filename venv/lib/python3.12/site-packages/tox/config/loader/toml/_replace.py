...
from tox.config.loader.replacer import (MatchRecursionError, ReplaceReference,
                                        load_posargs, replace, replace_env)
...
class Unroll:
    def __init__(
        self, conf: Config | None, loader: TomlLoader, args: ConfigLoadArgs
    ) -> None:
        self.conf = conf
        self.loader = loader
        self.args = args
    ...
    def __call__(
        self, value: TomlTypes, depth: int = 0
    ) -> TomlTypes:  # noqa: C901, PLR0912
        ...
        if isinstance(value, str):
            ...
            ):  # core config does not support string substitution
            ...
            if replace_type := value.get("replace"):
                if replace_type == "posargs" and self.conf is not None:
                    ...
                    return (
                        [
                            self(v, depth)
                            for v in cast("List[str]", value.get("default", []))
                        ]
    ...
    def _replace_ref(self, value: dict[str, TomlTypes], depth: int) -> TomlTypes:
        if (
            self.conf is not None
            and (env := value.get("env"))
            and (key := value.get("key"))
        ):
            return cast(
                "TomlTypes", self.conf.get_env(cast("str", env))[cast("str", key)]
            )
        if of := value.get("of"):
            validated_of = cast("List[str]", validate(of, List[str]))
            loaded = self.loader.load_raw_from_root(
                self.loader.section.SEP.join(validated_of)
            )
...
class TomlReplaceLoader(ReplaceReference):
    ...
    def __call__(self, value: str, conf_args: ConfigLoadArgs) -> str | None:
        if match := _REFERENCE_PATTERN.search(value):
            ...
            try:
                for src in self._config_value_sources(
                    settings["section"], conf_args.env_name
                ):
                    try:
                        value = src.load(settings["key"], conf_args.chain)
                    except (
                        KeyError
                    ) as exc:  # if fails, keep trying maybe another source can satisfy # noqa: PERF203
                        exception = exc
                    else:
            ...
            if exception is not None:
                if isinstance(
                    exception, KeyError
                ):  # if the lookup failed replace - else keep
    ...
    def _config_value_sources(
        self, sec: str | None, current_env: str | None
    ) -> Iterator[ConfigSet | RawLoader]:
        if sec is None:
...
class RawLoader:
    ...
    def load(self, item: str, chain: list[str] | None = None) -> Any:  # noqa: ARG002
        return self._loader.load_raw_from_root(
            f"{self._section}{self._loader.section.SEP}{item}"
        )
