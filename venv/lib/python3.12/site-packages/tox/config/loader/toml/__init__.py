...
class TomlLoader(Loader[TomlTypes]):
    ...
    def load_raw(
        self, key: str, conf: Config | None, env_name: str | None
    ) -> TomlTypes:  # noqa: ARG002
    ...
    @staticmethod
    def to_dict(
        value: TomlTypes, of_type: tuple[type[_T], type[_V]]
    ) -> Iterator[tuple[_T, _V]]:
    ...
    def to_command(value: TomlTypes) -> Command | None:
        if value:
            return Command(
                args=cast("List[str]", value)
            )  # validated during load in _ensure_type_correct
        return None
