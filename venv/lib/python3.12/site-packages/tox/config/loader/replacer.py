...
BACKSLASH_ESCAPE_CHARS: Final[tuple[str, ...]] = (
    ARG_DELIMITER,
    REPLACE_START,
    REPLACE_END,
    "[",
...
def replace(
    ...
    return Replacer(conf, reference, conf_args=args, depth=depth).join(
        find_replace_expr(value)
    )
...
def _flatten_string_fragments(
    seq_of_str_or_other: Sequence[str | Any],
) -> Sequence[str | Any]:
...
class Replacer:
    ...
    def __call__(self, value: MatchArg) -> Sequence[str]:
        return [
            self._replace_match(me) if isinstance(me, MatchExpression) else str(me)
            for me in value
        ]
    ...
    def _replace_match(self, value: MatchExpression) -> str:
        ...
        if replace_value is not None:
            needs_expansion = any(
                isinstance(m, MatchExpression) for m in find_replace_expr(replace_value)
            )
            if needs_expansion:
                try:
                    return replace(
...
def replace_env(conf: Config | None, args: list[str], conf_args: ConfigLoadArgs) -> str:
    ...
    if (
        conf is not None and conf_args.env_name is not None
    ):  # on core no set env support # pragma: no branch
        if new_key not in conf_args.chain:  # check if set env
            ...
            conf_args.chain[-1] != new_key
        ):  # if there's a chain but only self-refers than use os.environ
            circular = ", ".join(
                i[4:] for i in conf_args.chain[conf_args.chain.index(new_key) :]
            )
