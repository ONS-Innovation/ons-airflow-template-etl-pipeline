...
Factory = Optional[
    Callable[[object], T]
]  # note the argument is anything, due e.g. memory loader can inject anything
...
class Convert(ABC, Generic[T]):
    ...
    def to(
        self, raw: T, of_type: type[V], factory: Factory[V]
    ) -> V:  # noqa: PLR0911, C901
        """
    ...
    def _to_typing(
        ...
        if origin in {list, List}:
            ...
            result = [
                self.to(i, entry_type, factory) for i in self.to_list(raw, entry_type)
            ]
            ...
            entry_type = of_type.__args__[0]  # type: ignore[attr-defined]
            result = {
                self.to(i, entry_type, factory) for i in self.to_set(raw, entry_type)
            }
    ...
    @abstractmethod
    def to_dict(
        value: T, of_type: tuple[type[Any], type[Any]]
    ) -> Iterator[tuple[T, T]]:
        """
