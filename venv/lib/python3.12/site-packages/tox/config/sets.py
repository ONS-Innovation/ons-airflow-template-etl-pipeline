...
from typing import (TYPE_CHECKING, Any, Callable, Generator, Iterator, Mapping,
                    Sequence, TypeVar, cast)

from .of_type import (ConfigConstantDefinition, ConfigDefinition,
                      ConfigDynamicDefinition, ConfigLoadArgs)
...
class ConfigSet(ABC):
    ...
    def add_config(  # noqa: PLR0913
        ...
        definition = ConfigDynamicDefinition(
            keys_, desc, of_type, default, post_process, factory
        )
        result = self._add_conf(keys_, definition)
        return cast("ConfigDynamicDefinition[V]", result)

    def add_constant(
        self, keys: str | Sequence[str], desc: str, value: V
    ) -> ConfigConstantDefinition[V]:
        """
    ...
    def _add_conf(
        self, keys: Sequence[str], definition: ConfigDefinition[V]
    ) -> ConfigDefinition[V]:
        key = keys[0]
    ...
    def load(self, item: str, chain: list[str] | None = None) -> Any:
        ...
        return config_definition.__call__(
            self._conf, self.loaders, ConfigLoadArgs(chain, self.name, self.env_name)
        )  # noqa: PLC2801
...
class CoreConfigSet(ConfigSet):
    ...
    def __init__(
        self, conf: Config, section: Section, root: Path, src_path: Path
    ) -> None:
        self._root = root
    ...
    def register_config(self) -> None:
        ...
        self.add_constant(
            "host_python", "the host python executable path", sys.executable
        )
