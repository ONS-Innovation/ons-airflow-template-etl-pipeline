...
class IniSource(Source):
    ...
    def get_loader(
        self, section: Section, override_map: OverrideMap
    ) -> IniLoader | None:
    ...
    def get_base_sections(
        self, base: list[str], in_section: Section
    ) -> Iterator[Section]:  # noqa: PLR6301
        for a_base in base:
            section = IniSection.from_key(a_base)
            yield section  # the base specifier is explicit
            if (
                in_section.prefix is not None
            ):  # no prefix specified, so this could imply our own prefix
                yield IniSection(in_section.prefix, a_base)

    def get_tox_env_section(
        self, item: str
    ) -> tuple[Section, list[str], list[str]]:  # noqa: PLR6301
    ...
    def _discover_from_section(
        self, section: IniSection, known_factors: set[str]
    ) -> Iterator[str]:
