...
class SetEnv:
    def __init__(  # noqa: C901, PLR0912
        self,
        raw: str | dict[str, str] | list[dict[str, str]],
        name: str,
        env_name: str | None,
        root: Path,
    ) -> None:
        ...
        from .loader.replacer import (MatchExpression,  # noqa: PLC0415
                                      find_replace_expr)
    ...
    def _stream_env_file(
        self, filename: str, args: ConfigLoadArgs
        ...
        env_file = Path(
            self._replacer(filename, args.copy())
        )  # apply any replace options
    ...
    def load(self, item: str, args: ConfigLoadArgs | None = None) -> str:
        ...
        self._raw.pop(
            item, None
        )  # if the replace requires the env we may be called again, so allow pop to fail
        return result
        ...
        while self._needs_replacement:
            ...
            for sub_line in filter(None, expanded_line.splitlines()):
                if not self._is_file_line(sub_line):
                    ...
                    for key, value in self._stream_env_file(
                        self._parse_file_line(sub_line), args
                    ):
                        if key not in self._raw:
                            sub_raw[key] = value  # noqa: PERF403
            self._raw.update(sub_raw)
            self.changed = (
                True  # loading while iterating can cause these values to be missed
            )
            yield from sub_raw.keys()

    def update(
        self, param: Mapping[str, str] | SetEnv, *, override: bool = True
    ) -> None:
        for key in param:
