...
import random
import sys
from argparse import (SUPPRESS, Action, ArgumentDefaultsHelpFormatter,
...
class ArgumentParserWithEnvAndConfig(ArgumentParser):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.file_config = (
            kwargs.pop("file_config") if "file_config" in kwargs else IniConfig()
        )
    ...
    def get_type(action: Action) -> type[Any]:
        ...
        if of_type is None:
            if isinstance(action, argparse._AppendAction):  # noqa: SLF001
                ...
                isinstance(action, argparse._StoreAction) and action.choices
            ):  # noqa: SLF001
                loc = locals()
                loc["Literal"] = Literal
...
class ToxParser(ArgumentParserWithEnvAndConfig):
    ...
    def __init__(
        ...
        self._groups: list[
            tuple[Any, dict[str, Any], list[tuple[dict[str, Any], list[ArgumentArgs]]]]
        ] = []
        super().__init__(*args, **kwargs)
        if root is True:
        ...
        if add_cmd is True:
            ...
            self._cmd: Any | None = self.add_subparsers(
    ...
    def _add_provision_arguments(self, sub_parser: ToxParser) -> None:  # noqa: PLR6301
        ...
        if os.environ.get("PYTHONHASHSEED", "random") != "random":
            ...
            hashseed_default = random.randint(
                1, 1024 if sys.platform == "win32" else 4294967295
            )  # noqa: S311
        sub_parser.add_argument(
            "--hashseed",
            metavar="SEED",
    ...
    def add_argument_group(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if self.of_cmd is None and args not in {
            ...
            def add_mutually_exclusive_group(**e_kwargs: Any) -> Any:
                def add_argument(
                    *a_args: str, of_type: type[Any] | None = None, **a_kwargs: Any
                ) -> Action:
    ...
    def add_argument(
        self, *args: str, of_type: type[Any] | None = None, **kwargs: Any
    ) -> Action:
    ...
    def parse_known_args(  # type: ignore[override]
        ...
        elif tuple(args) not in {("--help",), ("-h",)} and (
...
def add_verbosity_flags(parser: ArgumentParser) -> None:
    ...
    level_map = "|".join(
        f"{c}={logging.getLevelName(level)}" for c, level in sorted(LEVELS.items())
    )
