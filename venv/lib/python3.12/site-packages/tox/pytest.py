...
if (
    fs_supports_symlink()
):  # pragma: no cover # used to speed up test suite run time where possible
...
def _disable_root_tox_py(request: SubRequest, mocker: MockerFixture) -> Iterator[None]:
    ...
    if request.node.get_closest_marker("plugin_test"):  # unregister inline plugin
        ...
        def _load_inline(
            path: Path,
        ) -> ModuleType | None:  # register only on first run, and unregister at end
            nonlocal module
...
def check_os_environ() -> Iterator[None]:
    ...
    to_clean = {
        ...
        for k in (ENV_VAR_KEY, "TOX_WORK_DIR", "PYTHONPATH", "COV_CORE_CONTEXT")
    }
...
@pytest.fixture(autouse=True)
def no_color(
    monkeypatch: pytest.MonkeyPatch, check_os_environ_stable: None
) -> None:  # noqa: ARG001
...
class ToxProject:
    ...
    def patch_execute(
        self, handle: Callable[[ExecuteRequest], int | None] | None = None
    ) -> MagicMock:  # noqa: C901
        ...
        class MockExecuteInstance(ExecuteInstance):
            ...
            def __enter__(self) -> ExecuteStatus:
                return MockExecuteStatus(
                    self.options, self._out, self._err, self.exit_code
                )
        ...
        def _execute_call(
            ...
            if exit_code is not None:
                executor = MockExecute(
                    colored=executor._colored, exit_code=exit_code
                )  # noqa: SLF001
            with original_execute_call(
                self, executor, out_err, request, show
            ) as status:
                yield status

        original_execute_call = ToxEnv._execute_call  # noqa: SLF001
        return self.mocker.patch.object(
            ToxEnv, "_execute_call", side_effect=_execute_call, autospec=True
        )
    ...
    def run(
        ...
        *args: str,
        ...
        raise_on_config_fail: bool = True,
    ) -> ToxRunOutcome:
...
def enable_pep517_backend_coverage() -> Iterator[None]:
    ...
    from tox.tox_env.python.virtual_env.package.pyproject import \
        Pep517VirtualEnvPackager  # noqa: PLC0415
...
class ToxRunOutcome:
    ...
    def assert_out_err(
        self, out: str, err: str, *, dedent: bool = True, regex: bool = False
    ) -> None:
        if dedent:
    ...
    def matches(pattern: str, text: str, flags: int = 0) -> None:
        try:
            ...
            if match is None:
                warnings.warn(
                    "install the re-assert PyPI package for bette error message",
                    UserWarning,
                    stacklevel=1,
                )
...
def init_fixture(
    ...
    def _init(
        files: dict[str, Any], base: Path | None = None, prj_path: Path | None = None
    ) -> ToxProject:
        """Create tox  projects."""
        return ToxProject(
            files, base, prj_path or tmp_path / "p", capfd, monkeypatch, mocker
        )
...
def pytest_collection_modifyitems(
    ...
    skip_int = pytest.mark.skip(
        reason=f"integration tests not run (no {_RUN_INTEGRATION_TEST_FLAG} flag)"
    )
...
def enable_pypi_server(monkeypatch: pytest.MonkeyPatch, url: str | None) -> None:
    if (
        url is None
    ):  # pragma: no cover # only one of the branches can be hit depending on env
