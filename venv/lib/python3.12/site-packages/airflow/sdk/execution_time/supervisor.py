...
from typing import TYPE_CHECKING, Callable, ClassVar, NoReturn, TextIO, cast
...
def _configure_logs_over_json_channel(log_fd: int):
    ...
    configure_logging(
        enable_pretty_log=False, output=log_io, sending_to_supervisor=True
    )
...
def block_orm_access():
    ...
    if "airflow.settings" in sys.modules:
        ...
        to_block = frozenset(
            ("engine", "async_engine", "Session", "AsyncSession", "NonScopedSession")
        )
        ...
        def __getattr__(name: str):
            if name in to_block:
                raise AttributeError(
                    "Access to the Airflow Metadatabase from dags is not allowed!"
                )
            raise AttributeError(
                f"module {settings.__name__!r} has no attribute {name!r}"
            )
...
class WatchedSubprocess:
    ...
    selector: selectors.BaseSelector = attrs.field(
        factory=selectors.DefaultSelector, repr=False
    )
        ...
        **constructor_kwargs,
        ...
        if pid == 0:
            ...
            cls._close_unused_sockets(
                feed_stdin, read_stdout, read_stderr, read_msgs, read_logs
            )
            ...
            try:
                ...
                _fork_main(
                    child_stdin, child_stdout, child_stderr, child_logs.fileno(), target
                )
        ...
        cls._close_unused_sockets(
            child_stdin, child_stdout, child_stderr, child_comms, child_logs
        )

        logger = logger or cast(
            "FilteringBoundLogger", structlog.get_logger(logger_name="task").bind()
        )
        proc = cls(
            pid=pid,
            stdin=feed_stdin,
    ...
    def _register_pipe_readers(
        self, stdout: socket, stderr: socket, requests: socket, logs: socket
    ):
        ...
        self.selector.register(
            stderr,
            selectors.EVENT_READ,
            self._create_socket_handler(
                target_loggers, channel="stderr", log_level=logging.ERROR
            ),
        )
        self.selector.register(
            logs,
        ...
        self.selector.register(
            ...
            selectors.EVENT_READ,
            make_buffered_socket_reader(
                self.handle_requests(log), on_close=self._on_socket_closed
            ),
        )

    def _create_socket_handler(
        self, loggers, channel, log_level=logging.INFO
        ...
        try:
            ...
            if self._exit_code == -signal.SIGSEGV:
                ...
                self.process_log.log(
                    level, message, signal=int(self._exit_code), signal_name=name
                )
                ...
                self.process_log.warning(
                    "Process exited abnormally", exit_code=self._exit_code
                )
        return self._exit_code
...
class ActivitySubprocess(WatchedSubprocess):
    ...
    def start(  # type: ignore[override]
        ...
        **kwargs,
        ...
        proc._on_child_started(
            ti=what, dag_rel_path=dag_rel_path, bundle_info=bundle_info
        )
        return proc

    def _on_child_started(
        self, ti: TaskInstance, dag_rel_path: str | os.PathLike[str], bundle_info
    ):
    ...
    def _handle_process_overtime_if_needed(self):
        ...
        if (
            ...
            log.warning(
                "Workload success overtime reached; terminating process", ti_id=self.id
            )
    ...
    def _handle_request(self, msg: ToSupervisor, log: FilteringBoundLogger):
        ...
        if isinstance(msg, TaskState):
            ...
            asset_event_result = AssetEventsResult.from_asset_events_response(
                asset_event_resp
            )
            ...
            dagrun_resp = self.client.task_instances.get_previous_successful_dagrun(
                self.id
            )
            ...
            resp = self.client.task_instances.get_reschedule_start_date(
                msg.ti_id, msg.try_number
            )
            ...
            resp = InactiveAssetsResult.from_inactive_assets_response(
                inactive_assets_resp
            )
            dump_opts = {"exclude_unset": True}
        else:
...
class InProcessTestSupervisor(ActivitySubprocess):
    ...
    def start(  # type: ignore[override]
        ...
        **kwargs,
        ...
        with set_supervisor_comms(supervisor.comms):
            ...
            start_date = datetime.now(tz=timezone.utc)
            ti_context = supervisor.client.task_instances.start(
                supervisor.id, supervisor.pid, start_date
            )
    ...
    def _api_client(dag=None):
        ...
        if dag is not None:
            ...
            dag_bag = DagBag(
                include_examples=False, collect_dags=False, load_op_links=False
            )

            # Mimic the behavior of the DagBag in the API server by converting the DAG to a SerializedDAG
            dag_bag.dags[dag.dag_id] = SerializedDAG.from_dict(
...
def process_log_messages_from_subprocess(
    ...
    while True:
        ...
        if ts := event.get("timestamp"):
            ...
            # confusing.
            event["timestamp"] = msgspec.json.decode(f'"{ts}"', type=datetime).replace(
                tzinfo=None
            )
...
def ensure_secrets_backend_loaded() -> list[BaseSecretsBackend]:
    ...
    backends = ensure_secrets_loaded(
        default_backends=DEFAULT_SECRETS_SEARCH_PATH_WORKERS
    )
...
def supervise(
    *,
    ...
    if not client:
        ...
        client = Client(
            base_url=server or "", limits=limits, dry_run=dry_run, token=token
        )
    ...
    if log_path:
        ...
        pretty_logs = False
        if pretty_logs:
            underlying_logger: WrappedLogger = structlog.WriteLogger(
                log_file.open("a", buffering=1)
            )
        else:
        ...
        logger = structlog.wrap_logger(
            underlying_logger, processors=processors, logger_name="task"
        ).bind()
