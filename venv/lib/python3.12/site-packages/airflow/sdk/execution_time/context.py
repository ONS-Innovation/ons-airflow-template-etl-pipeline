...
def _convert_variable_result_to_variable(
    var_result: VariableResult, deserialize_json: bool
) -> Variable:
...
def _set_variable(
    key: str, value: Any, description: str | None = None, serialize_json: bool = False
) -> None:
    # TODO: This should probably be moved to a separate module like `airflow.sdk.execution_time.comms`
    #   or `airflow.sdk.execution_time.variable`
    #   A reason to not move it to `airflow.sdk.execution_time.comms` is that it
    ...
    with SUPERVISOR_COMMS.lock:
        SUPERVISOR_COMMS.send_request(
            log=log, msg=PutVariable(key=key, value=value, description=description)
        )
...
class TriggeringAssetEventsAccessor(
    ...
    Mapping[
        Union[Asset, AssetAlias, AssetRef], Sequence["AssetEventDagRunReferenceResult"]
    ],
):
    ...
    @classmethod
    def build(
        cls, events: Iterable[AssetEventDagRunReferenceResult]
    ) -> TriggeringAssetEventsAccessor:
        coll: dict[
            BaseAssetUniqueKey, list[AssetEventDagRunReferenceResult]
        ] = collections.defaultdict(list)
        for event in events:
            coll[AssetUniqueKey(name=event.asset.name, uri=event.asset.uri)].append(
                event
            )
    ...
    def __getitem__(
        self, key: Asset | AssetAlias | AssetRef
        ...
        if isinstance(key, Asset):
            ...
            raise TypeError(
                f"Key should be either an asset or an asset alias, not {type(key)}"
            )
...
class InletEventsAccessors(
    Mapping[Union[int, Asset, AssetAlias, AssetRef], Any], _AssetEventAccessorsMixin
):
    ...
    def __getitem__(
        self, key: int | Asset | AssetAlias | AssetRef
        ...
        if isinstance(obj, Asset):
            ...
            SUPERVISOR_COMMS.send_request(
                log=log, msg=GetAssetEventByAsset(name=asset.name, uri=asset.uri)
            )
            ...
            SUPERVISOR_COMMS.send_request(
                log=log, msg=GetAssetEventByAsset(name=asset.name, uri=None)
            )
            ...
            try:
                ...
                raise KeyError(obj) from None
            SUPERVISOR_COMMS.send_request(
                log=log, msg=GetAssetEventByAsset(name=None, uri=asset.uri)
            )
        elif isinstance(obj, AssetAlias):
            asset_alias = self._asset_aliases[AssetAliasUniqueKey.from_asset_alias(obj)]
            SUPERVISOR_COMMS.send_request(
                log=log, msg=GetAssetEventByAssetAlias(alias_name=asset_alias.name)
            )
