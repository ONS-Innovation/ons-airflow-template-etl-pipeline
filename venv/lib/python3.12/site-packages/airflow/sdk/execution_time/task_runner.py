...
from airflow.sdk.execution_time.comms import (AssetEventDagRunReferenceResult,
                                              ...
                                              GetDagRunState, GetDRCount,
                                              GetTaskRescheduleStartDate,
                                              GetTaskStates, GetTICount,
                                              InactiveAssetsResult,
                                              RescheduleTask, RetryTask,
                                              SetRenderedFields,
                                              SkipDownstreamTasks,
...
class RuntimeTaskInstance(TaskInstance):
    ...
    def get_template_context(self) -> Context:
        ...
        validated_params = process_params(
            self.task.dag, self.task, dag_run_conf, suppress_exception=False
        )
        ...
        if from_server:
            ...
            context_from_server: Context = {
                ...
                "triggering_asset_events": TriggeringAssetEventsAccessor.build(
                    AssetEventDagRunReferenceResult.from_asset_event_dag_run_reference(
                        event
                    )
                ...
                "prev_start_date_success": lazy_object_proxy.Proxy(
                    lambda: coerce_datetime(
                        get_previous_dagrun_success(self.id).start_date
                    )
                ),
                "prev_end_date_success": lazy_object_proxy.Proxy(
                    lambda: coerce_datetime(
                        get_previous_dagrun_success(self.id).end_date
                    )
                ),
            }
                ...
                context.update(
                    {
                        ...
                        "data_interval_start": coerce_datetime(
    ...
    def xcom_pull(
        ...
        *,
        ...
        single_map_index_requested = isinstance(
            map_indexes, (int, type(None), ArgNotSet)
        )
    ...
    def get_dagrun_state(dag_id: str, run_id: str) -> str:
        ...
        with SUPERVISOR_COMMS.lock:
            SUPERVISOR_COMMS.send_request(
                log=log, msg=GetDagRunState(dag_id=dag_id, run_id=run_id)
            )
            response = SUPERVISOR_COMMS.get_message()
...
def _xcom_push(
    ti: RuntimeTaskInstance, key: str, value: Any, mapped_length: int | None = None
) -> None:
...
class CommsDecoder(Generic[ReceiveMsgType, SendMsgType]):
    ...
    decoder: TypeAdapter[ReceiveMsgType] = attrs.field(
        factory=lambda: TypeAdapter(ToTask), repr=False
    )
    ...
    def get_message(self) -> ReceiveMsgType:
        ...
        try:
            ...
            structlog.get_logger(logger_name="CommsDecoder").exception(
                "Unable to decode message", line=line
            )
            raise
        ...
        if isinstance(msg, StartupDetails):
            ...
            structlog.get_logger(logger_name="task").error(
...
def startup() -> tuple[RuntimeTaskInstance, Context, Logger]:
    ...
    if isinstance(msg, StartupDetails):
        ...
        with _airflow_parsing_context_manager(
            dag_id=msg.ti.dag_id, task_id=msg.ti.task_id
        ):
            ti = parse(msg, log)
...
def _serialize_outlet_events(
    events: OutletEventAccessorsProtocol,
...
def _prepare(
    ti: RuntimeTaskInstance, log: Logger, context: Context
) -> ToSupervisor | None:
    ...
    if rendered_fields := _serialize_rendered_fields(ti.task):
        ...
        SUPERVISOR_COMMS.send_request(
            log=log, msg=SetRenderedFields(rendered_fields=rendered_fields)
        )
...
def _validate_task_inlets_and_outlets(*, ti: RuntimeTaskInstance, log: Logger) -> None:
    ...
    if inactive_assets := inactive_assets_resp.inactive_assets:
        raise AirflowInactiveAssetInInletOrOutletException(
            inactive_asset_keys=[
                AssetUniqueKey.from_profile(asset_profile)
                for asset_profile in inactive_assets
            ]
        )
...
def run(
    ...
    try:
        ...
        SUPERVISOR_COMMS.send_request(
            log=log, msg=SkipDownstreamTasks(tasks=tasks_to_skip)
        )
        msg, state = _handle_current_task_success(context, ti)
...
def _handle_trigger_dag_run(
    ...
    if (
        ...
        and comms_msg.error == ErrorType.DAGRUN_ALREADY_EXISTS
    ):
        if drte.skip_when_already_exists:
            ...
            log.error(
                "Dag Run already exists, marking task as failed.",
                dag_id=drte.trigger_dag_id,
            )
            msg = TaskState(
                state=TaskInstanceState.FAILED,
    ...
    if drte.wait_for_completion:
        while True:
            ...
            SUPERVISOR_COMMS.send_request(
                log=log,
                msg=GetDagRunState(dag_id=drte.trigger_dag_id, run_id=drte.dag_run_id),
            )
            comms_msg = SUPERVISOR_COMMS.get_message()
            if TYPE_CHECKING:
    ...
    for i, callback in enumerate(getattr(task, kind)):
        try:
            create_executable_runner(
                callback, context_get_outlet_events(context), logger=log
            ).run(context)
        except Exception:
            log.exception(
                "Failed to run task callback", kind=kind, index=i, callback=callback
            )


def _send_task_error_email(
    to: Iterable[str], ti: RuntimeTaskInstance, exception: BaseException
) -> None:
    from airflow.models.taskinstance import _get_email_subject_content
    from airflow.utils.email import send_email

    subject, content, err = _get_email_subject_content(
        task_instance=ti, exception=exception
    )
    try:
...
def _execute_task(context: Context, ti: RuntimeTaskInstance, log: Logger):
    ...
    if ti._ti_context_from_server and (
        ...
        kwargs = BaseSerialization.deserialize(
            ti._ti_context_from_server.next_kwargs or {}
        )

        execute = functools.partial(
            task.resume_execution, next_method=next_method, next_kwargs=kwargs
        )
...
def _render_map_index(
    context: Context, ti: RuntimeTaskInstance, log: Logger
) -> str | None:
        ...
        if ti.task.template_fields:
            SUPERVISOR_COMMS.send_request(
                ...
                msg=SetRenderedFields(
                    rendered_fields=_serialize_rendered_fields(ti.task)
                ),
            )
