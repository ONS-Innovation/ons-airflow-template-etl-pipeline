...
from airflow.sdk.execution_time.comms import (DRCount, ErrorResponse,
                                              OKResponse, SkipDownstreamTasks,
...
class TaskInstanceOperations:
    ...
    def start(self, id: uuid.UUID, pid: int, when: datetime) -> TIRunContext:
        ...
        body = TIEnterRunningPayload(
            pid=pid, hostname=get_hostname(), unixname=getuser(), start_date=when
        )

        resp = self.client.patch(
            f"task-instances/{id}/run", content=body.model_dump_json()
        )
    ...
    def finish(
        ...
        if state == TaskInstanceState.SUCCESS:
            raise ValueError(
                "Logic error. SUCCESS state should call the `succeed` function instead"
            )
    ...
    def retry(self, id: uuid.UUID, end_date: datetime, rendered_map_index):
        ...
        body = TIRetryStatePayload(
            end_date=end_date, rendered_map_index=rendered_map_index
        )
        self.client.patch(f"task-instances/{id}/state", content=body.model_dump_json())
    ...
    def defer(self, id: uuid.UUID, msg):
        ...
        body = TIDeferredStatePayload(
            **msg.model_dump(exclude_unset=True, exclude={"type"})
        )
    ...
    def heartbeat(self, id: uuid.UUID, pid: int):
        ...
        self.client.put(
            f"task-instances/{id}/heartbeat", content=body.model_dump_json()
        )
    ...
    def skip_downstream_tasks(self, id: uuid.UUID, msg: SkipDownstreamTasks):
        ...
        self.client.patch(
            f"task-instances/{id}/skip-downstream", content=body.model_dump_json()
        )
    ...
    def get_reschedule_start_date(
        self, id: uuid.UUID, try_number: int = 1
    ) -> TaskRescheduleStartDate:
        """Get the start date of a task reschedule via the API server."""
        resp = self.client.get(
            f"task-reschedules/{id}/start_date", params={"try_number": try_number}
        )
    ...
    def get_count(
        ...
        params = {
            ...
            "logical_dates": [d.isoformat() for d in logical_dates]
    ...
    def get(self, key: str) -> VariableResponse | ErrorResponse:
        ...
        try:
            ...
            if e.response.status_code == HTTPStatus.NOT_FOUND:
                ...
                return ErrorResponse(
                    error=ErrorType.VARIABLE_NOT_FOUND, detail={"key": key}
                )
            raise
...
class XComOperations:
    ...
    def head(self, dag_id: str, run_id: str, task_id: str, key: str) -> int:
        ...
        ) or not content_range.startswith("map_indexes "):
            raise RuntimeError(
                f"Unable to parse Content-Range header from HEAD {resp.request.url}"
            )
    ...
    def get(
        ...
        try:
            resp = self.client.get(
                f"xcoms/{dag_id}/{run_id}/{task_id}/{key}", params=params
            )
        except ServerResponseError as e:
            if e.response.status_code == HTTPStatus.NOT_FOUND:
                log.error(
    ...
    def set(
        ...
        self.client.post(
            f"xcoms/{dag_id}/{run_id}/{task_id}/{key}", params=params, json=value
        )
        # Any error from the server will anyway be propagated down to the supervisor,
        # so we choose to send a generic response to the supervisor over the server response to
        # decouple from the server response string
    ...
    def get_sequence_slice(
        ...
        if step is not None:
            params["step"] = step
        resp = self.client.get(
            f"xcoms/{dag_id}/{run_id}/{task_id}/{key}/slice", params=params
        )
...
class DagRunOperations:
    ...
    def trigger(
        ...
        body = TriggerDAGRunPayload(
            logical_date=logical_date, conf=conf or {}, reset_dag_run=reset_dag_run
        )
        ...
        try:
            ...
            if e.response.status_code == HTTPStatus.CONFLICT:
                if reset_dag_run:
                    log.info(
                        "DAG Run already exists; Resetting DAG Run.",
                        dag_id=dag_id,
                        run_id=run_id,
                    )
...
class Client(httpx.Client):
    def __init__(
        self, *, base_url: str | None, dry_run: bool = False, token: str, **kwargs: Any
    ):
    ...
    _default_wait = wait_random_exponential(
        min=API_RETRY_WAIT_MIN, max=API_RETRY_WAIT_MAX
    )
    ...
    @retry(
        ...
        wait_rate_limited=wait_retry_after(
            fallback=_default_wait
        ),  # No infinite timeout on HTTP 429
