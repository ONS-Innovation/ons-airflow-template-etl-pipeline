...
def exception_group_tracebacks(
    ...
    def _exception_group_tracebacks(
        logger: Any, method_name: Any, event_dict: EventDict
    ) -> EventDict:
...
def logging_processors(
    enable_pretty_log: bool, mask_secrets: bool = True, colored_console_log: bool = True
):
    ...
    if (remote := load_remote_log_handler()) and (
        remote_processors := getattr(remote, "processors")
    ):
...
def configure_logging(
    ...
    if colored_console_log is None:
        ...
        colored_console_log = conf.getboolean(
            "logging", "colored_console_log", fallback=True
        )
    ...
    logging.config.dictConfig(
        {
            ...
            "loggers": {
                ...
                "": {
                    "handlers": [
                        "to_supervisor" if sending_to_supervisor else "default"
                    ],
                    "level": "INFO",
                    "propagate": True,
                },
...
def relative_path_from_logger(logger) -> Path | None:
    ...
    if not hasattr(logger, "_file"):
        logger.warning(
            "Unable to find log file, logger was of unexpected type", type=type(logger)
        )
        return None
