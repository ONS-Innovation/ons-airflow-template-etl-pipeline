...
class AssetWatcher:
    ...
    def __init__(
        ...
        if isinstance(trigger, BaseTrigger) and not isinstance(
            trigger, BaseEventTrigger
        ):
            raise ValueError(
                "The trigger used to watch an asset must inherit ``BaseEventTrigger``"
            )
...
class Asset(os.PathLike, BaseAsset):
    ...
    def ref(*, name: str = "", uri: str = "") -> AssetRef:
        if name and uri:
            raise TypeError(
                "Asset reference must be made to either name or URI, not both"
            )
        if name:
    ...
    def asprofile(self) -> AssetProfile:
        ...
        """
        return AssetProfile(
            name=self.name or None, uri=self.uri or None, type=Asset.__name__
        )
...
class AssetAlias(BaseAsset):
    ...
    group: str = attrs.field(
        kw_only=True, default="asset", validator=_validate_identifier
    )
