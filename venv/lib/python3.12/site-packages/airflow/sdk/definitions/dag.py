...
def _create_timetable(
    interval: ScheduleInterval, timezone: Timezone | FixedTimezone
) -> Timetable:
...
class DAG:
    ...
    max_consecutive_failed_dag_runs: int = attrs.field(
        default=0, validator=attrs.validators.instance_of(int)
    )
    ...
    on_success_callback: None | DagStateChangeCallback | list[
        DagStateChangeCallback
    ] = None
    on_failure_callback: None | DagStateChangeCallback | list[
        DagStateChangeCallback
    ] = None
    ...
    edge_info: dict[str, dict[str, EdgeInfoType]] = attrs.field(
    ...
    def __attrs_post_init__(self):
        ...
        if isinstance(_end_date := self.default_args.get("end_date"), str):
            self.default_args["end_date"] = timezone.parse(
                _end_date, timezone=self.timezone
            )
    ...
    def _validate_catchup(self, _, catchup: bool):
        ...
        if requires_automatic_backfilling and not (
            "start_date" in self.default_args or self.start_date
        ):
    ...
    def __eq__(self, other: Self | Any):
        ...
        if type(self) is not type(other):
            return False
        return all(
            getattr(self, c, None) == getattr(other, c, None) for c in _DAG_HASH_ATTRS
        )
    ...
    def task_group_dict(self):
        return {
            k: v
            for k, v in self.task_group.get_task_group_dict().items()
            if k is not None
        }
    ...
    def add_task(self, task: Operator) -> None:
        ...
        FailFastDagInvalidTriggerRule.check(
            fail_fast=self.fail_fast, trigger_rule=task.trigger_rule
        )
    ...
    def get_edge_info(
        ...
        if self.edge_info:
            return self.edge_info.get(upstream_task_id, {}).get(
                downstream_task_id, empty
            )
        return empty
    ...
    def test(
        ...
        with exit_stack:
            self.validate()
            log.debug(
                "Clearing existing task instances for logical date %s", logical_date
            )
            ...
            run_after = timezone.coerce_datetime(run_after) or timezone.coerce_datetime(
                timezone.utcnow()
            )
            data_interval = (
            ...
            while dr.state == DagRunState.RUNNING:
                ...
                scheduled_tis = {
                    x for x in all_tis if x.state == TaskInstanceState.SCHEDULED
                }
                ids_unrunnable = {
                    x for x in all_tis if x.state not in State.finished
                } - scheduled_tis
                    ...
                    if use_executor:
                        ...
                        try:
                            ...
                            if mark_success:
                                ...
                                log.info(
                                    "[DAG TEST] Marking success for %s on %s",
                                    ti.task,
                                    ti.logical_date,
                                )
                            else:
