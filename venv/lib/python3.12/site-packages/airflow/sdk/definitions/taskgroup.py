...
class TaskGroup(DAGNode):
    ...
    group_display_name: str = attrs.field(
    ...
    ui_color: str = attrs.field(
        default="CornflowerBlue", validator=attrs.validators.instance_of(str)
    )
    ui_fgcolor: str = attrs.field(
        default="#000", validator=attrs.validators.instance_of(str)
    )
    ...
    def add(self, task: DAGNode) -> DAGNode:
        ...
        if (
            ...
            raise TaskAlreadyInTaskGroup(
                task.node_id, existing_tg.node_id, self.node_id
            )
        ...
        def find_leaves(group_or_task) -> list[Any]:
            while group_or_task:
                ...
                if group_or_task.upstream_task_ids:
                    ...
                    return [
                        self.dag.get_task(task_id) for task_id in upstream_task_ids_list
                    ]
    ...
    def get_leaves(self) -> Generator[BaseOperator, None, None]:
        ...
        def recurse_for_first_non_teardown(task):
            for upstream_task in task.upstream_list:
                ...
                elif not has_non_teardown_downstream(
                    upstream_task, exclude=task.task_id
                ):
                    yield upstream_task
...
class MappedTaskGroup(TaskGroup):
    ...
    def __iter__(self):
        ...
        for child in self.children.values():
            if (
                isinstance(child, AbstractOperator)
                and child.trigger_rule == TriggerRule.ALWAYS
            ):
                raise ValueError(
...
def task_group_to_dict(task_item_or_group):
    ...
    if task_group.upstream_group_ids or task_group.upstream_task_ids:
        ...
        children.append(
            {"id": task_group.upstream_join_id, "label": "", "type": "join"}
        )

    if task_group.downstream_group_ids or task_group.downstream_task_ids:
        # This is the join node used to reduce the number of edges between two TaskGroup.
        children.append(
            {"id": task_group.downstream_join_id, "label": "", "type": "join"}
        )
