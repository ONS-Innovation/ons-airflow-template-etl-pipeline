...
def is_mappable(v: Any) -> TypeGuard[OperatorExpandArgument]:
    ...
    return isinstance(
        v, (MappedArgument, XComArg, Mapping, Sequence)
...
class DictOfListsExpandInput(ResolveMixin):
    ...
    def _get_map_lengths(
        ...
        map_lengths = {
            ...
            if v is not None
            if (res := _get_length(k, v)) is not None
        }
    ...
    def _expand_mapped_field(
        self, key: str, value: Any, map_index: int, all_lengths: dict[str, int]
    ) -> Any:
    ...
    def resolve(self, context: Mapping[str, Any]) -> tuple[Mapping[str, Any], set[int]]:
        ...
        # more efficient to do one single call and unpack the value here?
        resolved = {
            k: v.resolve(context) if _needs_run_time_resolution(v) else v
            for k, v in self.value.items()
        }
        ...
        data = {
            k: self._expand_mapped_field(k, v, map_index, all_lengths)
