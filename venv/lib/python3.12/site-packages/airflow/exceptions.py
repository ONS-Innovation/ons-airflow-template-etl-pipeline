...
class _AirflowExecuteWithInactiveAssetExecption(AirflowFailException):
    ...
    def __init__(
        self,
        inactive_asset_keys: Collection[AssetUniqueKey | AssetNameRef | AssetUriRef],
    ) -> None:
    ...
    def _render_asset_key(key: AssetUniqueKey | AssetNameRef | AssetUriRef) -> str:
        ...
        return repr(
            key
        )  # Should not happen, but let's fails more gracefully in an exception.
    ...
    def inactive_assets_message(self) -> str:
        return ", ".join(
            self._render_asset_key(key) for key in self.inactive_asset_keys
        )


class AirflowInactiveAssetInInletOrOutletException(
...
class DagRunAlreadyExists(AirflowBadRequest):
    ...
    def __init__(self, dag_run: DagRun) -> None:
        super().__init__(
            f"A DAG Run already exists for DAG {dag_run.dag_id} with run id {dag_run.run_id}"
        )
