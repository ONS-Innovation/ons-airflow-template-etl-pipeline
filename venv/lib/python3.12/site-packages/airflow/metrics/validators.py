...
BACK_COMPAT_METRIC_NAMES: set[Pattern[str]] = {
    re.compile(name) for name in BACK_COMPAT_METRIC_NAME_PATTERNS
}
...
def get_validator() -> ListValidator:
    ...
    metric_lists = {
        "allow": (
            metric_allow_list := conf.get(
                "metrics", "metrics_allow_list", fallback=None
            )
        ),
        "block": (
            metric_block_list := conf.get(
                "metrics", "metrics_block_list", fallback=None
            )
        ),
    }
