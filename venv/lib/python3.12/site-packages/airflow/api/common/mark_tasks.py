    ...
    if not dag.timetable.can_be_scheduled:
        ...
        run_ids = [
            ...
            for dr in DagRun.find(
                dag_id=dag.dag_id, logical_date=dates, session=session
            )
        ]
    return run_ids
...
def _set_dag_run_state(
    dag_id: str, run_id: str, state: DagRunState, session: SASession
):
    """
...
def set_dag_run_state_to_success(
    *,
    ...
    tasks_to_mark_success = normal_tasks + [
        ...
        if task.task_id not in unfinished_teardown_task_ids
    ]
...
def set_dag_run_state_to_failed(
    *,
    ...
    task_ids_of_running_tis = [
        ti.task_id for ti in running_tis if not dag.task_dict[ti.task_id].is_teardown
    ]
    ...
    pending_normal_tis = [
        ti for ti in pending_tis if not dag.task_dict[ti.task_id].is_teardown
    ]
    ...
    if commit:
        ...
        if not any(
            dag.task_dict[ti.task_id].is_teardown for ti in (running_tis + pending_tis)
        ):
