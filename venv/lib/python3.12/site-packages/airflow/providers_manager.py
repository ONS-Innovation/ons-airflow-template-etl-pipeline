...
class ProvidersManager(LoggingMixin, metaclass=Singleton):
    ...
    def __init__(self):
        ...
        self._hooks_lazy_dict: LazyDictWithCache[
            str, HookInfo | Callable
        ] = LazyDictWithCache()
    ...
    def _import_hook(
        ...
        if connection_type is not None:
            ...
            if not hook_class_name:
                raise ValueError(
                    "Either connection_type or hook_class_name must be set"
                )
            if not package_name:
                raise ValueError(
    ...
    def _add_widgets(
        self, package_name: str, hook_class: type, widgets: dict[str, Any]
    ):
        ...
        for provider_package, provider in self._provider_dict.items():
            if provider.data.get("logging"):
                for logging_class_name in provider.data["logging"]:
                    if _correctness_check(
                        provider_package, logging_class_name, provider
                    ):
