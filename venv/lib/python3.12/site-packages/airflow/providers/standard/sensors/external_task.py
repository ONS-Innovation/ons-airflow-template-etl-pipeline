...
if AIRFLOW_V_3_0_PLUS:
    ...
    from airflow.models.baseoperatorlink import \
        BaseOperatorLink  # type: ignore[no-redef]
    ...
    def get_link(self, operator: BaseOperator, *, ti_key: TaskInstanceKey) -> str:
        ...
        if AIRFLOW_V_3_0_PLUS:
            ...
            return build_airflow_dagrun_url(
                dag_id=external_dag_id, run_id=ti_key.run_id
            )
    ...
    *not* fail if the external task fails, but will continue to check the status
    ...
    def __init__(
        ...
        **kwargs,
        ...
        total_states = set(
            self.allowed_states + self.skipped_states + self.failed_states
        )

        if len(total_states) != len(self.allowed_states) + len(
            self.skipped_states
        ) + len(self.failed_states):
    ...
    def _calculate_count(
        self, count: int, dttm_filter: list[datetime.datetime]
    ) -> float | int:
    ...
    if not AIRFLOW_V_3_0_PLUS:
        ...
        @provide_session
        def _poke_af2(
            self, dttm_filter: list[datetime.datetime], session: Session = NEW_SESSION
        ) -> bool:
            ...
            if self.skipped_states:
                count_skipped = self.get_count(
                    dttm_filter, session, self.skipped_states
                )
                self._handle_skipped_states(count_skipped)
    ...
    def execute(self, context: Context) -> None:
        ...
        if not self.deferrable:
            ...
            logical_or_execution_dates = (
                {"logical_dates": dttm_filter}
                if AIRFLOW_V_3_0_PLUS
                else {"execution_dates": dttm_filter}
            )
            self.defer(
