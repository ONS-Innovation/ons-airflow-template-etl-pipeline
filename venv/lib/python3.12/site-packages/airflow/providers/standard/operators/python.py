...
from airflow.exceptions import (AirflowConfigException, AirflowException,
                                ...
                                AirflowSkipException, DeserializingResultError)
...
if AIRFLOW_V_3_0_PLUS:
    ...
    from airflow.operators.branch import \
        BaseBranchOperator  # type: ignore[no-redef]
...
class PythonOperator(BaseOperator):
    ...
    def execute(self, context: Context) -> Any:
        ...
        def __prepare_execution() -> (
            tuple[ExecutionCallableRunner, OutletEventAccessorsProtocol] | None
        ):
            if AIRFLOW_V_3_0_PLUS:
                from airflow.sdk.execution_time.callback_runner import \
...
class ShortCircuitOperator(PythonOperator, SkipMixin):
    ...
    def __init__(
        self, *, ignore_downstream_trigger_rules: bool = True, **kwargs
    ) -> None:
        super().__init__(**kwargs)
    ...
    def execute(self, context: Context) -> Any:
        ...
        if self.log.getEffectiveLevel() <= logging.DEBUG:
            self.log.debug(
                "Downstream task IDs %s", to_skip := list(get_tasks_to_skip())
            )
...
class _BasePythonVirtualenvOperator(PythonOperator, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            raise ValueError(
                f"{type(self).__name__} only supports functions for python_callable arg"
            )
        if inspect.isgeneratorfunction(python_callable):
            raise ValueError(
                f"{type(self).__name__} does not support using 'yield' in python_callable"
            )
        super().__init__(
        ...
        if serializer not in _SERIALIZERS:
            msg = f"Unsupported serializer {serializer!r}. Expected one of {', '.join(map(repr, _SERIALIZERS))}"
    ...
    def _write_args(self, file: Path):
        ...
        if self.op_args or self.op_kwargs:
            ...
            file.write_bytes(
                self.pickling_library.dumps(
                    {"args": self.op_args, "kwargs": resolve_proxies(self.op_kwargs)}
                )
            )
    ...
    def determine_kwargs(self, context: Mapping[str, Any]) -> Mapping[str, Any]:
        keyword_params = KeywordParameters.determine(
            self.python_callable, self.op_args, context
        )
        if AIRFLOW_V_3_0_PLUS:
...
class PythonVirtualenvOperator(_BasePythonVirtualenvOperator):
    ...
    def _calculate_cache_hash(
        self, exclude_cloudpickle: bool = False
    ) -> tuple[str, str]:
        """
    ...
    def _ensure_venv_cache_exists(self, venv_cache_path: Path) -> Path:
        ...
        with open(f"{venv_path}.lock", "w") as f:
            ...
            try:
                if venv_path.exists():
                    if hash_marker.exists():
                        ...
                        if previous_hash_data == hash_data:
                            self.log.info(
                                "Reusing cached Python virtual environment in %s",
                                venv_path,
                            )
                            return venv_path

                        _, hash_data_before_upgrade = self._calculate_cache_hash(
                            exclude_cloudpickle=True
                        )
                ...
                shutil.rmtree(venv_path)
                raise AirflowException(
                    f"Unable to create new virtual environment in {venv_path}"
                ) from e
    ...
    def _iter_serializable_context_keys(self):
        ...
        if self.system_site_packages:
            ...
            for raw_str in chain.from_iterable(
                req.splitlines() for req in self.requirements
            ):
                line = raw_str.strip()
...
class ExternalPythonOperator(_BasePythonVirtualenvOperator):
    ...
    template_fields: Sequence[str] = tuple(
        {"python"}.union(PythonOperator.template_fields)
    )
    ...
    def execute_callable(self):
        ...
        if python_version.major != sys.version_info.major and (
            self.op_args or self.op_kwargs
        ):
            raise AirflowException(
                "Passing op_args or op_kwargs is not supported across different Python "
    ...
    def _is_pendulum_installed_in_target_env(self) -> bool:
        try:
            ...
            if self.expect_pendulum:
                self.log.warning(
                    "When checking for Pendulum installed in virtual environment got %s",
                    e,
                )
                self.log.warning(
