...
class WorkflowTrigger(BaseTrigger):
    ...
    async def _get_count_af_3(self, states):
        ...
        if self.external_task_ids:
            ...
            run_id_task_state_map = await sync_to_async(
...
class DagStateTrigger(BaseTrigger):
    ...
    async def run(self) -> typing.AsyncIterator[TriggerEvent]:
        ...
        if AIRFLOW_V_3_0_PLUS:
            event = await self.validate_count_dags_af_3(
    ...
    async def validate_count_dags_af_3(
        self, runs_ids_or_dates_len: int = 0
                ...
                if isinstance(self.run_ids, list):
                    for run_id in self.run_ids:
                        state = await sync_to_async(
                            RuntimeTaskInstance.get_dagrun_state
                        )(
                            dag_id=self.dag_id,
                            run_id=run_id,
                        )
