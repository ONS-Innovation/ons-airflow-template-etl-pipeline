...
class Dialect(LoggingMixin):
    ...
    def __init__(self, hook, **kwargs) -> None:
        ...
        if not isinstance(hook, DbApiHook):
            raise TypeError(
                f"hook must be an instance of {DbApiHook.__class__.__name__}"
            )
    ...
    def escape_word(self, word: str) -> str:
        ...
        if word != self.escape_word_format.format(self.unescape_word(word)) and (
            ...
            or word.casefold() in self.reserved_words
            or self.pattern.search(word)
        ):
    ...
    def get_column_names(
        ...
        predicate: Callable[[T], bool] = lambda column: True,
    ) -> list[str] | None:
        if schema is None:
    ...
    def get_target_fields(
        ...
        target_fields = self.get_column_names(
            table,
            schema,
            lambda column: not column.get("identity", False)
    ...
    def run(
        ...
        return self.hook.run(
            sql, autocommit, parameters, handler, split_statements, return_last
        )
    ...
    def generate_insert_sql(self, table, values, target_fields, **kwargs) -> str:
        ...
        return self.insert_statement_format.format(
            table,
            self._joined_target_fields(target_fields),
