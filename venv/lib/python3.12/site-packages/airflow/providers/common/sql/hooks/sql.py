...
def return_single_query_results(
    ...
    warnings.warn(
        WARNING_MESSAGE.format("return_single_query_results"),
        DeprecationWarning,
        stacklevel=2,
    )
...
def fetch_all_handler(cursor) -> list[tuple] | None:
    warnings.warn(
        WARNING_MESSAGE.format("fetch_all_handler"), DeprecationWarning, stacklevel=2
    )
...
def fetch_one_handler(cursor) -> list[tuple] | None:
    warnings.warn(
        WARNING_MESSAGE.format("fetch_one_handler"), DeprecationWarning, stacklevel=2
    )
...
def resolve_dialects() -> MutableMapping[str, MutableMapping]:
    ...
    if hasattr(providers_manager, "dialects"):
        return {
            key: dict(value._asdict())
            for key, value in providers_manager.dialects.items()
        }
...
class DbApiHook(BaseHook):
    ...
    _resolve_target_fields = conf.getboolean(
        "core", "dbapihook_resolve_target_fields", fallback=False
    )

    def __init__(
        self, *args, schema: str | None = None, log_sql: bool = True, **kwargs
    ):
        super().__init__()
        ...
        self._insert_statement_format: str | None = kwargs.get(
            "insert_statement_format"
        )
        self._replace_statement_format: str | None = kwargs.get(
    ...
    def escape_column_names(self) -> bool:
        ...
        if self._escape_column_names is None:
            self._escape_column_names = self.connection_extra.get(
                "escape_column_names", False
            )
    ...
    def get_conn(self) -> Any:
        ...
        if self.connector is None:
            raise RuntimeError(
                f"{type(self).__name__} didn't have `self.connector` set!"
            )
    ...
    def dialect_name(self) -> str:
        try:
            ...
            if sqlalchemy_scheme:
                return (
                    sqlalchemy_scheme.split("+")[0]
                    if "+" in sqlalchemy_scheme
    ...
    def get_reserved_words(self, dialect_name: str) -> set[str]:
        ...
        with suppress(ImportError, ModuleNotFoundError, NoSuchModuleError):
            ...
            if hasattr(dialect_module, "RESERVED_WORDS"):
                ...
                dialect_module = import_string(
                    f"sqlalchemy.dialects.{dialect_name}.reserved_words"
                )
    ...
    def get_pandas_df_by_chunks(
        ...
        **kwargs,
        ...
        return self.get_df_by_chunks(
            sql, parameters, chunksize=chunksize, df_type="pandas", **kwargs
        )
        ...
        **kwargs,
        ...
        with closing(self.get_conn()) as conn:
            ...
            return pl.read_database(
                sql, connection=conn, execute_options=execute_options, **kwargs
            )
    ...
    def get_df_by_chunks(
        ...
        **kwargs,
        ...
        """
        if df_type == "pandas":
            return self._get_pandas_df_by_chunks(
                sql, parameters, chunksize=chunksize, **kwargs
            )
        if df_type == "polars":
            return self._get_polars_df_by_chunks(
                sql, parameters, chunksize=chunksize, **kwargs
            )
    ...
    def get_records(
        ...
        """
        return self.run(
            sql=sql, parameters=parameters, handler=handlers.fetch_all_handler
        )
    ...
    def get_first(
        ...
        """
        return self.run(
            sql=sql, parameters=parameters, handler=handlers.fetch_one_handler
        )
            ...
            with closing(conn.cursor()) as cur:
                ...
                for sql_statement in sql_list:
                    ...
                    if handler is not None:
                        ...
                        if handlers.return_single_query_results(
    ...
    def _generate_insert_sql(
        self, table, values, target_fields=None, replace: bool = False, **kwargs
    ) -> str:
        """
        ...
        if replace:
            return self.dialect.generate_replace_sql(
                table, values, target_fields, **kwargs
            )
