...
class ConfigModifications:
    ...
    def add_rename(
        self, old_section: str, old_option: str, new_section: str, new_option: str
    ) -> None:
...
def _is_template(
    configuration_description: dict[str, dict[str, Any]], section: str, key: str
) -> bool:
    """
    ...
    """
    return (
        configuration_description.get(section, {})
        .get(key, {})
...
class AirflowConfigParser(ConfigParser):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._default_values = create_default_config_parser(
    ...
    def _update_logging_deprecated_template_to_one_from_defaults(self):
        ...
        if default:
            ...
            original_replacement = self.deprecated_values["logging"][
    ...
    def get_default_value(
        self, section: str, key: str, fallback: Any = None, raw=False, **kwargs
    ) -> Any:
        """
    ...
    def get_provider_config_fallback_defaults(
        self, section: str, key: str, **kwargs
    ) -> Any:
        """Get provider config fallback default values."""
        return self._provider_config_fallback_default_values.get(
    ...
    def sensitive_config_values(self) -> set[tuple[str, str]]:
        ...
        depr_option = {
            self.deprecated_options[x][:-1]
            for x in sensitive
            if x in self.deprecated_options
        }
        depr_section = {
            (self.deprecated_sections[s][0], k)
            for s, k in sensitive
            if s in self.deprecated_sections
        }
    ...
    def inversed_deprecated_options(self):
        return {
            (sec, name): key
            for key, (sec, name, ver) in self.deprecated_options.items()
        }
    ...
    def inversed_deprecated_sections(self):
        return {
            ...
            for new_section, (old_section, ver) in self.deprecated_sections.items()
        }
    ...
    enums_options = {
        ...
        ("webserver", "grid_view_sorting_order"): [
            "topological",
    ...
    def get_sections_including_defaults(self) -> list[str]:
        ...
        return list(
            dict.fromkeys(
                itertools.chain(self.configuration_description, self.sections())
            )
        )
    ...
    def get_options_including_defaults(self, section: str) -> list[str]:
        ...
        all_options_from_defaults = self.configuration_description.get(section, {}).get(
            "options", {}
        )
        return list(
            dict.fromkeys(itertools.chain(all_options_from_defaults, my_own_options))
        )
    ...
    def write_custom_config(
        ...
        for section in self._sections:  # type: ignore[attr-defined]  # accessing _sections from ConfigParser
            for option, orig_value in self._sections[section].items():  # type: ignore[attr-defined]
                ...
                if key in modifications.default_updates:
                    mod_comment += f"# Default updated from {orig_value} to {modifications.default_updates[key]}\n"
                    value = modifications.default_updates[key]

                default_value = self.get_default_value(
                    effective_section, effective_option, fallback=""
                )
        ...
        for section, options in output.items():
            ...
            if include_descriptions:
                description = self.configuration_description.get(section, {}).get(
                    "description", ""
                )
                if description:
                    for line in description.splitlines():
    ...
    def write(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        if self.configuration_description is None:
            raise RuntimeError(
                "Cannot write default config, no default configuration description set"
            )
        with self.make_sure_configuration_loaded(with_providers=include_providers):
            for section_to_write in self.get_sections_including_defaults():
                section_config_description = self.configuration_description.get(
                    section_to_write, {}
                )
                if section_to_write != section and section is not None:
                    continue
                if self._default_values.has_section(
                    section_to_write
                ) or self.has_section(section_to_write):
    ...
    def _upgrade_postgres_metastore_conn(self):
        ...
        if parsed.scheme in bad_schemes:
            ...
            new_value = re.sub(
                "^" + re.escape(f"{parsed.scheme}://"), f"{good_scheme}://", old_value
            )
    ...
    @staticmethod
    def _create_future_warning(
        name: str, section: str, current_value: Any, new_value: Any
    ):
        warnings.warn(
        ...
        if env_var_secret_path in os.environ:
            ...
            if (section, key) in self.sensitive_config_values:
                return _get_config_value_from_secret_backend(
                    os.environ[env_var_secret_path]
                )
        return None
    ...
    def get(  # type: ignore[override,misc]
        ...
        **kwargs,
        ...
        if lookup_from_deprecated:
            option_description = (
                self.configuration_description.get(section, {})
                .get("options", {})
                .get(key, {})
            )
    ...
    def getjson(
        ...
        try:
            data = self.get(
                section=section, key=key, fallback=None, _extra_stacklevel=1, **kwargs
            )
        ...
        try:
            ...
            raise AirflowConfigException(
                f"Unable to parse [{section}] {key!r} as valid json"
            ) from e
    ...
    def has_option(
        self, section: str, option: str, lookup_from_deprecated: bool = True
    ) -> bool:
        """
    ...
    def getsection(self, section: str) -> ConfigOptionsDictType | None:
        ...
        """
        if not self.has_section(section) and not self._default_values.has_section(
            section
        ):
            return None
    ...
    def as_dict(
        ...
        if include_env:
            ...
            self._filter_by_source(
                config_sources, display_source, self._get_env_var_option
            )
        ...
        if include_secret:
            self._include_secrets(
                config_sources, display_sensitive, display_source, raw
            )
        else:
            self._filter_by_source(
                config_sources, display_source, self._get_secret_option
            )
    ...
    def _include_secrets(
        ...
        for section, key in self.sensitive_config_values:
            value: str | None = self._get_secret_option_from_config_sources(
    ...
    def _include_envs(
        ...
        for env_var in [
            ...
            if os_environment.startswith(ENV_VAR_PREFIX)
        ]:
            try:
            ...
            if opt is None:
                ...
                continue
            if not display_sensitive and env_var != self._env_var_name(
                "core", "unit_test_mode"
            ):
        ...
        for config_type, config in configs:
            if config_type != "default":
                with contextlib.suppress(NoSectionError):
                    deprecated_section_array = config.items(
                        section=deprecated_section, raw=True
                    )
                    if any(
                        key == deprecated_key for key, _ in deprecated_section_array
                    ):
                        return True
        else:
            return False
    ...
    @staticmethod
    def _deprecated_variable_is_set(
        deprecated_section: str, deprecated_key: str
    ) -> bool:
        return (
            os.environ.get(
                f"{ENV_VAR_PREFIX}{deprecated_section.upper()}__{deprecated_key.upper()}"
            )
            is not None
        )
    ...
    def _deprecated_command_is_set_in_config(
        ...
        return AirflowConfigParser._deprecated_value_is_set_in_config(
            ...
            deprecated_key=deprecated_key + "_cmd",
            configs=configs,
        )

    @staticmethod
    def _deprecated_variable_command_is_set(
        deprecated_section: str, deprecated_key: str
    ) -> bool:
        return (
            os.environ.get(
                f"{ENV_VAR_PREFIX}{deprecated_section.upper()}__{deprecated_key.upper()}_CMD"
            )
            is not None
        )
    ...
    def _deprecated_secret_is_set_in_config(
        ...
        return AirflowConfigParser._deprecated_value_is_set_in_config(
            deprecated_section=deprecated_section,
            deprecated_key=deprecated_key + "_secret",
            configs=configs,
        )

    @staticmethod
    def _deprecated_variable_secret_is_set(
        deprecated_section: str, deprecated_key: str
    ) -> bool:
        return (
            os.environ.get(
                f"{ENV_VAR_PREFIX}{deprecated_section.upper()}__{deprecated_key.upper()}_SECRET"
            )
            is not None
        )
    ...
    def _replace_section_config_with_display_sources(
        ...
        if isinstance(config, AirflowConfigParser):
            with config.suppress_future_warnings():
                items: Iterable[tuple[str, Any]] = config.items(
                    section=section, raw=raw
                )
        else:
            items = config.items(section=section, raw=raw)
        for k, val in items:
            deprecated_section, deprecated_key, _ = deprecated_options.get(
                (section, k), (None, None, None)
            )
            if deprecated_section and deprecated_key:
            ...
            if display_source:
                ...
                if source_name == "default":
                    ...
                    source_description_section = configuration_description.get(
                        section, {}
                    )
                    source_description_key = source_description_section.get(
                        "options", {}
                    ).get(k, {})
                    if source_description_key is not None:
                        updated_source_name = source_description_key.get(
                            "source", source_name
                        )
    ...
    def load_test_config(self):
        ...
        unit_test_config_file = (
            pathlib.Path(__file__).parent / "config_templates" / "unit_tests.cfg"
        )
    ...
    def load_providers_configuration(self):
        ...
        for provider, config in ProvidersManager().already_initialized_provider_configs:
            for provider_section, provider_section_content in config.items():
                ...
                section_in_current_config = self.configuration_description.get(
                    provider_section
                )
                if not section_in_current_config:
                    self.configuration_description[provider_section] = deepcopy(
                        provider_section_content
                    )
                    section_in_current_config = self.configuration_description.get(
                        provider_section
                    )
                    ...
                    for option in provider_options:
                        section_in_current_config["options"][option][
                            "source"
                        ] = f"default-{provider}"
                else:
                    section_source = section_in_current_config.get(
                        "source", "Airflow's core package"
                    ).split("default-")[-1]
                    raise AirflowConfigException(
...
def write_default_airflow_configuration_if_needed() -> AirflowConfigParser:
    ...
    if not airflow_config.exists():
        log.debug(
            "Creating new Airflow config file in: %s", airflow_config.__fspath__()
        )
        ...
        if not config_directory.exists():
            ...
            log.debug(
                "Create directory %r for Airflow config", config_directory.__fspath__()
            )
        ...
        if conf.get("core", "fernet_key", fallback=None) in (None, ""):
            ...
            conf.configuration_description["core"]["options"]["fernet_key"][
                "default"
            ] = FERNET_KEY

        JWT_SECRET_KEY = b64encode(os.urandom(16)).decode("utf-8")
        conf.configuration_description["api_auth"]["options"]["jwt_secret"][
            "default"
        ] = JWT_SECRET_KEY
...
def get_custom_secret_backend(worker_mode: bool = False) -> BaseSecretsBackend | None:
    ...
    try:
        ...
        log.warning(
            "Failed to parse [%s] %s as JSON, defaulting to no kwargs.",
            section,
            kwargs_key,
        )
        backend_kwargs = {}
        ...
        log.warning(
            "Failed to parse [%s] %s into a dict, defaulting to no kwargs.",
            section,
            kwargs_key,
        )
        backend_kwargs = {}
