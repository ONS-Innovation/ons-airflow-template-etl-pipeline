...
class _TaskDecoratorRemover(cst.CSTTransformer):
    ...
    def _is_task_decorator(self, decorator_node: cst.Decorator) -> bool:
        ...
        if isinstance(decorator_expr, cst.Attribute) and isinstance(
            decorator_expr.value, cst.Name
        ):
            return (
                f"{decorator_expr.value.value}.{decorator_expr.attr.value}"
    ...
    def leave_FunctionDef(
        ...
        new_decorators = [
            dec for dec in updated_node.decorators if not self._is_task_decorator(dec)
        ]
