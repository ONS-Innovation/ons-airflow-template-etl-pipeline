...
_parse_timestamp = conf.getimport(
    "logging", "interleave_timestamp_parser", fallback=None
)
...
class FileTaskHandler(logging.Handler):
    ...
    def set_context(
        ...
        return (
            SetContextPropagate.MAINTAIN_PROPAGATE if self.maintain_propagate else None
        )
    ...
    def _render_filename(
        self, ti: TaskInstance, try_number: int, session=NEW_SESSION
    ) -> str:
        ...
        if jinja_tpl:
            return render_template(
                jinja_tpl,
                {"ti": ti, "ts": date, "try_number": try_number},
                native=False,
            )
    ...
    def _get_executor_get_task_log(
        ...
        if executor_name == self.DEFAULT_EXECUTOR_KEY:
            ...
            self.executor_instances[executor_name] = ExecutorLoader.load_executor(
                executor_name
            )
    ...
    def _get_log_retrieval_url(
        ...
        if log_type == LogType.TRIGGER:
            if not ti.triggerer_job:
                raise RuntimeError(
                    "Could not build triggerer log URL; no triggerer job."
                )
