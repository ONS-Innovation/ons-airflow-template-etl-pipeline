...
def prompt_with_timeout(
    question: str, timeout: int, default: bool | None = None
) -> bool:
...
def parse_template_string(
    template_string: str,
) -> tuple[str, None] | tuple[None, jinja2.Template]:
...
def render_log_filename(ti: TaskInstance, try_number, filename_template) -> str:
    ...
    """
    filename_template, filename_jinja_template = parse_template_string(
        filename_template
    )
...
def render_template_as_native(template: jinja2.Template, context: Context) -> Any:
    ...
    return render_template(
        template, cast("MutableMapping[str, Any]", context), native=True
    )
