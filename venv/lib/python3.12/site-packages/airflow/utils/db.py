...
def create_default_connections(session: Session = NEW_SESSION):
    ...
    merge_conn(
        Connection(
            conn_id="mongo_default", conn_type="mongo", host="mongo", port=27017
        ),
        session,
    )
    merge_conn(
    ...
    *,
    ...
    if _revision_greater(config, _REVISION_HEADS_MAP["3.0.0"], to_revision):
        ...
        if unitest_mode:
            try:
                ...
                log.warning(
                    "Import error occurred while importing FABDBManager. Skipping the check."
                )
                return
    ...
    with create_global_lock(session=session, lock=DBLocks.MIGRATIONS):
        if show_sql_only:
            ...
            _offline_migration(
                command.downgrade, config=config, revision=revision_range
            )
        else:
...
class LazySelectSequence(Sequence[T]):
    ...
    * ``_process_row`` is called when an item is accessed. The lazy sequence
    ...
    def __iter__(self) -> Iterator[T]:
        return iter(
            self._process_row(r) for r in self._session.execute(self._select_asc)
        )
