...
config_dict: dict[str, _TableConfig] = {
    x.orm_model.name: x for x in sorted(config_list)
}
    ...
    *, query: Query, orm_model: Base, skip_archive: bool, session: Session
    ...
    try:
        if dialect_name == "mysql":
            ...
            session.execute(
                text(f"CREATE TABLE {target_table_name} LIKE {orm_model.name}")
            )
...
def _subquery_keep_last(
    *,
    ...
    subquery = select(
        *group_by_columns, func.max(recency_column).label(max_date_colname)
    )
...
def run_cleanup(
    *,
    ...
    if not dry_run and confirm:
        _confirm_delete(
            date=clean_before_timestamp, tables=sorted(effective_table_names)
        )
