...
class TriggerLoggingFactory:
    ...
    def __call__(
        self, processors: Iterable[structlog.typing.Processor]
    ) -> WrappedLogger:
...
class TriggerRunnerSupervisor(WatchedSubprocess):
    ...
    health_check_threshold = conf.getint(
    ...
    decoder: ClassVar[TypeAdapter[ToTriggerSupervisor]] = TypeAdapter(
        ToTriggerSupervisor
    )
    ...
    logger_cache: dict[int, TriggerLoggingFactory] = attrs.field(
    ...
    creating_triggers: deque[workloads.RunTrigger] = attrs.field(
        factory=deque, init=False
    )

    # Outbound queue of events
    events: deque[tuple[int, events.TriggerEvent]] = attrs.field(
        factory=deque, init=False
    )

    # Outbound queue of failed triggers
    failed_triggers: deque[tuple[int, list[str] | None]] = attrs.field(
        factory=deque, init=False
    )
    ...
    def start(  # type: ignore[override]
        ...
        **kwargs,
        ...
        proc = super().start(
            id=job.id, job=job, target=cls.run_in_process, logger=logger, **kwargs
        )
    ...
    def load_triggers(self):
        ...
        Trigger.assign_unassigned(
            self.job.id, self.capacity, self.health_check_threshold
        )
    ...
    def _register_pipe_readers(
        ...
        self.selector.register(
            ...
            make_buffered_socket_reader(
                self._process_log_messages_from_subprocess(),
    ...
    def _process_log_messages_from_subprocess(self) -> Generator[None, bytes, None]:
        ...
        while True:
            ...
            if ts := event.get("timestamp"):
                ...
                # confusing.
                event["timestamp"] = msgspec.json.decode(
                    f'"{ts}"', type=datetime
                ).replace(tzinfo=None)
...
class TriggerRunner:
    ...
    async def init_comms(self):
        ...
        if not isinstance(msg, messages.StartTriggerer):
            raise RuntimeError(
                f"Required first message to be a messages.StartTriggerer, it was {msg}"
            )
        ...
        self.requests_sock = asyncio.streams.StreamWriter(
    ...
    async def create_triggers(self):
        ...
        while self.to_create:
            ...
            try:
                ...
                self.log.error(
                    "Trigger failed to load code", error=e, classpath=workload.classpath
                )
    ...
    async def sync_state_to_supervisor(self, finished_ids: list[int]):
        ...
        async with SUPERVISOR_COMMS.lock:
            ...
            self.requests_sock.write(
                msg.model_dump_json(exclude_none=True).encode() + b"\n"
            )
            line = await self.response_sock.readline()
        ...
        if not isinstance(resp, messages.TriggerStateSync):
            raise RuntimeError(
                f"Expected to get a TriggerStateSync message, instead we got f{type(msg)}"
            )
