...
class SchedulerDagBag:
    ...
    def _get_dag(self, version_id: str, session: Session) -> DAG | None:
        if dag := self._dags.get(version_id):
            return dag
        dag_version = session.get(
            DagVersion, version_id, options=[joinedload(DagVersion.serialized_dag)]
        )
        if not dag_version:
            return None
    ...
    def _version_from_dag_run(dag_run, session):
        if dag_run.bundle_version:
            ...
            dag_version = DagVersion.get_latest_version(
                dag_id=dag_run.dag_id, session=session
            )
        return dag_version
...
class SchedulerJobRunner(BaseJobRunner, LoggingMixin):
    ...
    def __init__(
        ...
        scheduler_idle_sleep_time: float = conf.getfloat(
    ...
    def _debug_dump(self, signum: int, frame: FrameType | None) -> None:
        ...
        for threadId, stack in sys._current_frames().items():
            self.log.info(
                "Stack Trace for Scheduler Job Runner on thread: %s", id2name[threadId]
            )
    ...
    def _executable_task_instances_to_queued(
        ...
        starved_pools = {
            pool_name for pool_name, stats in pools.items() if stats["open"] <= 0
        }
        ...
        for loop_count in itertools.count(start=1):
            ...
            num_starved_tasks_task_dagrun_concurrency = len(
            ...
            if starved_tasks_task_dagrun_concurrency:
                query = query.where(
                    tuple_(TI.dag_id, TI.run_id, TI.task_id).not_in(
                        starved_tasks_task_dagrun_concurrency
                    )
                )
        ...
        for pool_name, num_starving_tasks in pool_num_starving_tasks.items():
            ...
            Stats.gauge(
                "pool.starving_tasks", num_starving_tasks, tags={"pool_name": pool_name}
            )
        ...
        for ti in task_instances:
            ...
            if executor.queue_workload.__func__ is not BaseExecutor.queue_workload:  # type: ignore[attr-defined]
                workload = workloads.ExecuteTask.make(
                    ti, generator=executor.jwt_generator
                )
            ...
            queue = ti.queue
            self.log.info(
                "Sending %s to %s with priority %s and queue %s",
                ti.key,
                executor.name,
                priority,
                queue,
    ...
    def _critical_section_enqueue_task_instances(self, session: Session) -> int:
        ...
        num_occupied_slots = sum(
            [executor.slots_occupied for executor in self.job.executors]
        )
        ...
        if max_tis <= 0:
            self.log.debug(
                "max_tis query size is less than or equal to zero. No query will be performed!"
            )
            return 0
        ...
        for executor, queued_tis_per_executor in executor_to_queued_tis.items():
            ...
            self._enqueue_task_instances_with_queued_state(
                queued_tis_per_executor, executor, session=session
            )
    ...
    def process_executor_events(
        ...
        job_id: str | None,
        scheduler_dag_bag: SchedulerDagBag,
        session: Session,
    ) -> int:
        """
        ...
        for ti_key, (state, _) in event_buffer.items():
            ...
            cls.logger().info(
                "Received executor event with state %s for task instance %s",
                state,
                ti_key,
            )
            if state in (
        ...
        for ti in tis:
            ...
            ti_requeued = (
                ti.queued_by_job_id
                != job_id  # Another scheduler has queued this task again
            ...
            if ti_queued and not ti_requeued:
                ...
                session.add(
                    Log(event="state mismatch", extra=msg, task_instance=ti.key)
                )
    ...
    def set_ti_span_attrs(cls, span, state, ti):
        ...
        if span.is_recording():
            span.add_event(
                name="airflow.task.queued", timestamp=datetime_to_nano(ti.queued_dttm)
            )
            span.add_event(
                name="airflow.task.started", timestamp=datetime_to_nano(ti.start_date)
            )
            span.add_event(
                name="airflow.task.ended", timestamp=datetime_to_nano(ti.end_date)
            )
        ...
        try:
            ...
            for executor in self.job.executors:
                try:
                    ...
                    self.log.exception(
                        "Exception when executing Executor.end on %s", executor
                    )
    ...
    def _update_dag_run_state_for_paused_dags(
        ...
        try:
            ...
            for dag_run in paused_runs:
                ...
                if dag is not None:
                    dag_run.dag = dag
                    _, callback_to_run = dag_run.update_state(
                        execute_callbacks=False, session=session
                    )
                    if callback_to_run:
                        self._send_dag_callbacks_to_processor(dag, callback_to_run)
        except Exception as e:  # should not fail the scheduler
            self.log.exception(
                "Failed to update dag run state for paused dags due to %s", e
            )
    ...
    def _end_active_spans(self, session: Session = NEW_SESSION):
        ...
        for key, span in self.active_spans.get_all().items():
            ...
            if isinstance(key, TaskInstanceKey):  # ti span.
                ...
                dag_run: DagRun = session.scalars(
                    select(DagRun).where(DagRun.run_id == key)
                ).one()
    ...
    def _end_spans_of_externally_ended_ops(self, session: Session):
        ...
        tis_should_end: list[TaskInstance] = session.scalars(
            select(TaskInstance).where(
                TaskInstance.span_status == SpanStatus.SHOULD_END
            )
        ).all()
        ...
        if (
            ...
            if not job.is_alive():
                ...
                tis_needing_spans = [
                    ...
                    and self.active_spans.get(ti.key) is None
                ]
        ...
        for loop_count in itertools.count(start=1):
            with (
                ...
                for executor in self.job.executors:
                    try:
                        with create_session() as session:
                            self._process_task_event_logs(
                                executor._task_event_logs, session
                            )
                    except Exception:
                        self.log.exception(
            ...
            if loop_count >= self.num_runs > 0:
                ...
                if span.is_recording():
                    span.add_event(
                        "Exiting scheduler loop as requested number of runs has been reached"
                    )
                break
    ...
    def _do_scheduling(self, session: Session) -> int:
        ...
        with prohibit_commit(session) as guard:
            ...
            total_free_executor_slots = sum(
                [executor.slots_available for executor in self.job.executors]
            )
            if total_free_executor_slots <= 0:
                ...
                try:
                    ...
                    if is_lock_not_available_error(error=e):
                        self.log.debug(
                            "Critical section lock held by another Scheduler"
                        )
    ...
    def _create_dagruns_for_dags(
        ...
        asset_triggered_dags = [
            dag
            for dag in all_dags_needing_dag_runs
            if dag.dag_id in triggered_date_by_dag
        ]
    ...
    def _should_update_dag_next_dagruns(
        ...
        *,
        ...
        if last_dag_run and not (
            ...
            and last_dag_run.run_type == DagRunType.SCHEDULED
        ):
            return False
    ...
    def _start_queued_dagruns(self, session: Session) -> None:
        ...
        active_runs_of_dags = Counter(
            {(dag_id, br_id): num for dag_id, br_id, num in session.execute(query)}
        )
    ...
    def _schedule_all_dag_runs(
        ...
        callback_tuples = [
            (run, self._schedule_dag_run(run, session=session)) for run in dag_runs
        ]
        guard.commit()
    ...
    def _schedule_dag_run(
        ...
        with Trace.start_span(
            ...
            dag = dag_run.dag = self.scheduler_dag_bag.get_dag(
            ...
            if (
                ...
                session.flush()
                self.log.info(
                    "Run %s of %s has timed-out", dag_run.run_id, dag_run.dag_id
                )

                if self._should_update_dag_next_dagruns(
                    dag, dag_model, last_dag_run=dag_run, session=session
                ):
                    dag_model.calculate_dagrun_date_fields(
                        dag, dag.get_run_data_interval(dag_run)
                    )
                ...
                Stats.timing(
                    "dagrun.duration.failed", duration, tags={"dag_id": dag_run.dag_id}
                )
            ...
            if (
                not dag_run.bundle_version
                and not self._verify_integrity_if_dag_changed(
                    dag_run=dag_run, session=session
                )
            ):
                self.log.warning(
                    "The DAG disappeared before verifying integrity: %s. Skipping.",
                    dag_run.dag_id,
                )
                return callback
            ...
            dag_run.schedule_tis(
                schedulable_tis, session, max_tis_per_query=self.job.max_tis_per_query
            )
    ...
    def _send_dag_callbacks_to_processor(
        self, dag: DAG, callback: DagCallbackRequest | None = None
    ) -> None:
        if callback:
    ...
    def _get_tis_stuck_in_queued(self, session) -> Iterable[TaskInstance]:
        ...
        return session.scalars(
            select(TI).where(
                ...
                TI.queued_dttm
                < (timezone.utcnow() - timedelta(seconds=self._task_queued_timeout)),
    ...
    def _maybe_requeue_stuck_ti(self, *, ti, session):
        ...
        if num_times_stuck < self._num_stuck_queued_retries:
            self.log.info(
                "Task stuck in queued; will try to requeue. task_id=%s", ti.task_id
            )
            session.add(
                Log(
    ...
    @provide_session
    def _get_num_times_stuck_in_queued(
        self, ti: TaskInstance, session: Session = NEW_SESSION
    ) -> int:
        """
    ...
    def _emit_running_ti_metrics(self, session: Session = NEW_SESSION) -> None:
        ...
        ti_running_metrics = {
            (row.dag_id, row.task_id, row.queue): row.running_count for row in running
        }
    ...
    def _emit_pool_metrics(self, session: Session = NEW_SESSION) -> None:
        ...
        with Trace.start_span(
            span_name="emit_pool_metrics", component="SchedulerJobRunner"
        ) as span:
            ...
            for pool_name, slot_stats in pools.items():
                ...
                Stats.gauge(
                    "pool.open_slots", slot_stats["open"], tags={"pool_name": pool_name}
                )
                Stats.gauge(
                    "pool.queued_slots",
                    slot_stats["queued"],
    ...
    def adopt_or_reset_orphaned_tasks(self, session: Session = NEW_SESSION) -> int:
        ...
        for attempt in run_with_db_retries(logger=self.log):
            with attempt:
                ...
                self.log.debug(
                    "Calling SchedulerJob.adopt_or_reset_orphaned_tasks method"
                )
                try:
        ...
        for attempt in run_with_db_retries(max_retries, logger=self.log):
            with attempt:
                ...
                if num_timed_out_tasks:
                    self.log.info(
                        "Timed out %i deferred tasks without fired triggers",
                        num_timed_out_tasks,
                    )
    ...
    def _update_asset_orphanage(self, session: Session = NEW_SESSION) -> None:
        ...
        asset_orphanation: dict[bool, Collection[AssetModel]] = {
            ...
            for orphaned, group in itertools.groupby(
                asset_reference_query, key=operator.itemgetter(0)
            )
        }
        self._orphan_unreferenced_assets(
            asset_orphanation.get(True, ()), session=session
        )
        self._activate_referenced_assets(
            asset_orphanation.get(False, ()), session=session
        )

    @staticmethod
    def _orphan_unreferenced_assets(
        assets: Collection[AssetModel], *, session: Session
    ) -> None:
        if assets:
            session.execute(
    ...
    @staticmethod
    def _activate_referenced_assets(
        assets: Collection[AssetModel], *, session: Session
    ) -> None:
        if not assets:
            return
        ...
        def _generate_warning_message(
            ...
            for ref in itertools.chain(
                offending.consuming_dags, offending.producing_tasks
            ):
                yield (
                    ref.dag_id,
                    (
        ...
        def _activate_assets_generate_warnings() -> Iterator[tuple[str, str]]:
            ...
            for asset in assets:
                if (asset.name, asset.uri) in active_assets:
                    continue
                existing_uri = active_name_to_uri.get(
                    asset.name
                ) or incoming_name_to_uri.get(asset.name)
                if existing_uri is not None and existing_uri != asset.uri:
                    yield from _generate_warning_message(asset, "name", existing_uri)
                    continue
                existing_name = active_uri_to_name.get(
                    asset.uri
                ) or incoming_uri_to_name.get(asset.uri)
    ...
    def _executor_to_tis(
        ...
        _executor_to_tis: defaultdict[BaseExecutor, list[TaskInstance]] = defaultdict(
            list
        )
        for ti in tis:
    ...
    def _try_to_load_executor(self, executor_name: str | None) -> BaseExecutor | None:
        ...
        self.log.warning(
            "Executor, %s, was not found but a Task was configured to use it",
            executor_name,
        )
        return None
