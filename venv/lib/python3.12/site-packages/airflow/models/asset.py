...
def fetch_active_assets_by_name(
    names: Iterable[str], session: Session
) -> dict[str, Asset]:
    return {
...
def fetch_active_assets_by_uri(
    uris: Iterable[str], session: Session
) -> dict[str, Asset]:
    return {
...
def expand_alias_to_assets(
    alias_name: str, *, session: Session
) -> Iterable[AssetModel]:
...
asset_trigger_association_table = Table(
    ...
    Column("asset_id", ForeignKey("asset.id", ondelete="CASCADE"), primary_key=True),
    Column(
        "trigger_id", ForeignKey("trigger.id", ondelete="CASCADE"), primary_key=True
    ),
...
class AssetModel(Base):
    ...
    triggers = relationship(
        "Trigger", secondary=asset_trigger_association_table, back_populates="assets"
    )
...
association_table = Table(
    ...
    Base.metadata,
    Column(
        "dag_run_id", ForeignKey("dag_run.id", ondelete="CASCADE"), primary_key=True
    ),
    Column(
        "event_id", ForeignKey("asset_event.id", ondelete="CASCADE"), primary_key=True
    ),
