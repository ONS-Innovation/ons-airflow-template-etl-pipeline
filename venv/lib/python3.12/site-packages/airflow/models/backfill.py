...
def _validate_backfill_params(
    dag, reverse, from_date, to_date, reprocess_behavior: ReprocessBehavior | None
):
...
def _do_dry_run(
    *, dag_id, from_date, to_date, reverse, reprocess_behavior, session
) -> list[datetime]:
    ...
    no_schedule = session.scalar(
        select(func.count()).where(
            DagModel.timetable_summary == "None", DagModel.dag_id == dag_id
        )
    )
    if no_schedule:
    ...
    for info in dagrun_info_list:
        dr = session.scalar(
            statement=_get_latest_dag_run_row_query(
                dag_id=dag_id, info=info, session=session
            ),
        )
        if dr:
...
def _create_backfill_dag_run(
    *,
    ...
    with session.begin_nested() as nested:
        dr = session.scalar(
            _get_latest_dag_run_row_query(dag_id=dag.dag_id, info=info, session=session)
        )
        if dr:
            non_create_reason = _get_dag_run_no_create_reason(dr, reprocess_behavior)
