...
class DagRun(Base, LoggingMixin):
    ...
    created_dag_version_id = Column(
        ...
        ForeignKey(
            "dag_version.id", name="created_dag_version_id_fkey", ondelete="set null"
        ),
        nullable=True,
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "dag_id", "logical_date", name="dag_run_dag_id_logical_date_key"
        ),
    ...
    created_dag_version = relationship(
        "DagVersion", uselist=False, passive_deletes=True
    )
    """
    ...
    def dag_versions(self) -> list[DagVersion]:
        ...
        dag_versions = [
            dv
            for dv in dict.fromkeys(
                list(self._tih_dag_versions) + list(self._ti_dag_versions)
            )
            if dv is not None
        ]
    ...
    @provide_session
    def check_version_id_exists_in_dr(
        self, dag_version_id: UUIDType, session: Session = NEW_SESSION
    ):
        select_stmt = (
            ...
            .union(
                select(TIH.dag_version_id).where(
                    TIH.dag_id == self.dag_id,
                    TIH.dag_version_id == dag_version_id,
    ...
    def refresh_from_db(self, session: Session = NEW_SESSION) -> None:
        ...
        dr = session.scalars(
            select(DagRun).where(
                DagRun.dag_id == self.dag_id, DagRun.run_id == self.run_id
            )
        ).one()
        self.id = dr.id
        self.state = dr.state
    ...
    def get_running_dag_runs_to_examine(cls, session: Session) -> Query:
        ...
        query = (
            select(cls)
            .with_hint(
                cls, "USE INDEX (idx_dag_run_running_dags)", dialect_name="mysql"
            )
    ...
    def get_queued_dag_runs_to_set_running(cls, session: Session) -> Query:
        ...
        query = (
            ...
            .order_by(
                ...
                nulls_first(cls.last_scheduling_decision, session=session),
                nulls_first(
                    running_drs.c.num_running, session=session
    ...
    def find(
        ...
        if logical_start_date and logical_end_date:
            qry = qry.where(
                cls.logical_date.between(logical_start_date, logical_end_date)
            )
    ...
    @provide_session
    def find_duplicate(
        cls, dag_id: str, run_id: str, *, session: Session = NEW_SESSION
    ) -> DagRun | None:
        """
        ...
        *None* is returned if no such DAG run is found.
        ...
        """
        return session.scalars(
            select(cls).where(cls.dag_id == dag_id, cls.run_id == run_id)
        ).one_or_none()
    ...
    def generate_run_id(
        *,
        ...
        return DagRunType(run_type).generate_run_id(
            suffix=f"{run_after.isoformat()}_{get_random_string()}"
        )
            ...
            if isinstance(state, str):
                ...
                if None in state:
                    if all(x is None for x in state):
                        ...
                        tis = tis.where(
                            or_(TI.state.in_(not_none_state), TI.state.is_(None))
                        )
                else:
    ...
    def _check_last_n_dagruns_failed(
        self, dag_id, max_consecutive_failed_dag_runs, session
    ):
    ...
    def fetch_task_instance(
        ...
        return session.scalars(
            select(TI).filter_by(
                dag_id=dag_id, run_id=dag_run_id, task_id=task_id, map_index=map_index
            )
        ).one_or_none()
    ...
    def get_previous_dagrun(
        ...
        return session.scalar(
            select(DagRun).where(*filters).order_by(DagRun.logical_date.desc()).limit(1)
        )
    ...
    def set_dagrun_span_attrs(self, span: Span | EmptySpan):
        ...
        if span.is_recording():
            ...
            span.add_event(
                name="airflow.dag_run.ended", timestamp=datetime_to_nano(self.end_date)
            )
    ...
    def update_state(
        ...
        class _UnfinishedStates(NamedTuple):
            ...
            def recalculate(self) -> _UnfinishedStates:
                return self._replace(
                    tis=[t for t in self.tis if t.state in State.unfinished]
                )
        ...
        if not unfinished.tis and any(
            x.state in State.failed_states for x in tis_for_dagrun_state
        ):
            ...
            if dag.max_consecutive_failed_dag_runs > 0:
                ...
                self._check_last_n_dagruns_failed(
                    dag.dag_id, dag.max_consecutive_failed_dag_runs, session
                )

        # if all leaves succeeded and no unfinished tasks, the run succeeded
        elif not unfinished.tis and all(
            x.state in State.success_states for x in tis_for_dagrun_state
        ):
        ...
        # if *all tasks* are deadlocked, the run failed
            ...
            self.log.error(
                "Task deadlock (no runnable tasks); marking run %s failed", self
            )
            self.set_state(DagRunState.FAILED)
            ...
            if execute_callbacks and dag.has_on_failure_callback:
                self.handle_dag_callback(
                    dag=dag, success=False, reason="all_tasks_deadlocked"
                )
            elif dag.has_on_failure_callback:
                callback = DagCallbackRequest(
                    filepath=self.dag_model.relative_fileloc,
    ...
    def task_instance_scheduling_decisions(
        ...
        def _filter_tis_and_exclude_removed(dag: DAG, tis: list[TI]) -> Iterable[TI]:
            ...
            for ti in tis:
                try:
                    ...
                    if ti.state != TaskInstanceState.REMOVED:
                        self.log.error(
                            "Failed to get task for ti %s. Marking it as removed.", ti
                        )
        ...
        if unfinished_tis:
            schedulable_tis = [
                ut for ut in unfinished_tis if ut.state in SCHEDULEABLE_STATES
            ]
            self.log.debug(
            ...
            if expansion_happened:
                changed_tis = True
                new_unfinished_tis = [
                    t for t in unfinished_tis if t.state in State.unfinished
                ]
                finished_tis.extend(
                    t for t in unfinished_tis if t.state in State.finished
                )
    ...
    def handle_dag_callback(
        self, dag: SDKDAG, success: bool = True, reason: str = "success"
    ):
        ...
        if not callbacks:
            self.log.warning(
                "Callback requested, but dag didn't have any for DAG: %s.", dag.dag_id
            )
            return
    ...
    def _get_ready_tis(
        ...
        def _expand_mapped_task_if_needed(ti: TI) -> Iterable[TI] | None:
            ...
            try:
                expanded_tis, _ = TaskMap.expand_mapped_task(
                    ti.task, self.run_id, session=session
                )
        ...
        for schedulable in itertools.chain(schedulable_tis, additional_tis):
            ...
            if not schedulable.are_dependencies_met(
                session=session, dep_context=dep_context
            ):
                ...
                if schedulable.task.task_id not in revised_map_index_task_ids:
                    ready_tis.extend(
                        self._revise_map_indexes_if_mapped(
                            schedulable.task, session=session
                        )
                    )
        ...
        try:
            ...
            try:
                first_start_date = min(
                    ti.start_date for ti in finished_tis if ti.start_date
                )
            ...
            self.log.warning(
                "Failed to record first_task_scheduling_delay metric:", exc_info=True
            )
    ...
    def verify_integrity(
        ...
        hook_is_noop: Literal[True, False] = getattr(
        ...
        def task_filter(task: Operator) -> bool:
            return task.task_id not in task_ids and (
                ...
                and (
                    ...
                    or self.logical_date is None
                    or self.logical_date <= task.end_date
                )
            )
        ...
        tis_to_create = self._create_tasks(
            tasks_to_create, task_creator, session=session
        )
        ...
        for ti in tis:
            ...
            try:
                ...
                if should_restore_task:
                    ...
                    Stats.incr(
                        f"task_restored_to_dag.{dag.dag_id}", tags=self.stats_tags
                    )
                ...
                if ti.state == TaskInstanceState.REMOVED:
                    ...
                    Stats.incr(
                        f"task_removed_from_dag.{dag.dag_id}", tags=self.stats_tags
                    )
                    # Same metric with tagging
                    Stats.incr(
            ...
            try:
                ...
                try:
                    total_length = BaseOperator.get_mapped_ti_count(
    ...
    def _get_task_creator(
        ...
        if hook_is_noop:
            ...
            def create_ti_mapping(
                task: Operator, indexes: Iterable[int]
    ...
    def _create_tasks(
        ...
        *,
        ...
        for task in tasks:
            try:
                count = BaseOperator.get_mapped_ti_count(
                    task, self.run_id, session=session
                )
        ...
        *,
        ...
        try:
            ...
            for task_type, count in created_counts.items():
                Stats.incr(
                    f"task_instance_created_{task_type}", count, tags=self.stats_tags
                )
                # Same metric with tagging
                Stats.incr(
    ...
    def _revise_map_indexes_if_mapped(
        self, task: Operator, *, session: Session
    ) -> Iterator[TI]:
        """
    ...
    def get_latest_runs(cls, session: Session = NEW_SESSION) -> list[DagRun]:
        ...
        return session.scalars(
            select(cls).join(
                subquery,
                and_(
                    cls.dag_id == subquery.c.dag_id,
                    cls.logical_date == subquery.c.logical_date,
                ),
            )
        ).all()
    ...
    def schedule_tis(
        ...
        for ti in schedulable_tis:
            ...
            # if not, we'll add this "ti" into "schedulable_ti_ids" and later execute it to run in the worker
                ...
                start_from_trigger = ti.task.expand_start_from_trigger(
        ...
        if empty_ti_ids:
            dummy_ti_ids_chunks = chunks(
                empty_ti_ids, max_tis_per_query or len(empty_ti_ids)
            )
    ...
    def get_log_template(self, *, session: Session = NEW_SESSION) -> LogTemplate:
        return DagRun._get_log_template(
            log_template_id=self.log_template_id, session=session
        )
    ...
    def _get_log_template(
        ...
        if log_template_id is None:  # DagRun created before LogTemplate introduction.
            template = session.scalar(
                select(LogTemplate).order_by(LogTemplate.id).limit(1)
            )
        else:
...
class DagRunNote(Base):
    ...
    def __repr__(self):
        prefix = (
            f"<{self.__class__.__name__}: {self.dag_id}.{self.dagrun_id} {self.run_id}"
        )
        if self.map_index != -1:
