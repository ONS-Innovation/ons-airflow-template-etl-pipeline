...
class DagCode(Base):
    ...
    @provide_session
    def write_code(
        cls, dag_version: DagVersion, fileloc: str, session: Session = NEW_SESSION
    ) -> DagCode:
        """
    ...
    def has_dag(cls, dag_id: str, session: Session = NEW_SESSION) -> bool:
        ...
        return (
            session.scalars(
                select(literal(True)).where(cls.dag_id == dag_id).limit(1)
            ).one_or_none()
            is not None
        )
    ...
    def update_source_code(
        cls, dag_id: str, fileloc: str, session: Session = NEW_SESSION
    ) -> None:
        """
