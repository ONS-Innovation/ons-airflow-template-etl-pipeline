...
class Trigger(Base):
    ...
    task_instance = relationship(
        "TaskInstance", back_populates="trigger", lazy="selectin", uselist=False
    )

    assets = relationship(
        "AssetModel",
        secondary=asset_trigger_association_table,
    ...
    def encrypt_kwargs(kwargs: dict[str, Any]) -> str:
        ...
        return (
            get_fernet()
            .encrypt(json.dumps(serialized_kwargs).encode("utf-8"))
            .decode("utf-8")
        )
    ...
    def rotate_fernet_key(self):
        ...
        self.encrypted_kwargs = (
            get_fernet().rotate(self.encrypted_kwargs.encode("utf-8")).decode("utf-8")
        )
    ...
    @provide_session
    def submit_event(
        cls, trigger_id, event: events.TriggerEvent, session: Session = NEW_SESSION
    ) -> None:
        """
        Fire an event.
    ...
    @provide_session
    def submit_failure(
        cls, trigger_id, exc=None, session: Session = NEW_SESSION
    ) -> None:
        """
    ...
    def ids_for_triggerer(
        cls, triggerer_id, session: Session = NEW_SESSION
    ) -> list[int]:
    ...
    def assign_unassigned(
        ...
        count = session.scalar(
            select(func.count(cls.id)).filter(cls.triggerer_id == triggerer_id)
        )
        capacity -= count
        ...
        alive_triggerer_ids = select(Job.id).where(
            ...
            > timezone.utcnow() - datetime.timedelta(seconds=health_check_threshold),
    ...
    def get_sorted_triggers(
        ...
        query = with_row_locks(
            ...
            .order_by(
                coalesce(TaskInstance.priority_weight, 0).desc(), cls.created_date
            )
            .limit(capacity),
            session,
            skip_locked=True,
...
def handle_event_submit(
    event: events.TriggerEvent, *, task_instance: TaskInstance, session: Session
) -> None:
    """
...
def _process_BaseTaskEndEvent(
    ...
    def _submit_callback_if_necessary() -> None:
        ...
        if event.task_instance_state in (
