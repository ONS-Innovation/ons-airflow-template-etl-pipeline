...
class SchedulerDictOfListsExpandInput:
    ...
    def _iter_parse_time_resolved_kwargs(self) -> Iterable[tuple[str, Sized]]:
        ...
        return (
            (k, v) for k, v in self.value.items() if not _needs_run_time_resolution(v)
        )
...
SchedulerExpandInput = Union[
    SchedulerDictOfListsExpandInput, SchedulerListOfDictsExpandInput
]
