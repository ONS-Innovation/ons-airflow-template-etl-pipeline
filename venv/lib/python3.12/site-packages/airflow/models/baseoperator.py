...
class BaseOperator(TaskSDKBaseOperator):
    ...
    def run(
        ...
        for info in self.dag.iter_dagrun_infos_between(
    ...
    if TYPE_CHECKING:
        ...
        @classmethod
        def get_mapped_ti_count(
            cls, task: DAGNode, run_id: str, *, session: Session
        ) -> int:
        ...
        @classmethod
        def _(
            cls, task: TaskSDKAbstractOperator, run_id: str, *, session: Session
        ) -> int:
            ...
            if not hasattr(exp_input, "get_total_map_length"):
                exp_input = _ExpandInputRef(
                    ...
                    BaseSerialization.deserialize(
                        BaseSerialization.serialize(exp_input.value)
                    ),
                )
                exp_input = exp_input.deref(task.dag)
