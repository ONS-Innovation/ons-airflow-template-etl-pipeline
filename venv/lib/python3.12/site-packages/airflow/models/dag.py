...
class InconsistentDataInterval(AirflowException):
    ...
    def __str__(self) -> str:
        return self._template.format(
            cls=self._class_name, start=self._start_field, end=self._end_field
        )
    ...
    return {
        ...
        for x in session.execute(
            select(
                ...
                case((func.count() == 1, func.max(AssetModel.uri)), else_="").label(
                    "uri"
                ),
                func.count().label("total"),
                func.sum(case((ADRQ.target_dag_id.is_not(None), 1), else_=0)).label(
                    "ready"
                ),
            )
            .join(
...
def _create_orm_dagrun(
    *,
    ...
    run.verify_integrity(
        session=session, dag_version_id=dag_version.id if dag_version else None
    )
    return run
...
if TYPE_CHECKING:
    ...
    def dag(dag_id: str = "", **kwargs):
        return task_sdk_dag_decorator(
            dag_id, __DAG_class=DAG, __warnings_stacklevel_delta=3, **kwargs
        )
...
class DAG(TaskSDKDag, LoggingMixin):
    ...
    def infer_automated_data_interval(self, logical_date: datetime) -> DataInterval:
        ...
        if issubclass(
            timetable_type, (NullTimetable, OnceTimetable, AssetTriggeredTimetable)
        ):
    ...
    @provide_session
    def has_dag_runs(
        self, session=NEW_SESSION, include_manually_triggered=True
    ) -> bool:
        return (
            get_last_dagrun(
                self.dag_id,
    ...
    def get_is_active(self, session=NEW_SESSION) -> None:
        ...
        return session.scalar(
            select(~DagModel.is_stale).where(DagModel.dag_id == self.dag_id)
        )
    ...
    def get_bundle_name(self, session=NEW_SESSION) -> str | None:
        ...
        return session.scalar(
            select(DagModel.bundle_name).where(DagModel.dag_id == self.dag_id)
        )
    ...
    @provide_session
    def fetch_dagrun(
        dag_id: str, run_id: str, session: Session = NEW_SESSION
    ) -> DagRun:
        """
    ...
    def get_latest_logical_date(
        ...
        return session.scalar(
            select(func.max(DagRun.logical_date)).where(DagRun.dag_id == self.dag_id)
        )
    ...
    def get_task_instances(
        ...
        return session.scalars(
            cast("Select", query).order_by(DagRun.logical_date)
        ).all()
    ...
    def set_task_instance_state(
        ...
        *,
        ...
        if (
            not future and not past
        ):  # Simple case 1: we're only dealing with exactly one run.
            ...
            exclude_run_id_stmt = select(DagRun.run_id).where(
            ...
            if future:
                ...
                exclude_run_id_stmt = exclude_run_id_stmt.where(DagRun.id < dr_id)
            subset.clear(
                exclude_run_ids=frozenset(session.scalars(exclude_run_id_stmt)),
                **clear_kwargs,
            )
        return altered
    ...
    def set_task_group_state(
        ...
        *,
        ...
        def get_logical_date() -> datetime:
            stmt = select(DagRun.logical_date).where(
                DagRun.run_id == run_id, DagRun.dag_id == self.dag_id
            )
    ...
    def bulk_write_to_db(
        ...
        orm_dags = dag_op.find_orm_dags(
            session=session
        )  # Refetch so relationship is up to date.
        asset_op.add_dag_asset_references(orm_dags, orm_assets, session=session)
        asset_op.add_dag_asset_alias_references(
            orm_dags, orm_asset_aliases, session=session
        )
    ...
    def deactivate_unknown_dags(active_dag_ids, session=NEW_SESSION):
        ...
        for dag in session.scalars(
            select(DagModel).where(~DagModel.dag_id.in_(active_dag_ids))
        ).all():
            dag.is_stale = True
            session.merge(dag)
        session.commit()
    ...
    def deactivate_stale_dags(expiration_date, session=NEW_SESSION):
        ...
        for dag in session.scalars(
            select(DagModel).where(
                DagModel.last_parsed_time < expiration_date, ~DagModel.is_stale
            )
        ):
            log.info(
    ...
    def get_num_task_instances(
        dag_id, run_id=None, task_ids=None, states=None, session=NEW_SESSION
    ) -> int:
        """
    ...
    def from_sdk_dag(cls, dag: TaskSDKDag) -> DAG:
        ...
        def create_tasks(task):
            ...
            new_task.task_group = (
                task_group_map.get(task.task_group.group_id)
                if task.task_group
                else None
            )
...
class DagModel(Base):
    ...
    is_paused_at_creation = airflow_conf.getboolean(
    ...
    _dag_display_property_value = Column(
    ...
    def __init__(self, **kwargs):
        ...
        if self.max_active_tasks is None:
            self.max_active_tasks = airflow_conf.getint(
                "core", "max_active_tasks_per_dag"
            )

        if self.max_active_runs is None:
            self.max_active_runs = airflow_conf.getint(
    ...
    def dag_display_name(self) -> str:
        ...
        return case(
            (
                self._dag_display_property_value.is_not(None),
    ...
    def deactivate_deleted_dags(
        ...
        """
        log.debug(
            "Deactivating DAGs (for which DAG files are deleted) from %s table ",
            cls.__tablename__,
        )
    ...
    def dags_needing_dagruns(
        ...
        def dag_ready(
            dag_id: str, cond: BaseAsset, statuses: dict[AssetUniqueKey, bool]
        ) -> bool | None:
            ...
            try:
                ...
                log.warning(
                    "dag '%s' has old serialization; skipping DAG run creation.", dag_id
                )
                return None
        ...
        ser_dags = SerializedDagModel.get_latest_serialized_dags(
            dag_ids=list(dag_statuses), session=session
        )
        for ser_dag in ser_dags:
            dag_id = ser_dag.dag_id
            statuses = dag_statuses[dag_id]
            if not dag_ready(
                dag_id, cond=ser_dag.dag.timetable.asset_condition, statuses=statuses
            ):
        ...
        triggered_date_by_dag: dict[str, datetime] = {
            dag_id: max(adrq.created_at for adrq in adrqs)
            for dag_id, adrqs in adrq_by_dag.items()
        }
        del adrq_by_dag
        ...
        if asset_triggered_dag_ids:
            ...
            if exclusion_list:
                ...
                triggered_date_by_dag = {
                    k: v
                    for k, v in triggered_date_by_dag.items()
    ...
    def calculate_dagrun_date_fields(
        ...
        if next_dagrun_info is None:
            self.next_dagrun_data_interval = (
                self.next_dagrun
    ...
    def get_asset_triggered_next_run_info(
        ...
        return get_asset_triggered_next_run_info([self.dag_id], session=session).get(
            self.dag_id, None
        )
...
def _get_or_create_dagrun(
    *,
    ...
    dr: DagRun = session.scalar(
        select(DagRun).where(
            DagRun.dag_id == dag.dag_id, DagRun.logical_date == logical_date
        )
    )
    if dr:
        session.delete(dr)
