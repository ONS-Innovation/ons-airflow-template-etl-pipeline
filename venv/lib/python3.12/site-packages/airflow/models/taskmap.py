...
class TaskMap(TaskInstanceDependencies):
    ...
    def expand_mapped_task(
        cls, task, run_id: str, *, session: Session
        ...
        try:
            total_length: int | None = DBBaseOperator.get_mapped_ti_count(
                task, run_id, session=session
            )
        ...
        if unmapped_ti:
            ...
            if total_length is None:
                ...
                if not zero_index_ti_exists:
                    ...
                    task.log.debug(
                        "Deleting the original task instance: %s", unmapped_ti
                    )
        ...
        query = with_row_locks(
            query, of=TaskInstance, session=session, skip_locked=True
        )
