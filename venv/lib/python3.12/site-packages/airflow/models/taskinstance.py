...
def _stop_remaining_tasks(
    *, task_instance: TaskInstance, task_teardown_map=None, session: Session
):
    """
    ...
    for ti in tis:
        ...
        if not teardown:
            if ti.state == TaskInstanceState.RUNNING:
                log.info(
                    "Forcing task %s to fail due to dag's `fail_fast` setting",
                    ti.task_id,
                )
                ...
                log.info(
                    "Setting task %s to SKIPPED due to dag's `fail_fast` setting.",
                    ti.task_id,
                )
        ...
        def render(key: str, content: str) -> str:
            if conf.has_option("email", key):
                ...
                try:
                    ...
                    log.warning(
                        "Could not find email template file '%s'. Using defaults...",
                        path,
                    )
                except OSError:
                    log.exception(
...
class TaskInstance(Base, LoggingMixin):
    ...
    dag_version_id = Column(
        UUIDType(binary=False), ForeignKey("dag_version.id", ondelete="CASCADE")
    )
    ...
    def refresh_from_db(
        ...
        if source:
            ...
            if target_state is None:
                raise RuntimeError(
                    f"Unable to inspect SQLAlchemy state of {type(self)}: {self}"
                )
            ...
            for attr_name, col in target_state.mapper.columns.items():
                if (
                    keep_local_changes
                    and target_state.attrs[attr_name].history.has_changes()
                ):
                    continue
    ...
    def refresh_from_task(
        self, task: Operator, pool_override: str | None = None
    ) -> None:
        """
    ...
    def is_premature(self) -> bool:
        ...
        return (
            self.state == TaskInstanceState.UP_FOR_RETRY and not self.ready_for_retry()
        )
    ...
    def get_previous_dagrun(
        ...
        if dag.catchup is True and not ignore_schedule:
            ...
            last_dagrun = DagRun.get_previous_dagrun(
                dag_run=dr, session=session, state=state
            )
    ...
    def are_dependencies_met(
        ...
        verbose_aware_logger(
            "Dependencies all met for dep_context=%s ti=%s",
            dep_context.description,
            self,
        )
        return True
    ...
    def _check_and_change_state_before_execution(
        ...
        if verbose:
            if mark_success:
                cls.logger().info(
                    "Marking success for %s on %s", ti.task, ti.logical_date
                )
            else:
    ...
    def emit_state_change_metric(self, new_state: TaskInstanceState) -> None:
        ...
        if new_state == TaskInstanceState.RUNNING:
            ...
            raise NotImplementedError(
                "no metric emission setup for state %s", new_state
            )
    ...
    def register_asset_changes_in_db(
        ...
        asset_name_refs = {
            Asset.ref(name=o.name)
            for o in task_outlets
            if o.type == AssetNameRef.__name__ and o.name
        }
        asset_uri_refs = {
            Asset.ref(uri=o.uri)
            for o in task_outlets
            if o.type == AssetUriRef.__name__ and o.uri
        }
                ...
                select(AssetModel).where(
                    ...
                    or_(
                        tuple_(AssetModel.name, AssetModel.uri).in_(
                            attrs.astuple(k) for k in asset_keys
                        ),
        ...
        if asset_name_refs:
            ...
            asset_event_extras_by_name = {
                key.name: extra for key, extra in asset_event_extras.items()
            }
        ...
        if asset_uri_refs:
            ...
            asset_event_extras_by_uri = {
                key.uri: extra for key, extra in asset_event_extras.items()
            }
        ...
        def _asset_event_extras_from_aliases() -> (
            dict[tuple[AssetUniqueKey, frozenset], set[str]]
        ):
            d = defaultdict(set)
            for event in outlet_events:
                try:
        ...
        outlet_alias_names = {
            o.name for o in task_outlets if o.type == AssetAlias.__name__ and o.name
        }
        if outlet_alias_names and (
            event_extras_from_aliases := _asset_event_extras_from_aliases()
        ):
            for (
                asset_key,
                extra_key,
                ...
                ti.log.debug(
                    "register event for asset %s with aliases %s",
                    asset_key,
                    event_aliase_names,
                )
    ...
    def update_rtif(self, rendered_fields, session: Session = NEW_SESSION):
        ...
        rtif = RenderedTaskInstanceFields(
            ti=self, render_templates=False, rendered_fields=rendered_fields
        )
        RenderedTaskInstanceFields.write(rtif, session=session)
        session.flush()
        RenderedTaskInstanceFields.delete_old_records(
            self.task_id, self.dag_id, session=session
        )
    ...
    def defer_task(
        ...
        if exception is not None:
            ...
            start_trigger_args = self.task.expand_start_trigger_args(
    ...
    def handle_failure(
        ...
        if (
            ...
            try:
                ...
                log.exception(
                    "Failed to send email to: %s", failure_context["task"].email
                )
    ...
    def get_template_context(
        ...
        def _get_previous_dagrun_success() -> PrevSuccessfulDagRunResponse:
            dr_from_db = self.get_previous_dagrun(
                state=DagRunState.SUCCESS, session=session
            )
            if dr_from_db:
                return PrevSuccessfulDagRunResponse.model_validate(
                    dr_from_db, from_attributes=True
                )
        ...
        def get_prev_data_interval_start_success() -> pendulum.DateTime | None:
            return timezone.coerce_datetime(
                _get_previous_dagrun_success().data_interval_start
            )

        def get_prev_data_interval_end_success() -> pendulum.DateTime | None:
            return timezone.coerce_datetime(
                _get_previous_dagrun_success().data_interval_end
            )
        ...
        context.update(
            {
                ...
                "task_instance": self,
                "ti": self,
                "triggering_asset_events": lazy_object_proxy.Proxy(
    ...
    def email_alert(self, exception, task: BaseOperator) -> None:
        ...
        """
        subject, html_content, html_content_err = self.get_email_subject_content(
            exception, task=task
        )
        if TYPE_CHECKING:
            assert task.email
    ...
    def get_relevant_upstream_map_indexes(
        ...
        *,
        ...
        ancestor_ti_count = BaseOperator.get_mapped_ti_count(
            common_ancestor, self.run_id, session=session
        )
            ...
            return query.values(
                {
                    ...
                    "duration": (
                        (
                            func.strftime("%s", end_date)
                            - func.strftime("%s", cls.start_date)
                        )
                        + func.round(
                            (
                                func.strftime("%f", end_date)
                                - func.strftime("%f", cls.start_date)
                            ),
                            3,
                        )
                    ),
...
class TaskInstanceNote(Base):
    ...
    task_instance = relationship(
        "TaskInstance", back_populates="task_instance_note", uselist=False
    )
