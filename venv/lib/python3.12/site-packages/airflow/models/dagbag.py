...
class DagBag(LoggingMixin):
        ...
        with zipfile.ZipFile(filepath) as current_zip_file:
            for zip_info in current_zip_file.infolist():
                ...
                if not might_contain_dag(
                    zip_info.filename, safe_mode, current_zip_file
                ):
    ...
    def _process_modules(self, filepath, mods, file_last_changed_on_disk):
        ...
        for dag, mod in top_level_dags:
            ...
            if self.bundle_path:
                dag.relative_fileloc = str(
                    Path(mod.__file__).relative_to(self.bundle_path)
                )
            else:
        ...
        **Note**: The patterns in ``.airflowignore`` are interpreted as either
        ...
        if include_examples:
            ...
            files_to_parse.extend(
                list_py_file_paths(example_dag_folder, safe_mode=safe_mode)
            )
