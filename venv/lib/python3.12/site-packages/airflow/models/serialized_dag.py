...
class _DagDependenciesResolver:
    ...
    def __init__(
        self, dag_id_dependencies: Sequence[tuple[str, dict]], session: Session
    ) -> None:
    ...
    def resolve(self) -> dict[str, list[DagDependency]]:
        (
            ...
            asset_ref_uris,
            asset_alias_names,
        ) = self.collect_asset_info()

        self.asset_key_to_id = self.collect_asset_key_to_ids(asset_names_uris)
        self.asset_ref_name_to_asset_id_name = self.collect_asset_name_ref_to_ids_names(
            asset_ref_names
        )
        self.asset_ref_uri_to_asset_id_name = self.collect_asset_uri_ref_to_ids_names(
            asset_ref_uris
        )
        self.alias_names_to_asset_ids_names = self.collect_alias_to_assets(
            asset_alias_names
        )
                ...
                if dep_type == "asset":
                    ...
                    for node_key in ("source", "target"):
                        if dep_data[node_key].startswith("asset:"):
                            unique_key = AssetUniqueKey.from_str(
                                dep_data[node_key].split(":")[1]
                            )
    ...
    def collect_asset_key_to_ids(
        self, asset_name_uris: set[tuple[str, str]]
    ) -> dict[AssetUniqueKey, int]:
        return {
    ...
    def collect_asset_name_ref_to_ids_names(
        self, asset_ref_names
    ...
    def collect_asset_uri_ref_to_ids_names(
        self, asset_ref_uris
    ) -> dict[str, tuple[int, str]]:
        return {
            uri: (asset_id, name)
    ...
    def resolve_asset_name_ref_dag_dep(self, dep_data) -> Iterator[DagDependency]:
        return self.resolve_asset_ref_dag_dep(
            dep_data=dep_data, ref_type="asset-name-ref"
        )

    def resolve_asset_uri_ref_dag_dep(self, dep_data: dict) -> Iterator[DagDependency]:
        return self.resolve_asset_ref_dag_dep(
            dep_data=dep_data, ref_type="asset-uri-ref"
        )
...
class SerializedDagModel(Base):
    ...
    * ``[core] compress_serialized_dags``:
    ...
    def _sort_serialized_dag_dict(cls, serialized_dag: Any):
        ...
        if isinstance(serialized_dag, list):
            if all(isinstance(i, dict) for i in serialized_dag):
                if all(
                    isinstance(i.get("__var", {}), Iterable)
                    and "task_id" in i.get("__var", {})
    ...
    def write_dag(
        ...
        if (
            serialized_dag_hash is not None
            and serialized_dag_hash == new_serialized_dag.dag_hash
        ):
            log.debug(
        ...
        if dag_version and not dag_version.task_instances:
            ...
            DagCode.update_source_code(
                dag_id=dag.dag_id, fileloc=dag.fileloc, session=session
            )
            return True
    ...
    def data(self) -> dict | None:
        ...
        if (
            not hasattr(self, "_SerializedDagModel__data_cache")
            or self.__data_cache is None
        ):
    ...
    def get_dag_dependencies(
        ...
        if COMPRESS_SERIALIZED_DAGS is False:
            if session.bind.dialect.name in ["sqlite", "mysql"]:
                data_col_to_select = func.json_extract(
                    cls._data, "$.dag.dag_dependencies"
                )
                ...
                data_col_to_select = func.json_extract_path(
                    cls._data, "dag", "dag_dependencies"
                )
                load_json = None
        else:
            ...
            def load_json(deps_data):
                return (
                    json.loads(zlib.decompress(deps_data))["dag"]["dag_dependencies"]
                    if deps_data
                    else []
                )

        latest_sdag_subquery = (
            select(cls.dag_id, func.max(cls.created_at).label("max_created"))
            .group_by(cls.dag_id)
            .subquery()
        )
        ...
        resolver = _DagDependenciesResolver(
            dag_id_dependencies=iterator, session=session
        )
