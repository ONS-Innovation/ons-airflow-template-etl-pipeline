...
def load_plugins_from_plugin_directory():
    ...
    if conf.getboolean("core", "LOAD_EXAMPLES"):
        ...
        example_files = find_path_from_directory(
            example_plugins_folder, ".airflowignore"
        )
...
def ensure_plugins_loaded():
    ...
    if plugins:
        log.debug(
            "Loading %d plugin(s) took %.2f seconds", len(plugins), timer.duration
        )
...
def integrate_macros_plugins() -> None:
    ...
    for plugin in plugins:
        ...
        macros_module = make_module(
            f"airflow.sdk.execution_time.macros.{plugin.name}", plugin.macros
        )
            ...
            for attr in attrs_to_dump:
                if attr in ("global_operator_extra_links", "operator_extra_links"):
                    ...
                    info[attr] = [
                        {
                            **d,
                            "app": qualname(d["app"].__class__) if "app" in d else None,
                        }
                        for d in getattr(plugin, attr)
                    ]
                    ...
                    info[attr] = [
                        {
                            k: (
                                v
                                if k != "middleware"
                                else qualname(middleware_dict["middleware"])
                            )
