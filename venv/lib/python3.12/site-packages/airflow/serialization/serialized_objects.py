...
def _get_default_mapped_partial() -> dict[str, Any]:
    ...
    default = (
        BaseOperator.partial(task_id="_")
        ._expand(EXPAND_INPUT_EMPTY, strict=False)
        .partial_kwargs
    )
        ...
        watchers=[
            SerializedAssetWatcher(
                ...
                trigger={
                    ...
                    "kwargs": _smart_decode_trigger_kwargs(
...
def encode_outlet_event_accessor(var: OutletEventAccessor) -> dict[str, Any]:
    ...
    return {
        ...
        "extra": var.extra,
        "asset_alias_events": [
            attrs.asdict(cast("attrs.AttrsInstance", e)) for e in var.asset_alias_events
        ],
    }
...
def encode_outlet_event_accessors(var: OutletEventAccessors) -> dict[str, Any]:
    return {
        ...
        "_dict": [
            {
                "key": BaseSerialization.serialize(k),
                "value": encode_outlet_event_accessor(v),
            }
...
def decode_outlet_event_accessors(var: dict[str, Any]) -> OutletEventAccessors:
    ...
    d._dict = {  # type: ignore[attr-defined]
        BaseSerialization.deserialize(row["key"]): decode_outlet_event_accessor(
            row["value"]
        )
...
def encode_priority_weight_strategy(var: PriorityWeightStrategy) -> str:
    ...
    if priority_weight_strategy_class in airflow_priority_weight_strategies_classes:
        return airflow_priority_weight_strategies_classes[
...
class _ExpandInputRef(NamedTuple):
    ...
    def deref(self, dag: DAG) -> SchedulerExpandInput:
        ...
        if isinstance(self.value, _XComRef):
            ...
            value = {
                k: v.deref(dag) if isinstance(v, _XComRef) else v
...
class BaseSerialization:
    ...
    def serialize(
        ...
        if cls._is_primitive(var):
            ...
            return cls._encode(
                SerializedBaseOperator.serialize_mapped_operator(var), type_=DAT.OP
            )
        elif isinstance(var, TaskSDKBaseOperator):
            var._needs_expansion = var.get_needs_expansion()
            return cls._encode(
                SerializedBaseOperator.serialize_operator(var), type_=DAT.OP
            )
        ...
        elif isinstance(var, (AirflowException, TaskDeferred)) and hasattr(
            var, "serialize"
        ):
    ...
    def deserialize(cls, encoded_var: Any) -> Any:
        ...
        if type_ == DAT.TASK_CONTEXT:
            ...
            d["task"] = d[
                "task_instance"
            ].task  # todo: add `_encode` of Operator so we don't need this
            d["macros"] = macros
            d["var"] = {
            ...
            return _XComRef(
                var
            )  # Delay deserializing XComArg objects until we have the entire DAG.
    ...
    @classmethod
    def _value_is_hardcoded_default(
        cls, attrname: str, value: Any, instance: Any
    ) -> bool:
        """
        ...
        if attrs.has(type(instance)):
            return any(
                fld.default is value
                for fld in attrs.fields(type(instance))
    ...
    def _deserialize_param(cls, param_dict: dict):
        ...
        def is_serialized(val):
            ...
            if isinstance(val, list):
                return all(
                    isinstance(item, dict) and Encoding.TYPE in item for item in val
                )
            return False
    ...
    def _deserialize_params_dict(
        cls, encoded_params: list[tuple[str, dict]]
    ) -> ParamsDict:
...
class DependencyDetector:
    ...
    def detect_task_dependencies(task: Operator) -> list[DagDependency]:
        ...
        if isinstance(task, TriggerDagRunOperator):
            ...
            and issubclass(
                cast("type[BaseOperator]", task.operator_class), TriggerDagRunOperator
            )
            ...
            isinstance(task, MappedOperator)
            and issubclass(
                cast("type[BaseOperator]", task.operator_class), ExternalTaskSensor
            )
    ...
    def detect_dag_dependencies(dag: DAG | None) -> Iterable[DagDependency]:
        ...
        if not dag:
            return
        yield from dag.timetable.asset_condition.iter_dag_dependencies(
            source="", target=dag.dag_id
        )
...
class SerializedBaseOperator(BaseOperator, BaseSerialization):
    ...
    def extra_links(self) -> list[str]:
        return sorted(
            set(self.operator_extra_link_dict).union(
                self.global_operator_extra_link_dict
            )
        )
    ...
    def get_extra_links(self, ti: TaskInstance, name: str) -> str | None:
        ...
        """
        link = self.operator_extra_link_dict.get(
            name
        ) or self.global_operator_extra_link_dict.get(name)
        if not link:
            return None
    ...
    def serialize_operator(
        cls, op: TaskSDKBaseOperator | MappedOperator
    ) -> dict[str, Any]:
    ...
    def populate_operator(cls, op: Operator, encoded_op: dict[str, Any]) -> None:
        ...
        for k, v in encoded_op.items():
            ...
            if k == "downstream_task_ids":
                ...
                if v is False:
                    raise RuntimeError(
                        "_is_sensor=False should never have been serialized!"
                    )
        ...
        setattr(
            op, "start_from_trigger", bool(encoded_op.get("start_from_trigger", False))
        )
    ...
    def deserialize_operator(cls, encoded_op: dict[str, Any]) -> Operator:
        ...
        if encoded_op.get("_is_mapped", False):
            ...
            op_data = {
                k: v
                for k, v in encoded_op.items()
                if k in TaskSDKBaseOperator.get_serialized_fields()
            }
    ...
    def serialize_dag(cls, dag: DAG) -> dict:
        ...
        try:
            ...
            serialized_dag["tasks"] = [
                cls.serialize(task) for _, task in dag.task_dict.items()
            ]
            ...
            serialized_dag["task_group"] = TaskGroupSerialization.serialize_task_group(
                dag.task_group
            )
    ...
    def conversion_v1_to_v2(ser_obj: dict):
        ...
        def _replace_dataset_with_asset_in_timetables(obj, parent_key=None):
            if isinstance(obj, dict):
                ...
                for k, v in obj.items():
                    ...
                    if new_key == "uri":
                        ...
                        new_value = (
                            ...
                            if isinstance(v, str)
                            else _replace_dataset_with_asset_in_timetables(
                                v, parent_key=new_key
                            )
                        )
                ...
                return [
                    _replace_dataset_with_asset_in_timetables(i, parent_key=parent_key)
                    for i in obj
                ]
        ...
        if timetable := dag_dict.get("timetable"):
            if timetable["__type"] in {
                ...
                dag_dict["timetable"] = _replace_dataset_with_asset_in_timetables(
                    dag_dict["timetable"]
                )
            ...
            dag_dict["timetable"] = _create_compat_timetable(
                datetime.timedelta(seconds=sched["__var"])
            )
        elif sched.get("__type") == "relativedelta":
            dag_dict["timetable"] = _create_compat_timetable(
                decode_relativedelta(sched["__var"])
            )
        else:
        ...
        for task in dag_dict["tasks"]:
            ...
            for item in itertools.chain(
                *(task_var.get(key, []) for key in ("inlets", "outlets"))
            ):
...
class TaskGroupSerialization(BaseSerialization):
    ...
    def serialize_task_group(cls, task_group: TaskGroup) -> dict[str, Any] | None:
        ...
        encoded = {
            ...
            "downstream_group_ids": cls.serialize(
                sorted(task_group.downstream_group_ids)
            ),
            "upstream_task_ids": cls.serialize(sorted(task_group.upstream_task_ids)),
            "downstream_task_ids": cls.serialize(
                sorted(task_group.downstream_task_ids)
            ),
        }
    ...
    def deserialize_task_group(
        ...
        kwargs["group_display_name"] = cls.deserialize(
            encoded_group.get("group_display_name", "")
        )

        if not encoded_group.get("is_mapped"):
            group = TaskGroup(
                group_id=group_id, parent_group=parent_group, dag=dag, **kwargs
            )
        else:
            ...
            group = MappedTaskGroup(
                ...
                dag=dag,
                expand_input=_ExpandInputRef(
                    xi["type"], cls.deserialize(xi["value"])
                ).deref(dag),
                **kwargs,
            )
        ...
        group.upstream_group_ids.update(
            cls.deserialize(encoded_group["upstream_group_ids"])
        )
        group.downstream_group_ids.update(
            cls.deserialize(encoded_group["downstream_group_ids"])
        )
        group.upstream_task_ids.update(
            cls.deserialize(encoded_group["upstream_task_ids"])
        )
        group.downstream_task_ids.update(
            cls.deserialize(encoded_group["downstream_task_ids"])
        )
        return group
...
class LazyDeserializedDAG(pydantic.BaseModel):
    ...
    def access_control(
        self,
    ) -> Mapping[str, Mapping[str, Collection[str]] | Collection[str]] | None:
    ...
    def has_task_concurrency_limits(self) -> bool:
        return any(
            ...
            or task[Encoding.VAR]
            .get("partial_kwargs", {})
            .get("max_active_tis_per_dag")
            is not None
            or task[Encoding.VAR]
            .get("partial_kwargs", {})
            .get("max_active_tis_per_dagrun")
            is not None
...
class XComOperatorLink(LoggingMixin):
    ...
    def get_link(self, operator: BaseOperator, *, ti_key: TaskInstanceKey) -> str:
        ...
        """
        self.log.info(
            "Attempting to retrieve link from XComs with key: %s for task id: %s",
            self.xcom_key,
            ti_key,
        )
