...
class BaseExecutor(LoggingMixin):
    ...
    def log_task_event(self, *, event: str, extra: str, ti_key: TaskInstanceKey):
        ...
        self._task_event_logs.append(
            Log(event=event, task_instance=ti_key, extra=extra)
        )
        ...
        for _ in range(min((open_slots, len(self.queued_tasks)))):
            ...
            if key in self.running:
                ...
                if isinstance(item, workloads.ExecuteTask) and hasattr(item, "ti"):
                    ...
                    if (
                        self.active_spans is not None
                        and self.active_spans.get(key) is None
                    ):
                ...
                if hasattr(self, "_process_workloads"):
                    ...
                    task_tuples.append(
                        (key, command, queue, getattr(ti, "executor_config", None))
                    )
    ...
    def _get_parser(cls) -> argparse.ArgumentParser:
        ...
        subparsers = parser.add_subparsers(
            dest="subcommand", metavar="GROUP_OR_COMMAND"
        )
