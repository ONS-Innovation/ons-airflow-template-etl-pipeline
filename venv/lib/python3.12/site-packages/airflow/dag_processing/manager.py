...
def _resolve_path(
    instance: Any, attribute: attrs.Attribute, val: str | os.PathLike[str] | None
):
    if val is not None:
...
class DagFileProcessorManager(LoggingMixin):
    ...
    _parallelism: int = attrs.field(
        factory=_config_int_factory("dag_processor", "parsing_processes")
    )
    ...
    _processors: dict[DagFileInfo, DagFileProcessorProcess] = attrs.field(
    ...
    _api_server: InProcessExecutionAPI = attrs.field(
        init=False, factory=InProcessExecutionAPI
    )
    ...
    def run(self):
        ...
        self.log.info(
            "Processing files using up to %s processes at a time ", self._parallelism
        )
        self.log.info(
        ...
        if self.bundle_names_to_parse:
            dag_bundles = [
                b for b in dag_bundles if b.name in self.bundle_names_to_parse
            ]
        self._dag_bundles = dag_bundles

        for bundle in self._dag_bundles:
            self.log.info(
                "Checking for new files in bundle %s every %s seconds",
                bundle.name,
    ...
    def deactivate_stale_dags(
        ...
        for dag in dags_parsed:
            ...
            file_info = DagFileInfo(
                rel_path=Path(dag.relative_fileloc), bundle_name=dag.bundle_name
            )
            if last_finish_time := last_parsed.get(file_info, None):
                if (
                    dag.last_parsed_time + timedelta(seconds=self.stale_dag_threshold)
                    < last_finish_time
                ):
                    self.log.info(
                        "DAG %s is missing and will be deactivated.", dag.dag_id
                    )
    ...
    def _run_parsing_loop(self):
        ...
        while True:
            ...
            if self.max_runs_reached():
                self.log.info(
                    "Exiting dag parsing loop as all files have been processed %s times",
                    self.max_runs,
                )
                break
    ...
    def _queue_requested_files_for_parsing(self) -> None:
        ...
        if self._force_refresh_bundles:
            self.log.info(
                "Bundles being force refreshed: %s",
    ...
    def _fetch_callbacks(
        ...
        with prohibit_commit(session) as guard:
            ...
            query = query.order_by(DbCallbackRequest.priority_weight.asc()).limit(
                self.max_callbacks_per_loop
            )
            query = with_row_locks(
                query, of=DbCallbackRequest, session=session, skip_locked=True
            )
            ...
            for callback in callbacks:
                try:
                    ...
                    self.log.warning(
                        "Error adding callback for execution: %s, %s", callback, e
                    )
            guard.commit()
    ...
    def _add_callback_to_queue(self, request: CallbackRequest):
        self.log.debug(
            "Queuing %s CallbackRequest: %s", type(request).__name__, request
        )
        try:
            bundle = DagBundlesManager().get_bundle(
                name=request.bundle_name, version=request.bundle_version
            )
        except ValueError:
            # Bundle no longer configured
            self.log.error(
                "Bundle %s no longer configured, skipping callback", request.bundle_name
            )
            return None
        ...
        for bundle in self._dag_bundles:
            ...
            if not bundle.is_initialized:
                try:
                    ...
                    self.log.exception(
                        "Error initializing bundle %s: %s", bundle.name, e
                    )
                    continue
            ...
            with create_session() as session:
                ...
                if bundle.supports_versioning:
                    ...
                    current_version_matches_db = (
                        pre_refresh_version == bundle_model.version
                    )
                else:
            ...
            found_files = {
                DagFileInfo(
                    rel_path=p, bundle_name=bundle.name, bundle_path=bundle.path
                )
            ...
            self.clear_orphaned_import_errors(
                bundle_name=bundle.name,
                observed_filelocs={
                    str(x.absolute_path) for x in found_files
                },  # todo: make relative
            )
    ...
    def _find_files_in_bundle(self, bundle: BaseDagBundle) -> list[Path]:
        ...
        rel_paths = [
            Path(x).relative_to(bundle.path) for x in list_py_file_paths(bundle.path)
        ]
    ...
    def deactivate_deleted_dags(
        self, bundle_name: str, present: set[DagFileInfo]
    ) -> None:
        ...
        def find_zipped_dags(abs_path: os.PathLike) -> Iterator[str]:
            ...
            try:
                ...
                self.log.exception(
                    "There was an error accessing ZIP file %s %s", abs_path
                )
        ...
        for files in known_files.values():
            for file in files:
                ...
                if last_run:
                    ...
                    Stats.gauge(
                        f"dag_processing.last_run.seconds_ago.{file_name}", seconds_ago
                    )
        ...
        for file in list(self._processors.keys()):
            if file not in present:
                ...
                Stats.decr(
                    ...
                    tags={"file_path": file, "action": "stop"},
                )
    ...
    def client(self) -> Client:
        ...
        client = Client(
            base_url=None, token="", dry_run=True, transport=self._api_server.transport
        )
    ...
    def _start_new_processes(self):
        ...
        while self._parallelism > len(self._processors) and self._file_queue:
            ...
            Stats.incr(
                "dag_processing.processes", tags={"file_path": file, "action": "start"}
            )
    ...
    def _sort_by_mtime(self, files: Iterable[DagFileInfo]):
        ...
        for file in files:
            try:
                ...
                modified_datetime = datetime.fromtimestamp(
                    modified_timestamp, tz=timezone.utc
                )
        ...
        file_infos = [
            info
            for info, ts in sorted(
                files_with_mtime.items(), key=itemgetter(1), reverse=True
            )
        ]
    ...
    def prepare_file_queue(self, known_files: dict[str, set[DagFileInfo]]):
        ...
        at_run_limit = [
            info
            for info, stat in self._file_stats.items()
    ...
    def _kill_timed_out_processors(self):
        ...
        for file, processor in self._processors.items():
            ...
            if duration > self.processor_timeout:
                ...
                Stats.decr(
                    "dag_processing.processes",
                    tags={"file_path": file, "action": "timeout"},
                )
                Stats.incr(
                    "dag_processing.processor_timeouts", tags={"file_path": file}
                )
                processor.kill(signal.SIGKILL)
    ...
    def max_runs_reached(self):
        ...
        if self._num_run < self.max_runs:
            return False
        return all(
            stat.run_count >= self.max_runs for stat in self._file_stats.values()
        )
    ...
    def terminate(self):
        ...
        for file, processor in self._processors.items():
            ...
            Stats.decr(
                "dag_processing.processes",
                tags={"file_path": file, "action": "terminate"},
            )
    ...
    def emit_metrics(self):
        ...
        with Trace.start_span(
            span_name="emit_metrics", component="DagFileProcessorManager"
        ) as span:
            parse_time = time.perf_counter() - self._parsing_start_time
            Stats.gauge("dag_processing.total_parse_time", parse_time)
            Stats.gauge(
                "dagbag_size", sum(stat.num_dags for stat in self._file_stats.values())
            )
            Stats.gauge(
            ...
            span.set_attributes(
                {
                    ...
                    "dag_bag_size": sum(
                        stat.num_dags for stat in self._file_stats.values()
                    ),
                    "import_errors": sum(
                        stat.import_errors for stat in self._file_stats.values()
                    ),
                }
            )
