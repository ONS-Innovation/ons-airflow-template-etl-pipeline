...
def _get_latest_runs_stmt(dag_ids: Collection[str]) -> Select:
    ...
    if (
        ...
        last_automated_runs_subq = (
            select(
                DagRun.dag_id, func.max(DagRun.logical_date).label("max_logical_date")
            )
            .where(
...
class _RunInfo(NamedTuple):
    ...
    @classmethod
    def calculate(
        cls, dags: dict[str, MaybeSerializedDAG], *, session: Session
    ) -> Self:
        """
        ...
        latest_runs = {
            run.dag_id: run
            for run in session.scalars(_get_latest_runs_stmt(dag_ids=dags.keys()))
        }
...
def _update_dag_tags(tag_names: set[str], dm: DagModel, *, session: Session) -> None:
    ...
    dm.tags.extend(
        DagTag(name=name, dag_id=dm.dag_id) for name in tag_names.difference(orm_tags)
    )


def _update_dag_owner_links(
    dag_owner_links: dict[str, str], dm: DagModel, *, session: Session
) -> None:
...
def _serialize_dag_capturing_errors(
    ...
    try:
        ...
        log.exception(
            "Failed to write serialized DAG dag_id=%s fileloc=%s",
            dag.dag_id,
            dag.fileloc,
        )
...
def update_dag_parsing_results_in_db(
    ...
    *,
    ...
    try:
        ...
        good_dag_filelocs = {
            dag.fileloc for dag in dags if dag.fileloc not in import_errors
        }
    ...
    try:
        _update_dag_warnings(
            [dag.dag_id for dag in dags], warnings, warning_types, session
        )
    except Exception:
...
class DagModelOperation(NamedTuple):
        ...
        *,
        ...
        for dag_id, dm in sorted(orm_dags.items()):
            ...
            if dag.max_consecutive_failed_dag_runs is not None:
                ...
                dag.max_consecutive_failed_dag_runs is None
                and dm.max_consecutive_failed_dag_runs is None
            ):
            ...
            if last_automated_run is None:
                ...
                last_automated_data_interval = dag.get_run_data_interval(
                    last_automated_run
                )
    ...
    def update_dag_asset_expression(
        ...
        *,
        ...
        def expand_asset_expr(
            asset_expr: dict[str, list | dict]
        ) -> dict[str, list | dict]:
        ...
        for dag_id, dm in sorted(orm_dags.items()):
            asset_expression = self.dags[
                dag_id
            ].timetable.asset_condition.as_expression()
...
def _find_active_assets(
    name_uri_assets: Iterable[tuple[str, str]], session: Session
) -> set[tuple[str, str]]:
    return set(
        session.execute(
            select(AssetModel.name, AssetModel.uri).where(
                ...
                AssetModel.consuming_dags.any(
                    DagScheduleAssetReference.dag.has(
                        ~DagModel.is_stale & ~DagModel.is_paused
                    )
                ),
            )
        )
...
class AssetModelOperation(NamedTuple):
    ...
    def collect(cls, dags: dict[str, MaybeSerializedDAG]) -> Self:
        coll = cls(
            schedule_asset_references={
                dag_id: [
                    asset for _, asset in dag.timetable.asset_condition.iter_assets()
                ]
            ...
            outlet_references={
                dag_id: list(dag.get_task_assets(inlets=False, outlets=True))
                for dag_id, dag in dags.items()
            },
            assets={
                (asset.name, asset.uri): asset
                for asset in _find_all_assets(dags.values())
            },
            asset_aliases={
                alias.name: alias for alias in _find_all_asset_aliases(dags.values())
            },
        )
        return coll
        ...
        orm_assets: dict[tuple[str, str], AssetModel] = {
            ...
            for am in session.scalars(
                select(AssetModel).where(
                    tuple_(AssetModel.name, AssetModel.uri).in_(self.assets)
                )
            )
        }
    ...
    def sync_asset_aliases(self, *, session: Session) -> dict[str, AssetAliasModel]:
        ...
        orm_aliases: dict[str, AssetAliasModel] = {
            ...
            for da in session.scalars(
                select(AssetAliasModel).where(
                    AssetAliasModel.name.in_(self.asset_aliases)
                )
            )
        }
        ...
        orm_aliases.update(
            ...
            for model in asset_manager.create_asset_aliases(
                [
                    alias
                    for name, alias in self.asset_aliases.items()
                    if name not in orm_aliases
                ],
                session=session,
            )
        )
    ...
    def activate_assets_if_possible(
        self, models: Iterable[AssetModel], *, session: Session
    ) -> None:
        """
        ...
        *,
        ...
        for dag_id, references in self.schedule_asset_references.items():
            ...
            referenced_asset_ids = {
                asset.id for asset in (assets[r.name, r.uri] for r in references)
            }
        ...
        *,
        ...
        for dag_id, references in self.schedule_asset_alias_references.items():
            ...
            if not references:
                ...
                continue
            referenced_alias_ids = {
                alias.id for alias in (aliases[r.name] for r in references)
            }
            orm_refs = {
                a.alias_id: a for a in dags[dag_id].schedule_asset_alias_references
            }
    ...
    def _add_dag_asset_references(
        ...
        *,
        ...
        if old_refs:
            session.execute(
                delete(model).where(
                    tuple_(model.dag_id, getattr(model, attr)).in_(old_refs)
                )
            )
        if new_refs:
            session.execute(
                insert(model), [{"dag_id": d, attr: r} for d, r in new_refs]
            )
    ...
    def add_task_asset_references(
        ...
        *,
        ...
        for dag_id, references in self.outlet_references.items():
            ...
            referenced_outlets = {
                (task_id, asset.id)
                for task_id, asset in (
                    (task_id, assets[d.name, d.uri]) for task_id, d in references
                )
            }
            orm_refs = {
                (r.task_id, r.asset_id): r
                for r in dags[dag_id].task_outlet_asset_references
            }
            ...
            session.bulk_save_objects(
                TaskOutletAssetReference(
                    asset_id=asset_id, dag_id=dag_id, task_id=task_id
                )
    ...
    def add_asset_trigger_references(
        ...
        active_assets = (
            _find_active_assets(self.assets, session=session) if self.assets else {}
        )
            ...
            orm_triggers: dict[int, Trigger] = {
                ...
                for trigger in session.scalars(
                    select(Trigger).where(
                        tuple_(Trigger.classpath, Trigger.encrypted_kwargs).in_(
                            all_trigger_keys
                        )
                    )
                )
        ...
        if refs_to_remove:
            ...
            for name_uri, trigger_hashes in refs_to_remove.items():
                ...
                asset_model.triggers = [
                    trigger
                    for trigger in asset_model.triggers
                    if BaseEventTrigger.hash(trigger.classpath, trigger.kwargs)
        ...
        orphan_assets = session.scalars(
            select(AssetModel)
            .filter(~AssetModel.consuming_dags.any())
            .filter(AssetModel.triggers.any())
        )
