...
def _parse_file_entrypoint():
    ...
    if not isinstance(msg, DagFileParseRequest):
        raise RuntimeError(
            f"Required first message to be a DagFileParseRequest, it was {msg}"
        )
...
def _parse_file(
    msg: DagFileParseRequest, log: FilteringBoundLogger
...
def _serialize_dags(
    bag: DagBag, log: FilteringBoundLogger
...
def _execute_dag_callbacks(
    dagbag: DagBag, request: DagCallbackRequest, log: FilteringBoundLogger
) -> None:
    ...
    if not callbacks:
        log.warning(
            "Callback requested, but dag didn't have any", dag_id=request.dag_id
        )
        return
