...
def _bundle_item_exc(msg):
    return AirflowConfigException(
        "Invalid config for section `dag_processor` key `dag_bundle_config_list`. "
        + msg
    )
...
def _validate_bundle_config(config_list):
    ...
    for item in config_list:
        ...
        if not actual_keys == expected_keys:
            raise _bundle_item_exc(
                f"Expected keys {expected_keys} but found {actual_keys}"
            )
    ...
    if len(all_names) != len(set(all_names)):
        raise _bundle_item_exc(
            f"One or more bundle names appeared multiple times: {all_names}"
        )
...
class DagBundlesManager(LoggingMixin):
    ...
    def sync_bundles_to_db(self, *, session: Session = NEW_SESSION) -> None:
        ...
        for name, bundle in stored.items():
            ...
            self.log.warning(
                "DAG bundle %s is no longer found in config and has been disabled", name
            )
