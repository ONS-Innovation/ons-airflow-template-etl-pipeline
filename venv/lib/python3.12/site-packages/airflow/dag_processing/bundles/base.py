...
def get_bundle_storage_root_path():
    if configured_location := conf.get(
        "dag_processor", "dag_bundle_storage_path", fallback=None
    ):
...
class BundleUsageTrackingManager:
    ...
    @staticmethod
    def _filter_for_min_versions(
        val: list[TrackedBundleVersionInfo],
    ...
    def _find_all_tracking_files(
        ...
        if not tracking_dir.exists():
            log.debug(
                "bundle usage tracking directory does not exist. tracking_dir=%s",
                tracking_dir,
            )
            return None
        for file in tracking_dir.iterdir():
            ...
            if not dt:
                ...
                continue
            found.append(
                TrackedBundleVersionInfo(lock_file_path=file, version=version, dt=dt)
            )
        return found
