...
def _rename_pk_constraint_unkown(
    *,
    ...
    if dialect == "postgresql":
        op.execute(
            f"ALTER TABLE {table_name} DROP CONSTRAINT IF EXISTS {original_name}"
        )
        op.execute(
            f"ALTER TABLE {table_name} DROP CONSTRAINT IF EXISTS {alternative_name}"
        )
    elif dialect == "mysql":
...
def upgrade():
    """Rename dataset as asset."""
    _drop_fkey_if_exists(
        "dataset_alias_dataset", "dataset_alias_dataset_dataset_id_fkey"
    )
    ...
    _drop_fkey_if_exists(
        "dataset_alias_dataset_event", "dataset_alias_dataset_dataset_id_fkey"
    )
    _drop_fkey_if_exists(
        "dataset_alias_dataset_event", "dataset_alias_dataset_event_alias_id_fkey"
    )
    _drop_fkey_if_exists(
        "dataset_alias_dataset_event", "dataset_alias_dataset_event_event_id_fkey"
    )
    ...
    _drop_fkey_if_exists(
        "dag_schedule_dataset_alias_reference", "dsdar_dataset_alias_fkey"
    )
    ...
    _drop_fkey_if_exists(
        "dagrun_dataset_event", "dagrun_dataset_events_dag_run_id_fkey"
    )
    ...
    with op.batch_alter_table("task_outlet_asset_reference", schema=None) as batch_op:
        batch_op.create_foreign_key(
            "toar_asset_fkey", "asset", ["asset_id"], ["id"], ondelete="CASCADE"
        )
        batch_op.create_foreign_key(
    ...
    with op.batch_alter_table("asset_dag_run_queue", schema=None) as batch_op:
        batch_op.create_foreign_key(
            "adrq_asset_fkey", "asset", ["asset_id"], ["id"], ondelete="CASCADE"
        )
        batch_op.create_foreign_key(
...
def downgrade():
    ...
    with op.batch_alter_table(
        "dag_schedule_dataset_reference", schema=None
    ) as batch_op:
        batch_op.alter_column(
            "asset_id", new_column_name="dataset_id", type_=sa.Integer(), nullable=False
        )
