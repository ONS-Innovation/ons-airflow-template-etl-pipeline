...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "value", sa.LargeBinary().with_variant(LONGBLOB, "mysql"), nullable=True
        ),
    ...
    if dialect == "postgresql":
        ...
        op.execute(
            "UPDATE xcom SET value_json = CAST(value AS CHAR CHARACTER SET utf8mb4)"
        )
        op.drop_column("xcom", "value")
        op.alter_column(
            "xcom", "value_json", existing_type=sa.JSON(), new_column_name="value"
        )
    elif dialect == "sqlite":
...
def downgrade():
    ...
    if dialect == "postgresql":
        ...
        op.drop_column("xcom", "value")
        op.alter_column(
            "xcom", "value_blob", existing_type=LONGBLOB, new_column_name="value"
        )
