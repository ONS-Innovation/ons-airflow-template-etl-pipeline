...
def _move_offending_dagruns():
    ...
    if (
        not offline
        and conn.execute(text(select_null_logical_date_query)).fetchone() is None
    ):
        return
    ...
    if conn.dialect.name == "mysql":
        ...
        op.execute(
            f"insert into {offending_table_name} {select_null_logical_date_query}"
        )
    else:
        op.execute(
            f"create table {offending_table_name} as {select_null_logical_date_query}"
        )
    ...
    if offline:
        op.execute(
            f"-- TODO: DAG runs unable to be downgraded are moved to {offending_table_name}."
        )
        op.execute(
            f"-- TODO: Table {offending_table_name} can be removed after contained data are reviewed."
        )
