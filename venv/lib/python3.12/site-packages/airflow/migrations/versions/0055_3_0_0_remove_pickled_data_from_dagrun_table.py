...
def upgrade():
        ...
        while True:
            ...
            for row in rows:
                ...
                try:
                    ...
                    print(
                        f"Error converting dagrun conf to json for dagrun ID {row_id}: {e}"
                    )
                    continue
            offset += BATCH_SIZE
    ...
    op.alter_column(
        "dag_run", "conf_json", existing_type=conf_type, new_column_name="conf"
    )
...
def downgrade():
            ...
            for row in rows:
                ...
                try:
                    pickled_data = pickle.dumps(
                        json_data, protocol=pickle.HIGHEST_PROTOCOL
                    )
                    conn.execute(
                        text(
                            """
    ...
    op.alter_column(
        "dag_run", "conf_pickle", existing_type=sa.PickleType(), new_column_name="conf"
    )
