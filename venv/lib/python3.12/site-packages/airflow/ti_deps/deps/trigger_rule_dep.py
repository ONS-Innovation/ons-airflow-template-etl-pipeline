        ...
        *,
        ...
        def _get_relevant_upstream_map_indexes(upstream_id: str) -> int | range | None:
            ...
            if isinstance(ti.task.task_group, MappedTaskGroup):
                is_fast_triggered = ti.task.trigger_rule in (
                    TR.ONE_SUCCESS,
                    TR.ONE_FAILED,
                    TR.ONE_DONE,
                )
        ...
        def _evaluate_setup_constraint(
            *, relevant_setups
            ...
            finished_upstream_tis = (
                x
                for x in dep_context.ensure_finished_tis(
                    ti.get_dagrun(session), session
                )
                if _is_relevant_upstream(
                    upstream=x, relevant_ids=indirect_setups.keys()
                )
            )
        ...
        def _evaluate_direct_relatives() -> Iterator[TIDepStatus]:
            ...
            if not any(t.get_needs_expansion() for t in upstream_tasks.values()):
                ...
                task_id_counts = session.execute(
                    ...
                    .where(
                        or_(*_iter_upstream_conditions(relevant_tasks=upstream_tasks))
                    )
                    .group_by(TaskInstance.task_id)
                ).all()
                upstream = sum(count for _, count in task_id_counts)
        ...
        if not ti.task.is_teardown:
            ...
            relevant_setups = {
                t.task_id: t for t in ti.task.get_upstreams_only_setups()
            }
            if relevant_setups:
