...
def _get_dagbag_dag_details(dag: DAG) -> dict:
    ...
    return {
        ...
        if hasattr(dag, "get_bundle_name")
        else None,
        "bundle_version": dag.get_bundle_version()
        if hasattr(dag, "get_bundle_version")
        else None,
...
def dag_list_dags(args, session: Session = NEW_SESSION) -> None:
    ...
    def filter_dags_by_bundle(
        dags: list[DAG], bundle_names: list[str] | None
    ) -> list[DAG]:
    ...
    AirflowConsole().print_as(
        data=sorted(
            filter_dags_by_bundle(
                dags_list, args.bundle_name if not args.local else None
            ),
...
def dag_reserialize(args, session: Session = NEW_SESSION) -> None:
    ...
    for bundle in all_bundles:
        ...
        dag_bag.sync_to_db(
            bundle.name, bundle_version=bundle.get_current_version(), session=session
        )
