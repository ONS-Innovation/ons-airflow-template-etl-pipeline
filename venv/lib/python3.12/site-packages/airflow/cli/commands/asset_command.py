...
def _detail_asset_alias(args, *, session: Session) -> BaseModel:
    ...
    alias = session.scalar(
        select(AssetAliasModel).where(AssetAliasModel.name == args.name)
    )
    if alias is None:
...
def asset_materialize(args, *, session: Session = NEW_SESSION) -> None:
    ...
    if args.uri:
        ...
        select_message_parts.append(f"URI {args.uri}")
    dag_id_it = iter(
        session.scalars(stmt.group_by(TaskOutletAssetReference.dag_id).limit(2))
    )
    select_message = " and ".join(select_message_parts)
    ...
    dagrun = trigger_dag(
        dag_id=dag_id, triggered_by=DagRunTriggeredByType.CLI, session=session
    )
