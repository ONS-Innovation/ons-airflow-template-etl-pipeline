...
def _get_dag_run(
    *,
    ...
    if not logical_date_or_run_id and not create_if_necessary:
        raise ValueError(
            "Must provide `logical_date_or_run_id` if not `create_if_necessary`."
        )
...
def task_failed_deps(args) -> None:
    ...
    ti, _ = _get_ti(
        task, args.map_index, logical_date_or_run_id=args.logical_date_or_run_id
    )
...
def task_render(args, dag: DAG | None = None) -> None:
    ...
    with create_session() as session, set_current_task_instance_session(
        session=session
    ):
        ti.render_templates()
...
def task_clear(args) -> None:
    ...
    if (
        args.dag_id
        and not args.bundle_name
        and not args.dag_regex
        and not args.task_regex
    ):
        ...
        dags = get_dags(
            args.bundle_name, args.dag_id, use_regex=args.dag_regex, from_db=True
        )
