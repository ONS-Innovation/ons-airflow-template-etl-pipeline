...
CONFIGS_CHANGES = [
    ...
    ConfigChange(
        ...
        renamed_to=ConfigParameter(
            "scheduler", "task_instance_heartbeat_timeout_detection_interval"
        ),
    ),
    ConfigChange(
...
def lint_config(args) -> None:
    ...
    for configuration in CONFIGS_CHANGES:
        ...
        if (
            ...
            and configuration.config.option not in option_to_check_if_provided
        ):
            continue
    ...
    if lint_issues:
        ...
        if args.verbose:
            ...
            console.print(
                f"Ignored sections: [green]{', '.join(ignore_sections)}[/green]"
            )
            console.print(
                f"Ignored options: [green]{', '.join(ignore_options)}[/green]"
            )
        console.print("\n[red]Please update your configuration file accordingly.[/red]")
    else:
        console.print(
            "[green]No issues found in your airflow.cfg. It is ready for Airflow 3![/green]"
        )
...
def update_config(args) -> None:
    ...
    for change in CONFIGS_CHANGES:
        ...
        if update_sections is not None and conf_section not in [
            s.lower() for s in update_sections
        ]:
            continue
        if update_options is not None and full_key not in [
            opt.lower() for opt in update_options
        ]:
            continue
        ...
        if (
            conf_section not in config_dict
            or conf_option not in config_dict[conf_section]
        ):
            continue
        ...
        prefix = (
            "[[red]BREAKING[/red]]"
            if change.breaking
            else "[[yellow]Recommended[/yellow]]"
        )
        if change.default_change:
            if str(current_value) != str(change.new_default):
                modifications.add_default_update(
                    conf_section, conf_option, str(change.new_default)
                )
    ...
    try:
        ...
        raise AirflowConfigException(
            "Backup creation failed. Aborting update_config operation."
        )
    ...
    if changes_applied:
        ...
        console.print(
            "[green]No updates needed. Your configuration is already up-to-date.[/green]"
        )

    if args.verbose:
        console.print(
            "[blue]Configuration update completed with verbose output enabled.[/blue]"
        )
