    ...
    * If *None*, the timedelta is calculated as 10% of the time between the
    ...
    def deserialize(cls, data: dict[str, Any]) -> Timetable:
        ...
        return cls(
            ...
            interval=_deserialize_interval(data["interval"]),
            run_immediately=_deserialize_run_immediately(
                data.get("run_immediately", False)
            ),
        )
    ...
    def _calc_first_run(self) -> DateTime:
        ...
        if (
            self._run_immediately is True
        ):  # Check for 'True' exactly because deltas also evaluate to true.
...
class MultipleCronTriggerTimetable(Timetable):
    ...
    def next_dagrun_info(
        ...
        *,
        ...
        if restriction.catchup:
            ...
            select_key = functools.partial(
                self._dagrun_info_sort_key_no_catchup, now=time.time()
            )
    ...
    @staticmethod
    def _dagrun_info_sort_key_no_catchup(
        info: DagRunInfo | None, *, now: float
    ) -> float:
        """
