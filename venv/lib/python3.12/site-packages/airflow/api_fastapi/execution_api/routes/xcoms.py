...
def get_xcom(
    ...
    if params.offset is not None:
        ...
        if params.offset >= 0:
            xcom_query = xcom_query.order_by(XComModel.map_index.asc()).offset(
                params.offset
            )
        else:
            xcom_query = xcom_query.order_by(XComModel.map_index.desc()).offset(
                -1 - params.offset
            )
    else:
...
def get_mapped_xcom_by_index(
    ...
    if (result := xcom_query.limit(1).first()) is None:
        message = f"XCom with {key=} {offset=} not found for task {task_id!r} in DAG run {run_id!r} of {dag_id!r}"
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={"reason": "not_found", "message": message},
        )
...
def set_xcom(
    ...
    if not run_id:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"Run with ID: `{run_id}` was not found"
        )
    ...
    if dag_run_id is None:
        raise HTTPException(
            ...
            f"DAG run not found on DAG {dag_id} with ID {run_id}",
        )
