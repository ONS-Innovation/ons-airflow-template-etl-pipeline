...
def ti_run(
    ...
    try:
        ...
        log.debug(
            "Retrieved task instance details",
            state=ti.state,
            dag_id=ti.dag_id,
            task_id=ti.task_id,
        )
    ...
    try:
        ...
        if not dr:
            ...
            raise ValueError(
                f"DagRun with dag_id={ti.dag_id} and run_id={ti.run_id} not found."
            )
        ...
        if dag := dag_bag.get_dag(ti.dag_id):
            upstream_map_indexes = dict(
                _get_upstream_map_indexes(
                    dag.get_task(ti.task_id), ti.map_index, ti.run_id, session
                )
            )
        else:
        ...
        context = TIRunContext(
            ...
            should_retry=_is_eligible_to_retry(
                previous_state, ti.try_number, ti.max_tries
            ),
...
def _get_upstream_map_indexes(
    ...
    for upstream_task in task.upstream_list:
        ...
        if not isinstance(upstream_task.task_group, MappedTaskGroup):
            ...
            map_indexes = (
                list(range(mapped_ti_count)) if mapped_ti_count is not None else None
            )
...
def ti_update_state(
    ...
    data = ti_patch_payload.model_dump(
        exclude={"task_outlets", "outlet_events"}, exclude_unset=True
    )
    query = update(TI).where(TI.id == ti_id_str).values(data)
    ...
    try:
        ...
        log.exception(
            "Error updating Task Instance state to %s. Set the task to failed",
            updated_state,
        )
        ti = session.get(TI, ti_id_str)
        query = TI.duration_expression_update(
            datetime.now(tz=timezone.utc), query, session.bind
        )
        query = query.values(state=TaskInstanceState.FAILED)
        _handle_fail_fast_for_dag(
            ti=ti, dag_id=dag_id, session=session, dag_bag=dag_bag
        )
...
def _handle_fail_fast_for_dag(
    ti: TI, dag_id: str, session: SessionDep, dag_bag: DagBagDep
) -> None:
    ...
    if ser_dag and getattr(ser_dag, "fail_fast", False):
        ...
        _stop_remaining_tasks(
            task_instance=ti, task_teardown_map=task_teardown_map, session=session
        )
...
def _create_ti_state_update_query_and_update_state(
    *,
    ...
    if isinstance(
        ti_patch_payload,
        (TITerminalStatePayload, TIRetryStatePayload, TISuccessStatePayload),
    ):
        ti = session.get(TI, ti_id_str)
        updated_state = ti_patch_payload.state
        query = TI.duration_expression_update(
            ti_patch_payload.end_date, query, session.bind
        )
        ...
        if session.get_bind().dialect.name == "mysql":
            ...
            if ti_patch_payload.reschedule_date > _MYSQL_TIMESTAMP_MAX:
                ...
                data = ti_patch_payload.model_dump(
                    exclude={"reschedule_date"}, exclude_unset=True
                )
                query = update(TI).where(TI.id == ti_id_str).values(data)
                query = TI.duration_expression_update(
                    datetime.now(tz=timezone.utc), query, session.bind
...
def ti_heartbeat(
    ...
    old = (
        select(TI.state, TI.hostname, TI.pid)
        .where(TI.id == ti_id_str)
        .with_for_update()
    )
    ...
    session.execute(
        update(TI).where(TI.id == ti_id_str).values(last_heartbeat_at=timezone.utcnow())
    )
...
def get_task_instance_count(
    ...
    if task_group_id:
        group_tasks = _get_group_tasks(
            dag_id, task_group_id, session, logical_dates, run_ids
        )
    ...
    if states:
        if "null" in states:
            ...
            if not_none_states:
                query = query.where(
                    or_(TI.state.is_(None), TI.state.in_(not_none_states))
                )
            else:
...
def validate_inlets_and_outlets(
    ...
    outlets = [
        asset.asprofile() for asset in ti.task.outlets if isinstance(asset, Asset)
    ]
