...
@pools_router.delete(
    ...
    dependencies=[
        Depends(requires_access_pool(method="DELETE")),
...
def delete_pool(
    ...
    if pool_name == "default_pool":
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "Default Pool can't be deleted"
        )

    affected_count = session.execute(
        delete(Pool).where(Pool.pool == pool_name)
    ).rowcount

    if affected_count == 0:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND,
            f"The Pool with name: `{pool_name}` was not found",
        )
...
def get_pools(
    ...
    order_by: Annotated[
        ...
        Depends(
            SortParam(
                ["id", "pool"], Pool, to_replace={"name": "pool"}
            ).dynamic_depends()
        ),
    ],
...
@pools_router.patch(
    ...
    dependencies=[
        Depends(requires_access_pool(method="PUT")),
        Depends(action_logging()),
    ],
)
def patch_pool(
    ...
    if not pool:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND,
            detail=f"The Pool with name: `{pool_name}` was not found",
        )
...
@pools_router.post(
    ...
    dependencies=[
        Depends(requires_access_pool(method="POST")),
        Depends(action_logging()),
    ],
)
def post_pool(
