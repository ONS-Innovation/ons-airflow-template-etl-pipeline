...
def get_log(
    ...
    if not task_log_reader.supports_read:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "Task log handler does not support read logs."
        )
    ...
    if accept == Mimetype.JSON or accept == Mimetype.ANY:  # default
        ...
        if not metadata.get("end_of_log", False):
            encoded_token = URLSafeSerializer(request.app.state.secret_key).dumps(
                metadata
            )
        return TaskInstancesLogResponse.model_construct(
            continuation_token=encoded_token, content=logs
        )
    ...
    if not metadata.get("end_of_log", False):
        headers = {
            "Airflow-Continuation-Token": URLSafeSerializer(
