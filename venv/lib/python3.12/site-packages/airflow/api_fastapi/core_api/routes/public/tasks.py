...
@tasks_router.get(
    ...
    dependencies=[
        Depends(requires_access_dag(method="GET", access_entity=DagAccessEntity.TASK))
    ],
)
def get_tasks(
    dag_id: str,
    ...
    try:
        tasks = sorted(
            dag.tasks,
            key=attrgetter(order_by.lstrip("-")),
...
def get_task(dag_id: str, task_id, dag_bag: DagBagDep) -> TaskResponse:
    ...
    try:
        ...
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"Task with id {task_id} was not found"
        )
