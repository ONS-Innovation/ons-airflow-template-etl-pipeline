...
def get_event_log(
    ...
    if event_log is None:
        raise HTTPException(
            ...
            f"The Event Log with id: `{event_log_id}` not found",
        )
    return event_log
...
def get_event_logs(
    ...
    dag_id: Annotated[
        FilterParam[str | None], Depends(filter_param_factory(Log.dag_id, str | None))
    ],
    task_id: Annotated[
        FilterParam[str | None], Depends(filter_param_factory(Log.task_id, str | None))
    ],
    run_id: Annotated[
        FilterParam[str | None], Depends(filter_param_factory(Log.run_id, str | None))
    ],
    map_index: Annotated[
    ...
    try_number: Annotated[
        FilterParam[int | None],
        Depends(filter_param_factory(Log.try_number, int | None)),
    ],
    owner: Annotated[
        FilterParam[str | None], Depends(filter_param_factory(Log.owner, str | None))
    ],
    event: Annotated[
        FilterParam[str | None], Depends(filter_param_factory(Log.event, str | None))
    ],
    excluded_events: Annotated[
        FilterParam[list[str] | None],
        Depends(
            filter_param_factory(
                Log.event, list[str] | None, FilterOptionEnum.NOT_IN, "excluded_events"
            )
        ),
    ],
    included_events: Annotated[
        ...
        Depends(
            filter_param_factory(
                Log.event, list[str] | None, FilterOptionEnum.IN, "included_events"
            )
        ),
    ],
    before: Annotated[
        ...
        Depends(
            filter_param_factory(
                Log.dttm, datetime | None, FilterOptionEnum.LESS_THAN, "before"
            )
        ),
    ],
    after: Annotated[
        ...
        Depends(
            filter_param_factory(
                Log.dttm, datetime | None, FilterOptionEnum.GREATER_THAN, "after"
            )
        ),
    ],
