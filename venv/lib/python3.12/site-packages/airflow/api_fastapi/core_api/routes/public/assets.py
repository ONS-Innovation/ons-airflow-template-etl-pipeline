...
def get_assets(
    ...
    order_by: Annotated[
        ...
        Depends(
            SortParam(
                ["id", "name", "uri", "created_at", "updated_at"], AssetModel
            ).dynamic_depends()
        ),
    ],
    session: SessionDep,
...
def get_asset_alias(asset_alias_id: int, session: SessionDep):
    ...
    alias = session.scalar(
        select(AssetAliasModel).where(AssetAliasModel.id == asset_alias_id)
    )
    if alias is None:
        raise HTTPException(
...
def get_asset_events(
    ...
    asset_id: Annotated[
        ...
        Depends(filter_param_factory(AssetEvent.asset_id, int | None)),
    ],
    source_dag_id: Annotated[
        FilterParam[str | None],
        Depends(filter_param_factory(AssetEvent.source_dag_id, str | None)),
    ],
    source_task_id: Annotated[
        FilterParam[str | None],
        Depends(filter_param_factory(AssetEvent.source_task_id, str | None)),
    ],
    source_run_id: Annotated[
        FilterParam[str | None],
        Depends(filter_param_factory(AssetEvent.source_run_id, str | None)),
    ],
    source_map_index: Annotated[
        FilterParam[int | None],
        Depends(filter_param_factory(AssetEvent.source_map_index, int | None)),
    ],
    timestamp_range: Annotated[
        RangeFilter, Depends(datetime_range_filter_factory("timestamp", AssetEvent))
    ],
    session: SessionDep,
...
@assets_router.post(
    ...
    responses=create_openapi_http_exception_doc([status.HTTP_404_NOT_FOUND]),
    dependencies=[
        Depends(requires_access_asset(method="POST")),
...
def create_asset_event(
    ...
    asset_model = session.scalar(
        select(AssetModel).where(AssetModel.id == body.asset_id).limit(1)
    )
    if not asset_model:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"Asset with ID: `{body.asset_id}` was not found"
        )
...
def materialize_asset(
    ...
    if (dag_id := next(dag_id_it, None)) is None:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"No DAG materializes asset with ID: {asset_id}"
        )
    ...
    if not (dag := dag_bag.get_dag(dag_id)):
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"DAG with ID `{dag_id}` was not found"
        )
...
def get_asset(
    ...
    asset = session.scalar(
        ...
        .options(
            joinedload(AssetModel.consuming_dags),
    ...
    if asset is None:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"The Asset with ID: `{asset_id}` was not found"
        )
...
def get_dag_asset_queued_events(
    ...
    if not adrqs:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND,
            f"Queue event with dag_id: `{dag_id}` was not found",
        )
...
def get_dag_asset_queued_event(
    ...
    return QueuedEventResponse(
        created_at=adrq.created_at, dag_id=adrq.target_dag_id, asset_id=asset_id
    )
