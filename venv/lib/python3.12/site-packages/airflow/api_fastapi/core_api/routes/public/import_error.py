...
def get_import_error(
    ...
    error = session.scalar(
        select(ParseImportError).where(ParseImportError.id == import_error_id)
    )
    if error is None:
        raise HTTPException(
    ...
    file_dag_ids = set(
        session.scalars(
            select(DagModel.dag_id).where(DagModel.fileloc == error.filename)
        ).all()
    )
...
def get_import_errors(
    ...
    visiable_files_cte = (
        select(DagModel.fileloc, DagModel.dag_id)
    ...
    import_errors_stmt = (
        ...
        .join(
            visiable_files_cte,
            ParseImportError.filename == visiable_files_cte.c.fileloc,
        )
