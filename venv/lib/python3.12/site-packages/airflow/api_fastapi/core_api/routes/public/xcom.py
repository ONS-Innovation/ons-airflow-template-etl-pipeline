...
xcom_router = AirflowRouter(
    tags=["XCom"],
    prefix="/dags/{dag_id}/dagRuns/{dag_run_id}/taskInstances/{task_id}/xcomEntries",
)
...
@xcom_router.get(
    ...
    dependencies=[
        Depends(requires_access_dag(method="GET", access_entity=DagAccessEntity.XCOM))
    ],
)
def get_xcom_entry(
    dag_id: str,
    ...
    if deserialize:
        if not conf.getboolean(
            "api", "enable_xcom_deserialize_support", fallback=False
        ):
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST,
                "XCom deserialization is disabled in configuration.",
            )
    ...
    query = query.join(
        DR, and_(XComModel.dag_id == DR.dag_id, XComModel.run_id == DR.run_id)
    )
    ...
    if item is None:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"XCom entry with key: `{xcom_key}` not found"
        )
...
def create_xcom_entry(
    ...
    try:
        ...
        raise HTTPException(
            status.HTTP_404_NOT_FOUND,
            f"Task with ID: `{task_id}` not found in DAG: `{dag_id}`",
        )
    ...
    if not dag_run:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND,
            f"DAG Run with ID: `{dag_run_id}` not found for DAG: `{dag_id}`",
        )
    ...
    try:
        ...
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
            f"Couldn't serialise the XCom with key: `{request_body.key}`",
        )
