...
from airflow.api_fastapi.core_api.services.ui.grid import (
    fill_task_instance_summaries, get_child_task_map, get_combined_structure,
    get_structure_from_dag, get_task_group_map)
...
def grid_data(
    ...
    order_by: Annotated[
        ...
        Depends(
            SortParam(
                ["run_after", "logical_date", "start_date", "end_date"], DagRun
            ).dynamic_depends()
        ),
    ],
    run_after: Annotated[
        RangeFilter, Depends(datetime_range_filter_factory("run_after", DagRun))
    ],
    logical_date: Annotated[
        RangeFilter, Depends(datetime_range_filter_factory("logical_date", DagRun))
    ],
    ...
    base_query = (
        ...
        .options(
            joinedload(DagRun.task_instances_histories).joinedload(
                TaskInstanceHistory.dag_version
            )
        )
    ...
    if order_by.value == order_by.get_primary_key_string():
        order_by = SortParam(
            allowed_attrs=[run_ordering for run_ordering in dag.timetable.run_ordering],
            model=DagRun,
    ...
    tis_of_dag_runs, _ = paginated_select(
        ...
        filters=[],
        order_by=SortParam(
            allowed_attrs=["task_id", "run_id"], model=TaskInstance
        ).set_value("task_id"),
        offset=offset,
        limit=None,
    )
    ...
    for tis in tis_by_run_id.values():
        ...
        if not version:
            version = session.scalar(
                ...
                .order_by(
                    DagVersion.id
                )  # ascending cus this is mostly for pre-3.0 upgrade
                .limit(1)
            )
