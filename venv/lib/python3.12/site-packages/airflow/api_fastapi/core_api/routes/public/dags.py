...
def get_dags(
    ...
    dag_run_end_date_range: Annotated[
        RangeFilter,
        Depends(datetime_range_filter_factory("dag_run_end_date", DagRun, "end_date")),
    ],
...
def get_dag(
    ...
    if not dag_model:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND,
            f"Unable to obtain dag with id {dag_id} from session",
        )
...
def get_dag_details(
    dag_id: str, session: SessionDep, dag_bag: DagBagDep
) -> DAGDetailsResponse:
    """Get details of DAG."""
    dag: DAG = dag_bag.get_dag(dag_id)
    if not dag:
