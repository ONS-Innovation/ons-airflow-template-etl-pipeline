...
def dag_stats(
    ...
    combined_query = select(
        func.coalesce(func.sum(case((latest_runs.c.is_paused == false(), 1))), 0).label(
            "active"
        ),
        func.coalesce(
            func.sum(case((latest_runs.c.state == DagRunState.FAILED, 1))), 0
        ).label("failed"),
        func.coalesce(
            func.sum(case((latest_runs.c.state == DagRunState.RUNNING, 1))), 0
        ).label("running"),
        func.coalesce(
            func.sum(case((latest_runs.c.state == DagRunState.QUEUED, 1))), 0
        ).label("queued"),
