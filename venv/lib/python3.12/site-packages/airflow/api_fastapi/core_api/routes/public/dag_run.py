...
@dag_run_router.delete(
    ...
    dependencies=[
        Depends(
            requires_access_dag(method="DELETE", access_entity=DagAccessEntity.RUN)
        ),
...
def get_dag_runs(
    ...
    update_at_range: Annotated[
        RangeFilter, Depends(datetime_range_filter_factory("updated_at", DagRun))
    ],
    ...
    if dag_id != "~":
        ...
        if not dag:
            raise HTTPException(
                status.HTTP_404_NOT_FOUND,
                f"The DAG with dag_id: `{dag_id}` was not found",
            )
...
def trigger_dag_run(
    ...
    if not dm:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, f"DAG with dag_id: '{dag_id}' not found"
        )
