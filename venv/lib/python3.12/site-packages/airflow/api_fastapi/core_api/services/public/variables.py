...
class BulkVariableService(BulkService[VariableBody]):
    ...
    def categorize_keys(self, keys: set) -> tuple[set, set]:
        ...
        existing_keys = {
            variable
            for variable in self.session.execute(select(Variable.key)).scalars()
        }
    ...
    def handle_bulk_create(
        self, action: BulkCreateAction, results: BulkActionResponse
    ) -> None:
    ...
    def handle_bulk_update(
        self, action: BulkUpdateAction, results: BulkActionResponse
    ) -> None:
        ...
        try:
            ...
            for variable in action.entities:
                if variable.key in update_keys:
                    old_variable = self.session.scalar(
                        select(Variable).filter_by(key=variable.key).limit(1)
                    )
    ...
    def handle_bulk_delete(
        self, action: BulkDeleteAction, results: BulkActionResponse
    ) -> None:
        ...
        try:
            ...
            for key in delete_keys:
                existing_variable = self.session.scalar(
                    select(Variable).where(Variable.key == key).limit(1)
                )
