...
def get_task_group_map(dag: DAG) -> dict[str, dict[str, Any]]:
    ...
    def _is_task_node_mapped_task_group(
    ...
    def _fill_task_group_map(
        ...
        if isinstance(task_node, MappedOperator):
            ...
            _append_child_task_count_to_parent(
                child_task_count=task_node, parent_node=parent_node
            )
            return
...
def get_child_task_map(parent_task_id: str, task_node_map: dict[str, dict[str, Any]]):
    ...
    return [
        task_id
        for task_id, task_map in task_node_map.items()
        if task_map["parent_id"] == parent_task_id
    ]


def _count_tis(
    node: int | MappedTaskGroup | MappedOperator, run_id: str, session: SessionDep
) -> int:
...
def fill_task_instance_summaries(
    ...
    for (task_id, run_id), tis in grouped_task_instances.items():
        ...
        task_group_map_cache[sdm.id] = task_group_map_cache.get(
            sdm.id
        ) or get_task_group_map(dag=dag)
        task_node_map = task_group_map_cache[sdm.id]
        ti_try_number = max([ti.try_number for ti in tis])
        ti_start_date = min(
            [ti.start_date for ti in tis if ti.start_date], default=None
        )
        ti_end_date = max([ti.end_date for ti in tis if ti.end_date], default=None)
        ti_queued_dttm = min(
            [ti.queued_dttm for ti in tis if ti.queued_dttm], default=None
        )
        ...
        child_states = {
            "no_status" if state is None else state.name.lower(): 0
            for state in state_priority
        }
        ...
        child_states.update(
            {
                ...
                if state is None
                    ...
                    if not task_node_map[task_id]["is_group"]
                        ...
                        for ti in tis
                        if ti.state == state
                        and ti.task_id in get_child_task_map(task_id, task_node_map)
                    ]
                )
                for state in state_priority
            }
        ...
        child_states.update(
            {
                ...
                and (task_node_id, run_id) in overall_states
            }
        )
        ...
        task_instance_summaries_to_fill[run_id].append(
            GridTaskInstanceSummary(
                ...
                task_count=sum(
                    _count_tis(n, run_id, session)
                    for n in task_node_map[task_id]["task_count"]
                ),
...
def get_structure_from_dag(dag: DAG) -> StructureDataResponse:
    ...
    nodes = [
        ...
        for child in get_task_group_children_getter()(dag.task_group)
    ]
