...
class BulkPoolService(BulkService[PoolBody]):
    ...
    def categorize_pools(self, pool_names: set) -> tuple[dict, set, set]:
        ...
        """
        existed_pools = self.session.execute(
            select(Pool).filter(Pool.pool.in_(pool_names))
        ).scalars()
    ...
    def handle_bulk_create(
        self, action: BulkCreateAction[PoolBody], results: BulkActionResponse
    ) -> None:
        """Bulk create pools."""
        ...
        (
            existing_pools_dict,
            matched_pool_names,
            not_found_pool_names,
        ) = self.categorize_pools(to_create_pool_names)
        try:
            if (
                action.action_on_existence == BulkActionOnExistence.FAIL
                and matched_pool_names
            ):
                raise HTTPException(
    ...
    def handle_bulk_update(
        self, action: BulkUpdateAction[PoolBody], results: BulkActionResponse
    ) -> None:
        """Bulk Update pools."""
        to_update_pool_names = {pool.pool for pool in action.entities}
        _, matched_pool_names, not_found_pool_names = self.categorize_pools(
            to_update_pool_names
        )

        try:
            if (
                action.action_on_non_existence == BulkActionNotOnExistence.FAIL
            ...
            for pool in action.entities:
                if pool.pool in update_pool_names:
                    old_pool = self.session.scalar(
                        select(Pool).filter(Pool.pool == pool.pool).limit(1)
                    )

                    data = {
                        key: val
                        for key, val in pool.model_dump(by_alias=True).items()
                        if val is not None
                    }
                    try:
                        PoolBody(**data)
    ...
    def handle_bulk_delete(
        self, action: BulkDeleteAction[PoolBody], results: BulkActionResponse
    ) -> None:
        """Bulk delete pools."""
        ...
        try:
            ...
            for pool_name in delete_pool_names:
                existing_pool = self.session.scalar(
                    select(Pool).where(Pool.pool == pool_name).limit(1)
                )
                if existing_pool:
