...
class BaseAuthManager(Generic[T], LoggingMixin, metaclass=ABCMeta):
    ...
    async def get_user_from_token(self, token: str) -> BaseUser:
        ...
        try:
            payload: dict[
                str, Any
            ] = await self._get_token_validator().avalidated_claims(token)
        ...
        try:
            ...
            log.error(
                "Couldn't deserialize user from token, JWT token is not valid: %s", e
            )
    ...
    def generate_jwt(
        ...
        user: T,
        *,
        expiration_time_in_seconds: int = conf.getint(
    ...
    def filter_authorized_menu_items(
        self, menu_items: list[MenuItem], *, user: T
    ) -> list[MenuItem]:
        """
        ...
        *,
        ...
        return all(
            self.is_authorized_connection(
                method=request["method"], details=request.get("details"), user=user
            )
