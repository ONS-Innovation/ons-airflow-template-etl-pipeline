...
class JWKS:
    ...
    def from_private_key(
        cls, *keys: AllowedPrivateKeys | tuple[AllowedPrivateKeys, str]
    ):
...
def _conf_list_factory(
    section, key, first_only: Literal[True], **kwargs
) -> Callable[[], str]:
    ...
...
class JWTValidator:
    ...
    jwks: JWKS | None = None
    secret_key: str | None = attrs.field(
        repr=False, default=None, converter=lambda v: None if v == "" else v
    )
    ...
    algorithm: list[str] = attrs.field(
        factory=_conf_list_factory("api_auth", "jwt_algorithm", fallback="GUESS"),
        converter=_to_list,
    )

    leeway: float = attrs.field(
        factory=_conf_factory("api_auth", "jwt_leeway"), converter=int
    )

    def __attrs_post_init__(self):
        if not (self.jwks is None) ^ (self.secret_key is None):
            raise ValueError(
                "Exactly one of private_key and secret_key must be specified"
            )
...
class JWTGenerator:
    ...
    _private_key: AllowedPrivateKeys | None = attrs.field(
        repr=False,
        alias="private_key",
        converter=_pem_to_key,
        factory=_load_key_from_configured_file,
    )
    """
    ...
    audience: str
    issuer: str | list[str] | None = attrs.field(
        factory=_conf_list_factory(
            "api_auth", "jwt_issuer", first_only=True, fallback=None
        )
    )
    algorithm: str = attrs.field(
        factory=_conf_list_factory(
            "api_auth", "jwt_algorithm", first_only=True, fallback="GUESS"
        )
    )
    ...
    def generate(
        ...
        return jwt.encode(
            claims, self.signing_arg, algorithm=self.algorithm, headers=headers
        )
...
def get_signing_key(
    section: str, key: str, make_secret_key_if_needed: bool = True
) -> str:
    """
...
def get_signing_args(make_secret_key_if_needed: bool = True) -> dict[str, Any]:
    ...
    return {
        "secret_key": get_signing_key(
            "api_auth", "jwt_secret", make_secret_key_if_needed
        )
    }
...
def get_sig_validation_args(make_secret_key_if_needed: bool = True) -> dict[str, Any]:
    ...
    if key is not None:
        ...
        return {
            "jwks": jwks,
            "algorithm": conf.get("api_auth", "jwt_algorithm", fallback=None)
            or _guess_best_algorithm(key),
        }
