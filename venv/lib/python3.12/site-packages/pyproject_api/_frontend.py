...
class Frontend(ABC):
    ...
    def create_args_from_folder(
        ...
        if py_project_toml.exists():
            ...
            if "backend-path" in build_system:
                backend_paths: tuple[Path, ...] = tuple(
                    folder / p for p in build_system["backend-path"]
                )
            else:
                backend_paths = ()
            if "requires" in build_system:
                requires: tuple[Requirement, ...] = tuple(
                    Requirement(r) for r in build_system.get("requires")
                )
            else:
    ...
    def backend_args(self) -> list[str]:
        ...
        result: list[str] = [
            str(_HERE / "_backend.py"),
            str(self._reuse_backend),
    ...
    def get_requires_for_build_sdist(
        ...
        if self.optional_hooks["get_requires_for_build_sdist"]:
            result, out, err = self._send(
                cmd="get_requires_for_build_sdist", config_settings=config_settings
            )
        else:
            result, out, err = [], "", ""
        if not isinstance(result, list) or not all(isinstance(i, str) for i in result):
            self._unexpected_response(
                "get_requires_for_build_sdist", result, "list of string", out, err
            )
        return RequiresBuildSdistResult(
            tuple(Requirement(r) for r in cast("list[str]", result)), out, err
        )
    ...
    def get_requires_for_build_wheel(
        ...
        if self.optional_hooks["get_requires_for_build_wheel"]:
            result, out, err = self._send(
                cmd="get_requires_for_build_wheel", config_settings=config_settings
            )
        else:
            result, out, err = [], "", ""
        if not isinstance(result, list) or not all(isinstance(i, str) for i in result):
            self._unexpected_response(
                "get_requires_for_build_wheel", result, "list of string", out, err
            )
        return RequiresBuildWheelResult(
            tuple(Requirement(r) for r in cast("list[str]", result)), out, err
        )
    ...
    def get_requires_for_build_editable(
        ...
        if self.optional_hooks["get_requires_for_build_editable"]:
            result, out, err = self._send(
                cmd="get_requires_for_build_editable", config_settings=config_settings
            )
        else:
            result, out, err = [], "", ""
        if not isinstance(result, list) or not all(isinstance(i, str) for i in result):
            self._unexpected_response(
                "get_requires_for_build_editable", result, "list of string", out, err
            )
        return RequiresBuildEditableResult(
            tuple(Requirement(r) for r in cast("list[str]", result)), out, err
        )
    ...
    def prepare_metadata_for_build_wheel(
        ...
        if not isinstance(basename, str):
            self._unexpected_response(
                "prepare_metadata_for_build_wheel", basename, str, out, err
            )
    ...
    def build_sdist(
        self, sdist_directory: Path, config_settings: ConfigSettings | None = None
    ) -> SdistResult:
        """
    ...
    def _unexpected_response(
        ...
        raise BackendFailed(
            {"code": None, "exc_type": TypeError.__name__, "exc_msg": msg}, out, err
        )
    ...
    def metadata_from_built(
        ...
        with self._wheel_directory() as wheel_directory:
            result: EditableResult | WheelResult = hook(
