...
def run(argv):  # noqa: C901, PLR0912, PLR0915
    ...
    try:
        backend_proxy = BackendProxy(
            argv[1], None if len(argv) == 2 else argv[2]
        )  # noqa: PLR2004
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                print(
                    "Backend: run command {} with args {}".format(
                        cmd, parsed_message["kwargs"]
                    )
                )
                outcome = backend_proxy(
                    parsed_message["cmd"], **parsed_message["kwargs"]
                )
                ...
                if not isinstance(
                    exception, MissingCommand
                ):  # for missing command do not print stack
                    traceback.print_exc()
            finally:
                try:
                    encoding = locale.getpreferredencoding(do_setlocale=False)
                    with open(
                        result_file, "w", encoding=encoding
                    ) as file_handler:  # noqa: PTH123
                    ...
                    flush()  # pragma: no branch
        if (
            reuse_process is False
        ):  # pragma: no branch # no test for reuse process in root test env
            break
    return 0
