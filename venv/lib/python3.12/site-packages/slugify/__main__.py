...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    input_group.add_argument(
        "--stdin", action="store_true", help="Take the text from STDIN"
    )
    ...
    parser.add_argument(
        "--max-length", type=int, default=0, help="Output string length, 0 for no limit"
    )
    parser.add_argument(
        "--word-boundary",
        action="store_true",
    ...
    parser.add_argument(
        "--regex-pattern", help="Python regex pattern for disallowed characters"
    )
    parser.add_argument(
        "--no-lowercase",
        action="store_false",
    ...
    if args.replacements:
        ...
        def split_check(repl):
            ...
            if SEP not in repl:
                parser.error(
                    "Replacements must be of the form: ORIGINAL{SEP}REPLACED".format(
                        SEP=SEP
                    )
                )
