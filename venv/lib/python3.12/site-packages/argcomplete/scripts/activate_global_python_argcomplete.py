...
parser.add_argument(
    "--dest",
    help='Specify the shell completion modules directory to install into, or "-" for stdout',
)
...
def get_activator_dir():
    return os.path.join(
        os.path.abspath(os.path.dirname(argcomplete.__file__)), "bash_completion.d"
    )
    ...
    if os.path.exists(path):
        with open(path, "r") as fh:
            if shellcode in fh.read():
                ...
                return
        print(
            f"argcomplete needs to append to the file {path}. The following code will be appended:",
            file=sys.stderr,
        )
...
def link_zsh_user_rcfile(zsh_fpath=None):
    zsh_rcfile = os.path.join(
        os.path.expanduser(os.environ.get("ZDOTDIR", "~")), ".zshenv"
    )
    append_to_config_file(
        zsh_rcfile, zsh_shellcode.format(zsh_fpath=zsh_fpath or get_activator_dir())
    )
...
def link_bash_user_rcfile():
    ...
    append_to_config_file(
        bash_completion_user_file, bash_shellcode.format(activator=get_activator_path())
    )
...
def add_zsh_system_dir_to_fpath_for_user():
    ...
    try:
        ...
        fpath_output = subprocess.check_output(
            [os.environ["SHELL"], "-c", 'printf "%s\n" "${fpath[@]}"']
        )
...
def main():
    ...
    if args.dest:
        if args.dest != "-" and not os.path.exists(args.dest):
            parser.error(
                f"directory {args.dest} was specified via --dest, but it does not exist"
            )
        destinations.append(args.dest)
    elif (
        site.ENABLE_USER_SITE
        and site.USER_SITE
        and site.USER_SITE in argcomplete.__file__
    ):
        print(
        ...
        print(
            "Argcomplete was installed in a virtual environment. Defaulting to user installation.",
            file=sys.stderr,
        )
        link_user_rcfiles()
    elif args.user:
